...
sub transfer_details {
	...
	for $i ( 1 .. $form->{rowcount} - 1 ) {
		...
		push(
			@{ $form->{qty} },
			$form->format_amount( $myconfig, $form->{"qty_$i"}, 2 )
		);
		push(
			@{ $form->{cost} },
			$form->format_amount( $myconfig, $form->{"cost_$i"}, 2 )
		);
		push(
			@{ $form->{netweight} },
			$form->format_amount(
				$myconfig, $form->{"weight_$i"} * $form->{"qty_$i"}, 2
			)
		);
	...
	$form->{totalnetweight} =
	  $form->format_amount( $myconfig, $form->{totalnetweight}, 2 );
...
sub retrieve_item {
	...
	if ( $form->{from_warehouse_id} ) {
		$onhandfld =
"(SELECT SUM(qty) FROM inventory i WHERE i.parts_id = p.id AND warehouse_id = $form->{from_warehouse_id}) AS onhand";
	}
