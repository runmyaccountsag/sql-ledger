...
sub edit_batch {
	...
	( $form->{batchnumber}, $form->{batchdescription}, $form->{transdate} ) =
...
sub save_batch {
	...
	$form->{batchnumber} =
	  $form->update_defaults( $myconfig, 'batchnumber', $dbh )
	  unless $form->{batchnumber};
...
sub list_batches {
	...
	my $where = "1 = 1";
	$where .= " AND a.batch = |.$dbh->quote($form->{batch}).qq|"
	  if $form->{batch};
	...
	$where .= " AND a.transdate <= " . $dbh->quote( $form->{transdateto} ) . ""
	  if $form->{transdateto};
...
sub list_vouchers {
	...
	(
		...
		$form->{transdate}, $form->{apprdate}, $form->{batch}
	) = $dbh->selectrow_array($query);
	...
	if ( $form->{batch} eq 'ap' ) {
		...
		$query =
		  qq|SELECT g.id, g.reference AS invnumber, g.description AS name,
...
sub delete_payment_reversal {
	...
	if ( $trans_id *= 1 ) {
		$form->update_balance( $dbh, 'ap', 'paid', qq|id = $trans_id|,
			$amount );
...
sub delete_batch {
	...
	while ( my ( $voucherid, $trans_id ) = $sth->fetchrow_array ) {
		...
		if ($pth) {
			...
			$form->update_balance( $dbh, $table, 'paid', qq|id = $trans_id|,
				$amount );
		}
...
sub payment_reversal {
	...
	if ( $form->{id} *= 1 ) {
		...
		WHERE vr.id = $form->{id}
		AND c.link LIKE '%AP_paid%'|;
		( $form->{source}, $form->{memo}, $accno, $description, $translation ) =
		  $dbh->selectrow_array($query);
...
sub post_payment_reversal {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		if ($ap_id) {
			...
			# post offsetting AP
			$pth->execute( $ref->{trans_id}, $ap_id, $ref->{amount},
				$ref->{fx_transaction} );
			$pth->finish;
