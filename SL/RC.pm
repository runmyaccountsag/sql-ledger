...
sub payment_transactions {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [ 'fx%_accno_id', 'precision', 'company' ] } );
	...
	if ( $form->{fromdate} ) {
		$transdate .=
		  qq| AND ac.transdate >= | . $dbh->quote( $form->{fromdate} ) . qq||;
	}
	if ( $form->{todate} ) {
		$transdate .=
		  qq| AND ac.transdate <= | . $dbh->quote( $form->{todate} ) . qq||;
	}
	...
	if ( $form->{report} ) {
		...
		if ( $form->{fromdate} || $form->{todate} ) {
			$cleared = "";
			if ( $form->{fromdate} ) {
				$cleared = qq| AND ac.cleared >= |
				  . $dbh->quote( $form->{fromdate} ) . qq||;
			}
			if ( $form->{todate} ) {
				$cleared .= qq| AND ac.cleared <= |
		...
		if ( $form->{summary} ) {
			...
			if ( "$ref->{transdate}$ref->{source}" eq $sameitem ) {
				if ( $ref->{fx_transaction} ) {
					...
					push @{ $form->{PR}->[$i]->{name} }, $ref->{description}
					  if $ref->{description} ne $samename;
					$form->{PR}->[$i]->{amount} += $ref->{amount};
					$form->{PR}->[$i]->{id} .= " $ref->{id}"
					  if $form->{PR}->[$i]->{id} !~ /$ref->{id}/;
	...
	for $i ( 1 .. $form->{rowcount} ) {
		if ( $form->{"datecleared_$i"} ne $form->{"oldcleared_$i"} ) {
			...
			foreach $payment_id ( split / /, $form->{"payment_id_$i"} ) {
	            ...
	            cleared = |
				  . $form->dbquote( $form->dbclean($cleared), SQL_DATE ) . qq|
