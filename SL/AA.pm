...
sub post_transaction {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [ 'fx%accno_id', 'cdt', 'precision', 'extendedlog' ] } );
	...
	my %zerotax
	  ; # Taxes checked or selected in this transaction to decide 0 tax posting.
	for ( 1 .. $form->{rowcount} ) {
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $amount{fxamount}{$i} or $form->{"tax_$i"} ) {
			...
			if ( $form->{currency} ne $form->{defaultcurrency} ) {
				...
				$fxlinetaxamount =
				  $form->{"linetaxamount_$i"} * $form->{exchangerate};
				$form->{"linetaxamount_$i"} =
				  $fxlinetaxamount - $form->{"linetaxamount_$i"};
	...
	if ( $form->{id} *= 1 ) {
		...
		if ( $form->{id} ) {
			if ( $defaults{extendedlog} ) {
				$query =
qq|INSERT INTO ${table}_log SELECT ${table}.* FROM $table WHERE id = $form->{id}|;
        ...
        |;
            ...
            WHERE trans_id = $form->{id}|;
				...
				$query =
qq|UPDATE ${table} SET ts = NOW() + TIME '00:00:01' WHERE id = $form->{id}|;
				$dbh->do($query) || $form->dberror($query);
...
# philipp_kroll overworked rounding issue fix 2018-09-26 ignoring credit, debit, credit_note, debit_note in calculation of amount.
	if ( $invamount eq $paid ) {
		...
		if ($invamount2) {
			my ($transdate) = $dbh->selectrow_array(
"select max(transdate) from acc_trans where trans_id = $form->{id}"
			);
			($accno) = split /--/, $form->{$ARAP};
...
# this acc_trans entry shall always be the opposite of the follow up query, so we multiply by -1 no matter if creadit, debit, credit_note, debit_note.
			$query =
qq|INSERT INTO acc_trans (trans_id, chart_id, amount, transdate, approved)
...
sub delete_transaction {
	...
	if ( $form->{id} and $defaults{extendedlog} ) {
		$query =
qq|INSERT INTO ${table}_log_deleted SELECT ${table}.* FROM $table WHERE id = $form->{id}|;
		$dbh->do($query) || $form->dberror($query);
...
sub transactions {
	...
	if ( $form->{outstanding} ) {
		$paid =
qq|CASE WHEN ROUND(a.fxamount::numeric, $form->{precision}) = ROUND(a.fxpaid::numeric, $form->{precision}) AND a.fxpaid != 0 AND a.paid != 0 |;
		$paid .= qq|
	...
	if ( $form->{"$form->{vc}_id"} ) {
		$where .= qq| AND a.$form->{vc}_id = |
		  . $form->dbclean( $form->{"$form->{vc}_id"} ) . qq||;
	}
	else {
		if ( $form->{ $form->{vc} } ) {
		...
		if ( $form->{"$form->{vc}number"} ) {
			...
			$where .=
			  " AND lower(vc.$form->{vc}number) LIKE " . $dbh->quote($var) . "";
		}
	}
...
sub company_details {
	...
	my @a =
	  qw(weightunit cdt company companyemail companywebsite address tel fax businessnumber);
	...
	if ( $form->{vc} eq 'customer' ) {
		...
		for (qw(iban bic membernumber rvc dcn)) {
			$form->{$_} = $form->{"bank$_"};
		}
	}
