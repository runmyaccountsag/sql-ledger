...
sub get_openvc {
	...
	if ( !$form->{"select$form->{vc}"} ) {
		...
		if ( $form->{"$form->{vc}number"} ) {
			...
			$where .= qq| AND lower(vc.$form->{vc}number) LIKE |
			  . $dbh->quote($var) . qq||;
		}
	}
...
sub retrieve {
	...
	$query = qq|SELECT br.description, vr.vouchernumber
	      ...
	      WHERE vr.id = $form->{id}|;
	( $form->{batchdescription}, $form->{vouchernumber} ) =
...
sub get_openinvoices {
	...
	my %defaults =
	  $form->get_defaults( $dbh, \@{ [qw(namesbynumber cdt precision)] } );
	...
	$where .= qq|
              AND a.curr = | . $dbh->quote( $form->{currency} ) . qq||
	  if $form->{currency};
	$where .= qq|
	      AND a.duedate >= | . $dbh->quote( $form->{duedatefrom} ) . qq||
	  if $form->{duedatefrom};
	...
	my $datepaid =
	  ( $form->{datepaid} ) ? "date '$form->{datepaid}'" : 'current_date';
	...
	my $query = qq|SELECT DISTINCT a.id, a.invnumber, a.transdate, a.duedate,
		 ...
		 JOIN |
	  . $form->dbclean( $form->{vc} )
	  . qq| vc ON (vc.id = a.|
	  . $form->dbclean( $form->{vc} ) . qq|_id)
		 JOIN chart ch ON (ch.id = ac.chart_id)
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		$ref->{fxdue} = $form->round_amount(
			( $ref->{amount} - $ref->{paid} ) / $ref->{vcexch},
			$form->{precision} );
		...
		if ( $form->{vc} eq 'vendor' ) {
			if ( $ref->{threshold} > 0 ) {
				$total{ $ref->{"$form->{vc}_id"} } =
				  $form->round_amount( $total{ $ref->{"$form->{vc}_id"} },
					$form->{precision} );
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"checked_$i"} ) {
		  ...
		  WHERE trans_id = $form->{"id_$i"}|;
			...
			$amount =
			  $form->round_amount(
				$form->{"paid_$i"} * $trans{ $form->{"id_$i"} }{exchangerate},
				$form->{precision} );
			...
			$query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
			  ...
			  . $form->dbclean($arap) . qq|, '|
			  . $form->dbclean( $form->{datepaid} ) . qq|',
		  $amount * $ml, '$approved',
		  $voucherid)|;
			$dbh->do($query) || $form->dberror($query);
		  ...
		  $paymentmethod_id)|;
			...
			$amount =
			  $form->round_amount(
				$form->{"paid_$i"} * ( $form->{exchangerate} - 1 ) * $ml * -1,
				$form->{precision} );
		  ...
		  $voucherid)|;
		    ...
		    '|
				  . $form->dbclean( $form->{datepaid} )
				  . qq|', $amount, '1', '$approved',
		    $voucherid)|;
				$dbh->do($query) || $form->dberror($query);
			}
			...
			if ( $form->{"discount_$i"} ) {
				...
				if ( $defaults{cdt}
					&& !$trans{ $form->{"id_$i"} }{taxincluded} )
				{

					$tth->execute( $form->{"id_$i"},
						$trans{ $form->{"id_$i"} }{transdate} )
					  || $form->dberror;
					...
					$cdt =
					  $form->round_amount( $form->{"discount_$i"} * $totalrate,
						$form->{precision} );
					...
					$cdt =
					  $form->round_amount(
						$cdt * $trans{ $form->{"id_$i"} }{exchangerate},
						$form->{precision} );
				...
				for ( keys %cdt ) {
					...
					$amount =
					  $form->round_amount(
						$cdt{$_} * $trans{ $form->{"id_$i"} }{exchangerate},
						$form->{precision} );
					$query =
					  qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
		      ...
		      $voucherid, $form->{"id_$i"})|;
					...
					$amount =
					  $form->round_amount(
						$cdt{$_} * ( $form->{exchangerate} - 1 ) * $ml * -1,
						$form->{precision} );
...
qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
			...
			$fxpaid = $form->round_amount(
				$form->{"paid_$i"} +
				  $trans{ $form->{"id_$i"} }{paid} /
				  $trans{ $form->{"id_$i"} }{exchangerate},
				$form->{precision}
			);
			...
			$form->{"paid_$i"} =
			  $form->round_amount(
				$form->{"paid_$i"} * $trans{ $form->{"id_$i"} }{exchangerate},
				$form->{precision} );
			$form->{"discount_$i"} = $form->round_amount(
				$form->{"discount_$i"} *
				  $trans{ $form->{"id_$i"} }{exchangerate},
				$form->{precision}
			);
		  ...
		  WHERE id = | . $form->dbclean( $form->{"id_$i"} ) . qq||;
			...
			my ( $amount, $paid, $fxamount, $fxpaid ) = $dbh->selectrow_array(
qq|SELECT amount, paid, fxamount, fxpaid FROM ar WHERE id = $form->{"id_$i"}|
			);

			if (    ( $fxamount eq $fxpaid )
				and ( $amount ne $paid )
				and ( $form->{exchangerate} ne 1 ) )
			{
				$correction =
				  $form->round_amount( $amount - $paid, $form->{precision} );

				$dbh->do(
qq|UPDATE $form->{arap} SET paid = amount WHERE id = $form->{"id_$i"}|
				);
              ...
              and entry_id = (
                select entry_id from acc_trans where trans_id = |
				  . $form->dbclean( $form->{"id_$i"} ) . qq|
                and chart_id in (select id from chart where link = |
				  . $dbh->quote( $form->{ARAP} )
...
qq|select count(*) from acc_trans where trans_id = $form->{"id_$i"} and chart_id in ($defaults{fxgain_accno_id}, $defaults{fxloss_accno_id})|
				);
				if ($has_gain_or_loss) {
					$query = qq|
                update acc_trans 
