...
sub invoice_details {
	...
	foreach $item (@sortlist) {
		...
		if ( $form->{groupprojectnumber} || $form->{grouppartsgroup} ) {
			if ( $item->[1] ne $sameitem ) {
				...
				if ($ok) {
					...
					for (
						qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes package netweight grossweight volume countryorigin hscode barcode xml_deliverydate xml_qty xml_sellprice xml_linetotal)
					  )
					{
						push( @{ $form->{$_} }, "" );
					}
		...
		if ( $form->{"qty_$i"} ) {
			...
			for (
				qw(sku serialnumber ordernumber customerponumber bin description unit deliverydate sellprice listprice package netweight grossweight volume countryorigin hscode barcode itemnotes)
			  )
			{
			...
			if ($subtotal) {
				if ( $j < $k ) {
					...
					if ( $sortlist[$j]->[1] ne $sameitem ) {
						...
						for (
							qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate itemnotes package netweight grossweight volume countryorigin hscode barcode xml_deliverydate xml_qty xml_sellprice)
						  )
						{
							push( @{ $form->{$_} }, "" );
						}
	...
	for ( @{ $form->{lineitems} } ) {
		push(
			@{ $form->{linetax} },
			$form->format_amount(
				$myconfig, $_->{tax}, $form->{precision}, '0.00'
			)
		);
	}
	...
	for ( sort keys %taxaccs ) {
		...
		push(
			@{ $form->{taxbase} },
			$form->format_amount(
				$myconfig, $taxbase{$_}, $form->{precision}, '0.00'
			)
		);
		...
		push(
			@{ $form->{tax} },
			$form->format_amount(
				$myconfig, $taxaccounts{$_}, $form->{precision}, '0.00'
			)
		);
...
# Now remove all taxes which are not applicable to this invoice from db and from $form.
	$query =
	  qq|SELECT id, accno FROM chart WHERE id IN (SELECT chart_id FROM tax)|;
	$sth = $dbh->prepare($query);
	$sth->execute || $form->dberror($query);
	$form->{id} *= 1;
	while ( my $row = $sth->fetchrow_hashref(NAME_lc) ) {
		if ( !$taxaccs{ $row->{accno} } ) {
			map { delete $form->{"$row->{accno}_$_"} }
			  (
				qw(tax taxbase taxbaseinclusive taxrate description rate taxnumber)
			  );
			$dbh->do(
"DELETE FROM acc_trans WHERE trans_id = $form->{id} AND chart_id = $row->{id} AND amount = 0"
			);
		}
	}
	$sth->finish;
	...
	(
		$form->{iban}, $form->{bic}, $form->{membernumber}, $form->{dcn},
		$form->{rvc}
...
sub delete_invoice {
	...
	if ( $form->{id} and $defaults{extendedlog} ) {
		$query =
qq|INSERT INTO ar_log_deleted SELECT ar.* FROM ar WHERE id = $form->{id}|;
		$dbh->do($query) || $form->dberror($query);
...
sub assembly_details {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		if ( $form->{grouppartsgroup} && $ref->{partsgroup} ne $sm ) {
			for (
				qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber unit qty ship bin deliverydate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes package netweight grossweight volume countryorigin hscode barcode)
			  )
			{
				push( @{ $form->{$_} }, "" );
			}
		...
		if ( $form->{stagger} ) {
			...
			for (
				qw(taxrates runningnumber ship serialnumber ordernumber customerponumber reqdate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes package netweight grossweight volume countryorigin hscode barcode)
			  )
			{
				push( @{ $form->{$_} }, "" );
			}
...
sub post_invoice {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [ 'fx%_accno_id', 'cdt', 'precision', 'extendedlog' ] } );
	...
	if ( $form->{id} *= 1 ) {
		...
		if ( $dbh->selectrow_array($query) ) {
			...
			if ( $form->{oe_id} ) {
				$form->{oe_id} *= 1;
				$dbh->do(
					"DELETE FROM inventory WHERE trans_id = $form->{oe_id}")
				  ; # Delete any 'inventory' transactions saved from order. For existing invoices.
			}
			if ( $defaults{extendedlog} ) {
				$query =
qq|INSERT INTO ar_log SELECT ar.* FROM ar WHERE id = $form->{id}|;
				$dbh->do($query) || $form->dberror($query);
				$query = qq|
        ...
        |;
				...
				$query =
qq|UPDATE ar SET ts = NOW() + TIME '00:00:01'  WHERE id = $form->{id}|;
				$dbh->do($query) || $form->dberror($query);
			...
			if ( $form->{order_id} ) {
				$form->{order_id} *= 1;
				$dbh->do(
					"DELETE FROM inventory WHERE trans_id = $form->{order_id}")
				  ;    # Delete any 'inventory' transactions saved from order.
			}
		}
	...
	if ( $form->{department_id} ) {
		$query = qq|INSERT INTO dpt_trans (trans_id, department_id)
                VALUES ($form->{id}, |
		  . $form->dbclean( $form->{department_id} ) . qq|)|;
		$dbh->do($query) || $form->dberror($query);
	}
	...
	foreach $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"qty_$i"} ) {
		  ...
		  qty = | . $form->dbclean( $form->{"qty_$i"} ) . qq|,
                  sellprice = |
			  . $form->dbclean( $form->{"sellprice_$i"} ) . qq|,
		  fxsellprice = $fxsellprice,
		  discount = $form->{"discount_$i"},
		  allocated = $allocated,
		  unit = | . $dbh->quote( $form->{"unit_$i"} ) . qq|,
		  transdate = |
			  . $form->dbquote( $form->dbclean( $form->{"transdate"} ),
				SQL_DATE )
			  . qq|,
		  deliverydate = |
			  . $form->dbquote( $form->dbclean( $form->{"deliverydate_$i"} ),
				SQL_DATE )
			  . qq|,
...
"SELECT 1 FROM customertax WHERE customer_id = $form->{customer_id} AND chart_id IN (SELECT id FROM chart WHERE accno = '$_')"
				);
				if ($ok) {
					$taxamount = $linetotal * $form->{"${_}_rate"}
					  if $form->{"${_}_rate"} != 0;
...
qq|INSERT INTO invoicetax (trans_id, invoice_id, chart_id, amount, taxamount)
			...
			itemnotes, description, invoice_id)
                VALUES (|
			  . $form->dbclean( $form->{warehouse_id} ) . qq|, |
			  . $form->dbclean( $form->{"id_$i"} )
			  . qq|, $form->{id},
                        1, 0 - (|
			  . $form->dbclean( $form->{"qty_$i"} ) . qq|), |
			  . $form->dbquote( $form->dbclean( $form->{"transdate"} ),
				SQL_DATE )
			  . qq|, |
			  . $form->dbclean( $form->{employee_id} )
			  . qq|, $form->{department_id}, |
	...
	if (
		$form->round_amount( $form->{paid} - ( $fxamount + $fxtax_total ),
			$form->{precision} ) == 0
	  )
	{
		$form->{paid} = $invamount;
		...
		@{ $form->{acc_trans}{lineitems} } )
	{
		...
		VALUES ($form->{id}, $ref->{chart_id}, $amount,
	      |
		  . $dbh->quote( $form->{transdate} )
		  . qq|, $ref->{project_id}, $ref->{id})|;
		$dbh->do($query) || $form->dberror($query);
		$diff   = 0;
		$fxdiff = 0;
	}
	...
	for $i ( 1 .. $form->{paidaccounts} ) {
		...
		if ( $form->{"paid_$i"} ) {
			...
			$form->{id}, $voucherid, |
						  . $dbh->quote( $form->{voucher}{payment}{$voucherid}
							  {vouchernumber} )
						  . qq|)|;
						$dbh->do($query) || $form->dberror($query);
...
qq|id = $form->{voucher}{payment}{$voucherid}{br_id}|,
			...
			if ($keepcleared) {
				$cleared =
				  $form->dbquote( $form->dbclean( $form->{"cleared_$i"} ),
					SQL_DATE );
			}
		  ...
		  $paymentmethod_id)|;
			...
			if ($amount) {
				...
				if ( $form->round_amount( $amount, 1 ) ) {
					...
					$accno_id =
					  ( $amount > 0 )
					  ? $defaults{fxgain_accno_id}
					  : $defaults{fxloss_accno_id};
				}
				else {

					# rounding difference
					($accno_id) = $dbh->selectrow_array(
						"SELECT id FROM chart WHERE accno = '$araccno'");
				}
				$query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
			...
			$amount =
			  ( $paymentadjust1 + $paymentadjust2 + $paymentadjust3 ) * -1
			  ; # Override calculated value above to fix outstanding report difference
	...
	if ( !$form->{importing} )
	{    # We are not coming from data import script im.pl
		( $form->{membernumber}, $form->{dcn} ) = $dbh->selectrow_array($query);

		$form->{dcn} =
		  ( $form->{dcn} == '<%external%>' )
		  ? ''
		  : $form->format_dcn( $form->{dcn} );
	}
	...
	my $fxtotalamount = 0;
	$fxtotalamount =
	  $form->round_amount( $fxtax_total, $form->{precision} ) + $fxamount;

	if (    ( $fxtotalamount eq $fxtotalamount_paid )
		and ( $invamount ne $form->{paid} ) )
	{
		$correction =
		  $form->round_amount( $invamount - $form->{paid}, $form->{precision} );
		$form->{paid} = $invamount;
		$query = qq|
         update acc_trans 
         ...
         and entry_id = (
               ...
               and chart_id in (select id from chart where accno = |
		  . $dbh->quote($araccno)
		  . qq|) and amount > 0 limit 1
               )
     |;
		$dbh->do($query) or $form->error($query);

		my ($has_gain_or_loss) = $dbh->selectrow_array(
qq|select count(*) from acc_trans where trans_id = $form->{id} and chart_id in ($defaults{fxgain_accno_id}, $defaults{fxloss_accno_id}) limit 1|
		);
		if ($has_gain_or_loss) {
			$query = qq|
           update acc_trans 
	...
	$query = qq|UPDATE ar set
	      ...
	      quonumber = | . $dbh->quote( $form->{quonumber} ) . qq|,
              ...
              fxpaid = $fxpaid_total,
	      datepaid = |
	  . $form->dbquote( $form->dbclean( $form->{datepaid} ), SQL_DATE ) . qq|,
	      duedate = |
	  . $form->dbquote( $form->dbclean( $form->{duedate} ), SQL_DATE ) . qq|,
	      invoice = '1',
...
"DELETE FROM acc_trans WHERE trans_id = $form->{id} AND chart_id = $row->{id} AND amount = 0"
	...
	( $transdate, $diff ) = $dbh->selectrow_array(
		"SELECT transdate, amount-paid FROM ar WHERE id = $form->{id}");
	if ( $diff == 0 ) {    # Invoice is fully paid
		$ar_amount =
		  $dbh->selectrow_array("SELECT amount FROM ar WHERE id = $form->{id}");
     ...
     " );
		...
		if ( $ar_amount != $ac_amount ) {
			$dbh->do(
"UPDATE ar SET amount = $ac_amount, netamount = $ac_netamount, paid = $ac_amount WHERE id = $form->{id}"
			) or $form->dberror('Error running query ...');
			$dbh->commit;
		}

		# Now check if there is minor difference in the AR posting
		$query =
qq|SELECT ROUND(sum(amount)::numeric,2) FROM acc_trans WHERE trans_id=$form->{id} AND chart_id IN (SELECT id FROM chart WHERE link = 'AR')|;
		($ar_amount) = $dbh->selectrow_array($query);
		if ( $ar_amount != 0 ) {
			($ar_accno_id) = $dbh->selectrow_array(
				qq|
            SELECT chart_id FROM acc_trans WHERE trans_id=$form->{id} AND chart_id IN (SELECT id FROM chart WHERE link = 'AR') LIMIT 1|
			);
			($income_accno_id) = $dbh->selectrow_array(
				qq|
			...
			$query =
qq|INSERT INTO acc_trans (trans_id, chart_id, transdate, amount, memo) VALUES ($form->{id}, $income_accno_id, '$transdate', $ar_amount, 'rounding adjustment')|;
			$dbh->do($query) or $form->error($query);
			$ar_amount *= -1;
			$query =
qq|INSERT INTO acc_trans (trans_id, chart_id, transdate, amount, memo) VALUES ($form->{id}, $ar_accno_id, '$transdate', $ar_amount, 'rounding adjustment')|;
			$dbh->do($query) or $form->error($query);
			$dbh->commit;
		}
	}
		...
		if ( $ref->{assembly} ) {
			...
			if ( $ref->{inventory_accno_id} ) {
				if ( $ref->{qty} > 0 ) {
					...
					$allocated =
					  &cogs_returns( $dbh, $form, $ref->{parts_id}, $ref->{qty},
						$project_id, $i );
				}
			}
		}
...
sub cogs_returns {
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		$linetotal = $form->round_amount( $form->{"sellprice_$i"} * $qty,
			$form->{precision} );
...
sub retrieve_invoice {
	...
	if ( $form->{id} *= 1 ) {
		...
		$form->{type} =
		  ( $form->round_amount( $form->{amount}, $form->{precision} ) < 0 )
		  ? 'credit_invoice'
		  : 'invoice';
		...
		FROM invoice i
	        ...
	        LEFT JOIN partsgroup pg ON (p.partsgroup_id = pg.id)
		LEFT JOIN translation t ON (t.trans_id = p.partsgroup_id AND t.language_code = |
		...
		while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
			...
			&price_matrix( $pmh, $ref, $form->{transdate}, $decimalplaces,
				$form, $myconfig );
...
sub retrieve_item {
	...
	my $query = qq|SELECT p.id, p.partnumber, p.description, p.sellprice,
		 ...
		 LEFT JOIN translation t1 ON (t1.trans_id = p.id AND t1.language_code = |
	  . $dbh->quote( $form->{language_code} ) . qq|)
		 LEFT JOIN translation t2 ON (t2.trans_id = p.partsgroup_id AND t2.language_code = |
	  . $dbh->quote( $form->{language_code} ) . qq|)
	         $where|;
	my $sth = $dbh->prepare($query);
	$sth->execute || $form->dberror($query);
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		# get matrix
		&price_matrix( $pmh, $ref, $transdate, $decimalplaces, $form,
			$myconfig );
...
sub price_matrix_query {
	...
	my $query =
	  qq|SELECT p.id AS parts_id, 0 AS customer_id, 0 AS pricegroup_id,
...
sub price_matrix {
	...
	while ( $mref = $pmh->fetchrow_hashref(NAME_lc) ) {
		...
		if ( $mref->{validfrom} ) {
			next
			  if $transdate < $form->datetonum( $myconfig, $mref->{validfrom} );
		}
		if ( $mref->{validto} ) {
			next
			  if $transdate > $form->datetonum( $myconfig, $mref->{validto} );
		}

		# convert price
		$sellprice =
		  $form->round_amount( $mref->{sellprice} * $form->{ $mref->{curr} },
			$decimalplaces );
	...
	if ( $i > 1 ) {
		...
		$ref->{sellprice} =
		  $form->round_amount( $p{0} * ( 1 - $form->{tradediscount} ),
			$decimalplaces );
