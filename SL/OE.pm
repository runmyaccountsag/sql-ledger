...
", oi.description AS memo, oi.sellprice, oi.qty, oi.id AS orderitemsid";
		$orderitems_join = qq|JOIN orderitems oi ON (oi.trans_id = o.id)|
		  unless $orderitems_join;
	}
	...
	if ( $form->{transdatefrom} ) {
		$query .= " AND o.transdate >= '"
		  . $form->dbclean( $form->{transdatefrom} ) . "'";
	}
	if ( $form->{transdateto} ) {
		$query .= " AND o.transdate <= '"
		  . $form->dbclean( $form->{transdateto} ) . "'";
	}
	...
	if ( $form->{type} =~ /^consolidate_/ ) {
		...
		foreach $ref ( @{ $form->{OE} } ) {
			push @a, $ref
			  if $objid{vc}{ $ref->{curr} }{ $ref->{"$form->{vc}_id"} } > 1;
		}
...
sub save {
	...
	if ( $form->{id} *= 1 ) {
		...
		if ( $form->{id} ) {
			&adj_onhand( $dbh, $form, $sw )
			  if $form->{type} =~ /_order$/ && !$form->{aa_id};
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"qty_$i"} ) {
			...
			$form->{"projectnumber_$i"} =
			  $form->dbclean( $form->{"projectnumber_$i"} );
		  ...
		  reqdate = |
			  . $form->dbquote( $form->dbclean( $form->{"reqdate_$i"} ),
				SQL_DATE )
			  . qq|,
		  ...
		  AND trans_id = $form->{id}|;
			...
			if ( $form->{add_shipping} ) {
				$query =
qq|INSERT INTO inventory (parts_id, warehouse_id, department_id,
		  ...
		  |
				  . $form->dbclean( $form->{"orderitems_id_$i"} ) . qq|, |
		  ...
		  | . $form->dbclean( $form->{employee_id} ) . qq|)|;
		    ...
		    |
				  . $form->dbclean( $form->{"netweight_$i"} ) . qq|, |
				  . $form->dbclean( $form->{"grossweight_$i"} ) . qq|,
	...
	# add up the tax
	my $tax = 0;
	for ( keys %taxaccounts ) {
		$tax += $form->round_amount( $taxaccounts{$_}, $form->{precision} );
	}
	...
	$form->{$ordnumber} = $form->update_defaults( $myconfig, $numberfld, $dbh )
	...
	$query = qq|UPDATE oe set
	      ...
	      customer_id = | . $form->dbclean( $form->{customer_id} ) . qq|,
              ...
              netamount = $netamount,
	      reqdate = |
	  . $form->dbquote( $form->dbclean( $form->{reqdate} ), SQL_DATE ) . qq|,
...
sub delete {
	...
	if ( $dbh->selectrow_array($query) ) {
		...
		if ( $form->{type} =~ /_order$/ ) {
			...
			{
				$form->update_balance( $dbh, "parts", "onhand", qq|id = $id|,
					$ship * $ml )
				  if ( $inv || $assembly );
			}
		}
...
sub retrieve {
	...
	my %defaults =
	  $form->get_defaults( $dbh, \@{ [qw(weightunit closedto precision)] } );
	for ( keys %defaults ) { $form->{$_} = $defaults{$_} }
...
sub price_matrix_query {
	...
	if ( $form->{customer_id} ) {
		$query =
	     ...
	     |
		  . $dbh->quote( $form->{defaultcurrency} )
		  . qq| AS curr, '' AS pricegroup
	     FROM parts p
	     WHERE p.id = ?
...
sub order_details {
	...
	$query = qq|SELECT p.description, t.description
              FROM project p
	       LEFT JOIN translation t ON (t.trans_id = p.id AND t.language_code = |
	  . $dbh->quote( $form->{language_code} ) . qq|)
	       WHERE id = ?|;
	...
	foreach $item (@sortlist) {
		...
		if ( $form->{groupprojectnumber} || $form->{grouppartsgroup} ) {
			if ( $item->[1] ne $sameitem ) {
				...
				if ($ok) {
					...
					for (
						qw(taxrates runningnumber number sku qty ship unit bin serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode drawing toolnumber barcode)
					  )
					{
						push( @{ $form->{$_} }, "" );
					}
		...
		if ( $form->{"qty_$i"} ) {
			...
			for (
				qw(sku description unit bin serialnumber ordernumber customerponumber sellprice listprice package netweight grossweight volume countryorigin hscode drawing toolnumber barcode itemnotes lineitemdetail)
			  )
			{
			...
			if ($subtotal) {
				if ( $j < $k ) {
					...
					if ( $sortlist[$j]->[1] ne $sameitem ) {
						...
						for (
							qw(taxrates runningnumber number sku qty ship unit bin serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate itemnotes package netweight grossweight volume countryorigin hscode drawing toolnumber barcode)
						  )
						{
							push( @{ $form->{$_} }, "" );
						}
					...
					if ( $form->{groupsubtotaldescription} ne "" ) {
						...
						for (
							qw(taxrates runningnumber number sku qty ship unit bin serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode drawing toolnumber barcode)
						  )
						{
							push( @{ $form->{$_} }, "" );
						}
	...
	foreach $item ( sort keys %taxaccounts ) {
		if ( $form->round_amount( $taxaccounts{$item}, $form->{precision} ) ) {
			$tax += $taxamount =
			  $form->round_amount( $taxaccounts{$item}, $form->{precision} );

			push(
				@{ $form->{taxbaseinclusive} },
				$form->{"${item}_taxbaseinclusive"} = $form->round_amount(
					$taxbase{$item} + $taxamount,
					$form->{precision}
				)
			);
			push(
				@{ $form->{taxbase} },
				$form->{"${item}_taxbase"} = $form->format_amount(
					$myconfig, $taxbase{$item}, $form->{precision}
				)
			);
			push(
				@{ $form->{tax} },
				$form->{"${item}_tax"} = $form->format_amount(
					$myconfig, $taxamount, $form->{precision}
				)
			);
	...
	$form->{subtotal} =
	  $form->format_amount( $myconfig, $form->{subtotal}, $form->{precision},
		0 );
	...
	$form->{quototal} = $form->{ordtotal} =
	  $form->format_amount( $myconfig, $form->{ordtotal}, $form->{precision},
		0 );
...
sub assembly_details {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		@a =
		  qw(partnumber description partsgroup itemnotes drawing toolnumber barcode unit bin);
		...
		if ( $form->{grouppartsgroup} && $ref->{partsgroup} ne $sm ) {
			for (
				qw(taxrates number sku unit qty runningnumber ship bin serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode drawing toolnumber barcode)
			  )
			{
				push( @{ $form->{$_} }, "" );
			}
		...
		if ( $form->{stagger} ) {
			...
			for (qw(drawing toolnumber barcode itemnotes)) {
				push( @{ $form->{$_} }, qq|$spacer$form->{"a_$_"}| );
			}
			push(
				@{ $form->{description} },
				qq|$spacer$form->{"a_partnumber"}, $form->{"a_description"}|
			);
			for (
				qw(taxrates number sku runningnumber ship serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate linetotal lineitemdetail package netweight grossweight volume countryorigin hscode)
			  )
			{
				push( @{ $form->{$_} }, "" );
			}
			...
			for (qw(description sku drawing toolnumber barcode itemnotes)) {
				push( @{ $form->{$_} }, $form->{"a_$_"} );
			}

			for (
				qw(taxrates runningnumber ship serialnumber ordernumber customerponumber requiredate projectnumber sellprice listprice netprice discount discountrate linetotal lineitemdetail package netweight grossweight volume countryorigin hscode)
			  )
			{
				push( @{ $form->{$_} }, "" );
			}
		...
		if ( $ref->{assembly} && $form->{formname} eq 'work_order' ) {
			&assembly_details( $myconfig, $form, $dbh, $ref->{id},
				$ref->{qty} * $qty );
		}
...
sub save_inventory {
	...
	for $i ( 1 .. $form->{rowcount} - 1 ) {
		...
		if ($ship) {
		  ...
		  |
			  . $form->dbclean( $form->{"orderitems_id_$i"} ) . qq|, |
			  . $dbh->quote( $form->{shippingdate} ) . qq|,
		  |
			  . $form->dbclean($employee_id) . qq|,|
		  ...
		  AND id = | . $form->dbclean( $form->{"orderitems_id_$i"} ) . qq||;
		    ...
		    AND trans_id = $form->{id}|;
	            ...
	            grossweight, volume) VALUES (
		    |
				  . $form->dbclean( $form->{"orderitems_id_$i"} )
				  . qq|, $form->{id}, |
				  . $dbh->quote( $form->{"package_$i"} ) . qq|,
		    |
				  . $form->dbclean( $form->{"netweight_$i"} ) . qq|, |
			...
			$query = qq|UPDATE oe SET
                  shippingpoint = |
			  . $dbh->quote( $form->{shippingpoint} ) . qq|,
                  shipvia = | . $dbh->quote( $form->{shipvia} ) . qq|,
                  waybill = | . $dbh->quote( $form->{waybill} ) . qq|,
...
sub get_soparts {
	...
	for ( $i = 1 ; $i <= $form->{rowcount} ; $i++ ) {
		...
		if ( $form->{"ndx_$i"} ) {
			...
			while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
				&add_items_required( "", $dbh, $form, $id, $ref->{id},
					$ref->{required}, $ref->{assembly}, $ref->{orderitemsid} );
			}
			$sth->finish;
		}
...
sub add_items_required {
	my ( $self, $dbh, $form, $id, $parts_id, $required, $assembly,
		$orderitemsid )
	  = @_;
	...
	if ($assembly) {
		...
		while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
			for ( keys %$ref ) {
				$form->{orderitems}{$ph}{partsvendor}{ $ref->{vendor_id} }{$_} =
				  $ref->{$_};
			}
		}
		$sth->finish;
	...
	foreach $vendor_id ( keys %a ) {
		...
		foreach my $parts_id ( keys %{ $a{$vendor_id} } ) {
		  ...
		  WHERE pt.parts_id = $ph|;
			...
			if ( $form->{detail} ) {
				$query =
				  qq|SELECT oi.description, oi.itemnotes AS notes, p.unit,
	            oi.ordernumber
		    FROM parts p
			...
			if ($taxincluded) {
				...
				$amount += $form->round_amount( $linetotal * ( 1 + $rate ),
					$form->{precision} );
			}
...
sub consolidate_orders {
	...
	foreach $curr ( keys %{ $oe{orders} } ) {
		...
		foreach $vc_id ( sort { $a <=> $b } keys %{ $oe{orders}{$curr} } ) {
		  ...
		  netamount = $netamount,
		  reqdate = |
			  . $form->dbquote( $form->dbclean( $ref->{reqdate} ), SQL_DATE )
			  . qq|,
		  ...
		  WHERE id = $id|;
			...
			foreach $item (@orderitems) {
				for (qw(qty sellprice discount project_id ship)) {
					$item->{$_} *= 1;
				}
		    ...
		    $id, $item->{parts_id}, |
				  ...
				  . $form->dbquote( $form->dbclean( $item->{reqdate} ),
					SQL_DATE )
