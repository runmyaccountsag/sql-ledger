...
sub create_links {
	...
	if ( $form->{id} *= 1 ) {
		...
		( $form->{department_id}, $form->{department} ) = $dbh->selectrow_array(
			qq|
        SELECT id, description 
        FROM department 
        WHERE id IN (
            SELECT department_id FROM dpt_trans WHERE trans_id = |
			  . $form->dbclean( $form->{id} ) . qq|
        )
    |
		);
		...
		$query = qq|SELECT a.id
	      ...
	      JOIN $form->{db} ct ON (a.|
		  . $form->dbclean( $form->{db} )
		  . qq|_id = ct.id)
	      WHERE ct.id = $form->{id}
	    UNION
	      SELECT a.id
...
sub save {
	...
	for (
		qw(id terms discountterms taxincluded addressid contactid remittancevoucher)
	  )
	{
		$form->{$_} *= 1;
	}
	...
	for (qw(address1 address2 city state zipcode country)) {
		if ( $form->{"bank$_"} ) {
			if ($bank_address_id) {
				$query =
				  qq|INSERT INTO address (id, trans_id, address1, address2,
	...
	$form->{"$form->{db}number"} =
	  $form->update_defaults( $myconfig, "$form->{db}number", $dbh )
	...
	$query = qq|UPDATE $form->{db} SET
              $form->{db}number = |
	  . $dbh->quote( $form->{"$form->{db}number"} ) . qq|,
	  ...
	  . $form->dbquote( $form->dbclean( $form->{enddate} ), SQL_DATE ) . qq|,
	      arap_accno_id = (SELECT id FROM chart WHERE accno = |
	  . $dbh->quote( $rec{arap_accno} ) . qq|),
	      payment_accno_id = (SELECT id FROM chart WHERE accno = |
	  . $dbh->quote( $rec{payment_accno} ) . qq|),
	      discount_accno_id = (SELECT id FROM chart WHERE accno = |
	  . $dbh->quote( $rec{discount_accno} ) . qq|),
	...
	if ( $form->{department} ) {
		...
		$query =
qq|INSERT INTO dpt_trans (trans_id, department_id) VALUES ($form->{id}, $form->{department_id})|;
		$dbh->do($query) || $form->dberror($query);
	}
...
sub delete {
	...
	$query =
	  qq|DELETE FROM parts$form->{db} WHERE $form->{db}_id = $form->{id}|;
	$dbh->do($query) || $form->dberror($query);
...
sub search {
	...
	if ( $form->{address} ne "" ) {
		...
		$where .=
" AND (lower(ad.address1) LIKE '$var' OR lower(ad.address2) LIKE '$var')";
	}
	...
	if ( $form->{department} ) {
		...
		$department_id *= 1;
		$department_where =
qq|AND c.id IN (SELECT trans_id FROM dpt_trans WHERE department_id = $department_id)|;
	}

	my $query =
	  qq|SELECT c.*, b.description AS business, d.description AS dispatch,
		...
		|| $form->{l_quonumber} )
	{
		...
		if ( $form->{l_transnumber} ) {
			...
			$transwhere = "";
			$transwhere .= " AND a.transdate >= '$form->{transdatefrom}'"
			  if $form->{transdatefrom};
			$transwhere .= " AND a.transdate <= '$form->{transdateto}'"
			  if $form->{transdateto};

			$query =
			  qq|SELECT c.*, b.description AS business, d.description dispatch,
		...
		if ( $form->{l_ordnumber} ) {
			...
			$transwhere = "";
			$transwhere .= " AND o.transdate >= '$form->{transdatefrom}'"
			  if $form->{transdatefrom};
			$transwhere .= " AND o.transdate <= '$form->{transdateto}'"
			  if $form->{transdateto};
			$query .= qq|$union
		   SELECT c.*, b.description AS business, d.description AS dispatch,
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		for (qw(arap payment discount)) {
			$ref->{"${_}_accno"} = $accno{ $ref->{"${_}_accno_id"} };
		}
...
sub get_history {
	...
	$form->{direction} =~ s/;//g;
	$sortorder =
	  "2 $form->{direction}, 1, $ordinal{$sortorder} $form->{direction}";

	$query =
qq|SELECT ct.id AS ctid, ct.$form->{db}number, a.transdate, ct.name, ad.address1,
...
sub save_pricelist {
	...
	foreach $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"id_$i"} ) {
		    ...
		    |
				  . $form->dbclean( $form->{"pricebreak_$i"} ) . qq|, |
				  . $form->dbclean( $form->{"sellprice_$i"} ) . qq|,|
				  . $form->dbquote( $form->dbclean( $form->{"validfrom_$i"} ),
					SQL_DATE )
				  . qq|,|
				  . $form->dbquote( $form->dbclean( $form->{"validto_$i"} ),
					SQL_DATE )
				  . qq|,
