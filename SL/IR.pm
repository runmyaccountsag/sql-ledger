...
sub invoice_details {
	...
	$form->{terms} =
	  $form->datediff( $myconfig, $form->{duedate}, $form->{transdate} );
		...
		if ( $form->{groupprojectnumber} || $form->{grouppartsgroup} ) {
			if ( $item->[1] ne $sameitem ) {
				...
				if ($ok) {
					...
					for (
						qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes package netweight grossweight volume countryorigin hscode barcode)
					  )
					{
						push( @{ $form->{$_} }, "" );
					}
			...
			if ($subtotal) {
				if ( $j < $k ) {
					...
					if ( $sortlist[$j]->[1] ne $sameitem ) {
						...
						for (
							qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate itemnotes package netweight grossweight volume countryorigin hscode barcode)
						  )
						{
							push( @{ $form->{$_} }, "" );
						}
	...
	for ( sort keys %taxaccounts ) {
		...
		if ( $taxaccounts{$_} ) {
			...
			push(
				@{ $form->{taxbase} },
				$form->format_amount(
					$myconfig, $taxbase{$_}, $form->{precision}
				)
			);
			push(
				@{ $form->{tax} },
				$form->format_amount(
					$myconfig, $taxaccounts{$_}, $form->{precision}
				)
			);
			...
			$form->{"${_}_taxbaseinclusive"} =
			  $form->format_amount( $myconfig, $form->{"${_}_taxbaseinclusive"},
				$form->{precision} );
			$form->{"${_}_taxbase"} =
			  $form->format_amount( $myconfig, $taxbase{$_},
				$form->{precision} );
			$form->{"${_}_tax"} =
	...
	$form->format_string(
		qw(text_amount text_decimal text_cd_invtotal text_cd_decimal text_out_decimal text_out_amount)
	);
...
sub delete_invoice {
	...
	if ( $form->{id} and $defaults{extendedlog} ) {
		$query =
qq|INSERT INTO ap_log_deleted SELECT ap.* FROM ap WHERE id = $form->{id}|;
		...
		if ( $dbh->selectrow_array($query) ) {
		  ...
		  JOIN parts p ON (p.id = i.parts_id)
			...
			if ( $defaults{extendedlog} ) {
				$query =
qq|INSERT INTO ap_log SELECT ap.* FROM ap WHERE id = $form->{id}|;
				$dbh->do($query) || $form->dberror($query);
				$query = qq|
        ...
        |;
				$dbh->do($query) || $form->dberror($query);
				$query =
qq|UPDATE ap SET ts = NOW() + TIME '00:00:01' WHERE id = $form->{id}|;
				$dbh->do($query) || $form->dberror($query);
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"qty_$i"} ) {
			...
			for (@taxaccounts) {
				$applicabletaxes .= " $_"
				  if index( $form->{taxaccounts}, $_ ) != -1
					  or index( $form->{applicabletaxes}, $_ ) != 1;
			}
...
qq|INSERT INTO invoicetax (trans_id, invoice_id, chart_id, amount, taxamount)
			...
			if ( $form->{"inventory_accno_id_$i"} ) {
				...
				if ( $form->{"qty_$i"} > 0 ) {
					...
					$query =
qq|SELECT i.id, i.qty, i.allocated, i.trans_id, i.project_id,
		      ...
		      ORDER BY i.trans_id|;
					...
					while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
						...
						$linetotal =
						  $form->round_amount( $form->{"sellprice_$i"} * $qty,
							$form->{precision} );
						$ref->{project_id} ||= 'NULL';
...
qq|id = $form->{voucher}{payment}{$voucherid}{br_id}|,
	...
	$fxamount = $form->round_amount( $invamount / $form->{exchangerate},
		$form->{precision} );
	$fxamount *= 1;
...
sub reverse_invoice {
	...
	$query =
	  qq|SELECT i.parts_id, i.qty, i.allocated, i.sellprice, i.project_id,
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$netamount += $form->round_amount( $ref->{sellprice} * $ref->{qty} * -1,
			$form->{precision} );
	...
	for (
		qw(acc_trans dpt_trans invoice invoicetax inventory shipto cargo vr payment)
	  )
	{
...
sub retrieve_item {
	...
	my $vwhere = qq|$where
                  AND pv.vendor_id = |
	  . $form->dbclean( $form->{vendor_id} ) . qq||;
