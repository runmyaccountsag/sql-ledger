...
sub get_part {
	...
	$ref->{inventory_description} = $ref->{inventory_translation}
	  if $ref->{inventory_translation};
	$ref->{income_description} = $ref->{income_translation}
	  if $ref->{income_translation};
	$ref->{expense_description} = $ref->{expense_translation}
	  if $ref->{expense_translation};
...
sub save {
	...
	if ( $form->{id} ) {
		...
		# get old price
		$query =
		  qq|SELECT id, listprice, sellprice, lastcost, weight, project_id
                FROM parts
		WHERE id = $form->{id}|;
	...
	$query = qq|UPDATE parts SET
	      ...
	      weight = $form->{weight},
	      priceupdate = |
	  . $form->dbquote( $form->dbclean( $form->{priceupdate} ), SQL_DATE )
	  . qq|,
		...
		if ( $form->{orphaned} ) {
			for $i ( 1 .. $form->{assembly_rows} ) {
				...
				if ( $form->{"qty_$i"} ) {
					for (qw(bom adj)) { $form->{"${_}_$i"} *= 1 }
					$query =
					  qq|INSERT INTO assembly (aid, parts_id, qty, bom, adj)
	...
	if ( $form->{item} ne 'assembly' ) {
		...
		for $i ( 1 .. $form->{vendor_rows} ) {
			if ( ( $form->{"vendor_$i"} ne "" ) && $form->{"lastcost_$i"} ) {
				...
				$query =
				  qq|INSERT INTO partsvendor (vendor_id, parts_id, partnumber,
	            lastcost, leadtime, curr)
		    VALUES ($vendor_id, $form->{id},|
				  . $dbh->quote( $form->{"partnumber_$i"} ) . qq|,
		    $form->{"lastcost_$i"},
		    $form->{"leadtime_$i"}, |
				  . $dbh->quote( $form->{"vendorcurr_$i"} ) . qq|)|;
				$dbh->do($query) || $form->dberror($query);
			}
		}
	}
	...
	for $i ( 1 .. $form->{customer_rows} ) {
		...
		if ( $form->{"customerprice_$i"} ) {
		  ...
		  $form->{"customerprice_$i"}, |
			  . $dbh->quote( $form->{"customercurr_$i"} ) . qq|,|
...
sub restock_assemblies {
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"qty_$i"} ) {
			&adjust_inventory( $dbh, $form, $form->{"id_$i"} * 1,
				$form->{"qty_$i"} );
		}
...
sub all_parts {
	...
	if ( $form->{searchitems} eq 'component' ) {
		$assemblyflds =
		  qq|, p1.partnumber AS assemblypartnumber, a.aid AS assembly_id|;
	}
		...
		|| $form->{quoted} )
	{
		...
		if ( $form->{bought} || $form->{sold} ) {
			...
			my $invwhere = "$where";
			my $transdate =
			  ( $form->{method} eq 'accrual' ) ? "transdate" : "datepaid";
		...
		if ( $form->{onorder} || $form->{ordered} ) {
			...
			$ordwhere .=
			  " AND a.transdate <= " . $dbh->quote( $form->{transdateto} ) . ""
			  if $form->{transdateto};
			$ordwhere .= " AND a.warehouse_id = $form->{warehouse_id}"
			  if $form->{warehouse};
	...
	if ( ( $form->{searchitems} =~ /assembly/ ) && $form->{individual} ) {
		...
		if ( $form->{sold} || $form->{ordered} || $form->{quoted} ) {
			$flds =
qq|p.id, p.partnumber, p.description, p.onhand AS perassembly, p.unit,
                 p.bin, p.sellprice, p.listprice, p.lastcost, p.rop,
		 p.avgcost,
 		 p.weight, p.priceupdate, p.image, p.drawing, p.microfiche,
		...
		for ( @{ $form->{parts} } ) {
			...
			if ( $form->{warehouse} ne "" ) {
				...
				if ( $ref->{onhand} != 0 ) {
					$_->{onhand} = $ref->{onhand}
					  if $_
						  ->{inventory}; # Don't show any onhand for non-inventory items.
					push @a, $_;
				}
...
sub create_links {
	...
	if ( $form->{id} *= 1 ) {
		my %defaults =
		  $form->get_defaults( $dbh, \@{ [qw(weightunit precision)] } );
		for ( keys %defaults ) { $form->{$_} = $defaults{$_} }
		...
		my %defaults =
		  $form->get_defaults( $dbh,
			\@{ [ 'weightunit', '%_accno_id', 'precision' ] } );
		...
		for (qw(inventory income expense)) {
		  ...
		  WHERE c.id = '$defaults{"${_}_accno_id"}'|;
			...
			$form->{"${_}_description"} = $form->{"${_}_translation"}
			  if $form->{"${_}_translation"};
			$form->{amount}{"IC_$_"} = {
				accno       => $form->{"${_}_accno"},
				description => $form->{"${_}_description"}
			};
...
sub so_requirements {
	...
	( $form->{reqdatefrom}, $form->{reqdateto} ) =
	  $form->from_to( $form->{year}, $form->{month}, $form->{interval} )
	  if $form->{year} && $form->{month};
	$where .= " AND o.reqdate >= " . $dbh->quote( $form->{reqdatefrom} ) . ""
	  if $form->{reqdatefrom};
	$where .= " AND o.reqdate <= " . $dbh->quote( $form->{reqdateto} ) . ""
	  if $form->{reqdateto};
