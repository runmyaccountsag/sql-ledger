...
sub income_statement {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [ 'company', 'address', 'businessnumber' ] } );
	...
	my $tmpstr;
	my $translink =
	  qq|ca.pl?action=list_transactions&fx_transaction=1&l_name=1|;
	for (qw(path login fromdate todate method)) {
		$translink .= qq|&$_=$form->{$_}|;
	}
	...
	foreach $category (@categories) {
		...
		foreach $key ( sort keys %{ $form->{$category} } ) {
			...
			if ( $form->{$category}{$key}{charttype} eq "A" ) {
				...
				$form->{tmpstr} =
qq|<a href=$translink&accno=$form->{$category}{$key}{accno} target=_blank>|
				  . $form->{tmpstr}
				  . qq|</a>|
...
sub get_accounts {
	...
	if ($fromdate) {
		if ( $form->{method} eq 'cash' ) {
			$subwhere .=
			  " AND ac.transdate >= '" . $form->dbclean($fromdate) . "'";
			$subwhere2 .=
			  " AND datepaid >= '" . $form->dbclean($fromdate) . "'";
			$glwhere =
			  " AND ac.transdate >= '" . $form->dbclean($fromdate) . "'";
		}
		else {
...
|;
	...
	if ( $form->{fromdate} || $form->{todate} ) {
		if ( $form->{fromdate} ) {
			$where .= " AND ac.transdate >= '"
			  . $form->dbclean( $form->{fromdate} ) . "'";
			$invwhere .= " AND a.transdate >= '"
			  . $form->dbclean( $form->{fromdate} ) . "'";
		}
		if ( $form->{todate} ) {
			$where .= " AND ac.transdate <= '"
			  . $form->dbclean( $form->{todate} ) . "'";
			$invwhere .=
			  " AND a.transdate <= '" . $form->dbclean( $form->{todate} ) . "'";
		}
	}
	...
	foreach my $accno ( sort keys %trb ) {
		...
		if ( $trb{$accno}{charttype} eq 'A' ) {
			...
			$ref->{debit} =
			  $form->round_amount( $ref->{debit}, $form->{precision} );
			$ref->{credit} =
			  $form->round_amount( $ref->{credit}, $form->{precision} );

			if ( !$form->{all_accounts} ) {
				next
				  if $form->round_amount( $ref->{debit} + $ref->{credit},
						  $form->{precision} ) == 0;
			}
		}
...
sub aging {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [qw(company address businessnumber tel fax precision)] } );
	for ( keys %defaults ) { $form->{$_} = $defaults{$_} }
	...
	if ( $form->{department} ) {
		( $null, $department_id ) = split /--/, $form->{department};
		$where .=
		  qq| AND a.department_id = | . $form->dbclean($department_id) . qq||;
	}
	...
	if ( $form->{payed} ) {
		$paidAmountCheck =
qq|AND a.amount <> 0 AND (a.paid != a.amount or (a.datepaid is null or a.datepaid > |
		  . $dbh->quote( $form->{todate} ) . qq|))|;
	}
	...
	my %c = (
		c0 => {
			flds =>
'(a.amount - a.paid) AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90'
		},
		c15 => {
			flds =>
'0.00 AS c0, (a.amount - a.paid) AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90'
		},
		c30 => {
			flds =>
'0.00 AS c0, 0.00 AS c15, (a.amount - a.paid) AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90'
		},
		c45 => {
			flds =>
'0.00 AS c0, 0.00 AS c15, 0.00 AS c30, (a.amount - a.paid) AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90'
		},
		c60 => {
			flds =>
'0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, (a.amount - a.paid) AS c60, 0.00 AS c75, 0.00 AS c90'
		},
		c75 => {
			flds =>
'0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, (a.amount - a.paid) AS c75, 0.00 AS c90'
		},
		c90 => {
			flds =>
'0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, (a.amount - a.paid) AS c90'
		}
	);
...
qq| round(a.amount::numeric, 2) - round((coalesce((SELECT SUM(ac.amount) FROM acc_trans ac JOIN chart ch ON (ch.id = ac.chart_id) WHERE ac.trans_id = a.id and ac.transdate <= |
		...
		%c = (
			c0 => {
				flds => $openAmountQuery
				  . qq| AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90|
			},
			c15 => {
				    flds => qq| 0.00 AS c0, |
				  . $openAmountQuery
				  . qq| AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90|
			},
			c30 => {
				    flds => qq| 0.00 AS c0, 0.00 AS c15, |
				  . $openAmountQuery
				  . qq| AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90|
			},
			c45 => {
				    flds => qq| 0.00 AS c0, 0.00 AS c15, 0.00 AS c30, |
				  . $openAmountQuery
				  . qq| AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90|
			},
			c60 => {
				flds => qq| 0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, |
				  . $openAmountQuery
				  . qq| AS c60, 0.00 AS c75, 0.00 AS c90|
			},
			c75 => {
				flds =>
qq| 0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, |
				  . $openAmountQuery
				  . qq| AS c75, 0.00 AS c90|
			},
			c90 => {
				flds =>
qq| 0.00 AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, |
				  . $openAmountQuery
				  . qq| AS c90|
			}
		);
	...
	$item = $#c;
	$c{ $c[$item] }{and} =
	  qq|AND a.$transdate < $interval{$myconfig->{dbdriver}}{$c[$item]}|;
...
sub reminder {
	...
	my @a =
	  qw(company companyemail companywebsite address businessnumber tel fax precision);
	...
	if ( $form->{overpaid} ne "on" ) {
		$where .=
qq| AND ((a.amount > 0 AND a.paid < a.amount) OR (a.amount < 0 AND a.paid > a.amount))|;
	}
...
sub save_level {
	...
	for ( split / /, $form->{ids} ) {
		if ( $form->{"ndx_$_"} ) {
			...
			if ( $form->{"level_$_"} *= 1 ) {
				$ath->execute( $_, qq|reminder$form->{"level_$_"}| )
				  || $form->dberror;
				$ath->finish;
...
sub get_customer {
	...
	( $form->{ $form->{vc} }, $form->{email}, $form->{cc}, $form->{bcc} ) =
...
sub tax_report {
	...
	if ( $form->{fromdate} || $form->{todate} ) {
		...
		if ( $form->{todate} ) {
			$where .=
			  " AND $transdate <= '" . $form->dbclean( $form->{todate} ) . "'";
		}
	}
	...
	if ( $form->{report} =~ /nontaxable/ ) {
		...
		if ( $form->{summary} ) {
			...
			$query =
			  qq|SELECT DISTINCT a.id, a.invoice, $transdate AS transdate,
		  a.invnumber, n.name, n.${vc}number, a.netamount,
		  a.description,
		  a.till, n.id AS vc_id
		  FROM acc_trans ac
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		if ( $form->{report} =~ /nontaxable/ ) {
			...
			push @{ $form->{TR} },
			  $ref;    # Removed if $ref->{tax} to show 0 taxes
		}
	}
...
sub payments {
	...
	if ( $form->{fromdate} ) {
		$where .=
		  " AND ac.transdate >= '" . $form->dbclean( $form->{fromdate} ) . "'";
	}
	if ( $form->{todate} ) {
		$where .=
		  " AND ac.transdate <= '" . $form->dbclean( $form->{todate} ) . "'";
	}
	if ( !$form->{fx_transaction} ) {
