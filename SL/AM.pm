...
sub save_account {
	...
	foreach my $item (
		...
		$form->{AR_paid},     $form->{AR_discount}, $form->{AP},
		$form->{AP_amount},   $form->{AP_tax},      $form->{AP_paid},
		$form->{AP_discount}, $form->{IC},          $form->{IC_income},
		$form->{IC_sale},     $form->{IC_expense},  $form->{IC_cogs},
		$form->{IC_taxpart},  $form->{IC_taxservice},
	  )
	{
	...
	if ( $form->{id} *= 1 ) {
		...
		|
		  . $dbh->quote( $form->{category} ) . qq|, |
		  . $dbh->quote( $form->{link} ) . qq|, |
		  . $dbh->quote( $form->{contra} ) . qq|, |
		  . $dbh->quote( $form->{allow_gl} ) . qq|)|;
	}
...
sub update_recurring {
	...
	my %advance = (
		...
		'Sybase' =>
qq|dateadd($myconfig->{dateformat}, $repeat $unit, $form->{nextdate})|,
...
sub save_defaults {
	...
	for (
		qw(transitionaccount selectedaccount glnumber sinumber vinumber batchnumber vouchernumber sonumber ponumber sqnumber rfqnumber partnumber employeenumber customernumber vendornumber projectnumber precision)
	  )
	{
...
sub backup {
	...
	if ( $form->{media} eq 'email' ) {
		print OUT
qx(PGPASSWORD="$myconfig->{dbpasswd}" /usr/bin/pg_dump -U $myconfig->{dbuser} $myconfig->{dbname} | gzip -c);
		close OUT;
		...
		$mail->{subject} =
"Run my Accounts Backup / $myconfig->{dbname}-$form->{version}-$t[5]$t[4]$t[3].sql.gz";
...
sub backup_templates {
	...
	if ( $form->{media} eq 'email' ) {
		...
		$mail->{from}    = qq|"$myconfig->{name}" <$myconfig->{email}>|;
		$mail->{subject} =
"Run my Accounts Templates Backup / $myconfig->{dbname}-$form->{version}-$t[5]$t[4]$t[3]-templates.tar.gz";
		@{ $mail->{attachments} } = ($tmpfile);
		$mail->{version} = $form->{version};
		$mail->{fileid}  = "$boundary.";
...
sub closedto {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [qw(closedto revtrans audittrail extendedlog)] } );
	for ( keys %defaults ) { $form->{$_} = $defaults{$_} }
...
sub get_bank {
	...
	$ref->{account} .=
	  ( $ref->{translation} ) ? $ref->{translation} : $ref->{description};
	for ( keys %$ref ) { $form->{$_} = $ref->{$_} }
	$sth->finish;
...
sub save_bank {
	...
	my $ok;
	for (
		qw(name iban bic address1 address2 city state zipcode country membernumber rvc dcn)
	  )
	{
		if ( $form->{$_} ) {
			$ok = 1;
	...
	if ($ok) {
		if ($id) {
		  ...
		  WHERE id = $form->{id}|;
			...
			$query =
			  qq|INSERT INTO bank (id, name, iban, bic, membernumber, rvc, dcn)
		  VALUES ($form->{id}, |
...
sub save_exchangerate {
	...
	for ( split /:/, $form->{currencies} ) {
		...
		if ( $form->{$_} ) {
			...
			$form->{"${_}buy"} =
			  $form->parse_amount( $myconfig, $form->{"${_}buy"} );
			$form->{"${_}sell"} =
			  $form->parse_amount( $myconfig, $form->{"${_}sell"} );

			if ( $form->{"${_}buy"} || $form->{"${_}sell"} ) {
				$sth->execute( $form->{transdate}, $form->{"${_}buy"},
					$form->{"${_}sell"}, $_ )
				  || $form->dberror;
				$sth->finish;
			}
...
sub move {
	...
	$form->error('Invalid table name...')
	  if !grep( /^$form->{db}$/, @dballowed );
	$form->error('Invalid column name...')
	  if !grep( /^$form->{fld}$/, @fldallowed );
