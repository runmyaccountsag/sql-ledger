...
sub get_spoolfiles {
	...
	if ( $form->{type} eq 'timecard' ) {
		...
		$query .= " AND j.checkedin >= '$form->{transdatefrom}'"
		  if $form->{transdatefrom};
		$query .= " AND j.checkedin < date '$form->{transdateto}' + 1"
		  if $form->{transdateto};
		...
		foreach $item ( keys %{ $arap{ $form->{type} } } ) {
			(
				$form->{ $arap{ $form->{type} }{$item} },
				$form->{"$arap{$form->{type}}{$item}_id"}
			) = split /--/, $form->{ $arap{ $form->{type} }{$item} };
		}
						...
						for (qw(printed emailed)) {
					      ...
					      AND s.formname LIKE '$wildcard$form->{type}')|;
							...
							if ( $form->{"not$_"} ) {
								if ( !$form->{$_} ) {
									$where .=
									  qq| AND NOT a.id IN (SELECT s.trans_id
					      FROM status s
					      WHERE s.trans_id = a.id
					      AND s.$_ = '1'
					      AND s.formname LIKE '$wildcard$form->{type}')|;
		  ...
		  WHERE $where|;
			...
			if ( $form->{ $arap{ $form->{type} }{$item} } ) {
				if ( $form->{"$arap{$form->{type}}{$item}_id"} ) {
					$query .=
qq| AND a.$arap{$form->{type}}{$item}_id = $form->{"$arap{$form->{type}}{$item}_id"}|;
				}
				else {
					$form->{ $arap{ $form->{type} }{$item} } =~
					  s/^\s+|\s+$//g;    # remove leading/trailing white space
					$var = $form->like(
						lc $form->{ $arap{ $form->{type} }{$item} } );
			...
			if ( $form->{"$arap{$form->{type}}{$item}number"} ) {
				$var = $form->like(
					lc $form->{"$arap{$form->{type}}{$item}number"} );
				$query .=
" AND lower(vc.$arap{$form->{type}}{$item}number) LIKE '$var'";
			}
			...
			$query .= " AND a.transdate >= '$form->{transdatefrom}'"
			  if $form->{transdatefrom};
			$query .= " AND a.transdate <= '$form->{transdateto}'"
			  if $form->{transdateto};
