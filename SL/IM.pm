...
sub sales_invoice {
	...
	for (@d) {
		...
		if (@a) {
			...
			if ( $customernumber ne
				   $a[ $form->{ $form->{type} }->{customernumber}{ndx} ]
				|| $invnumber ne
				$a[ $form->{ $form->{type} }->{invnumber}{ndx} ] )
			{
				...
				if ( $a[ $form->{ $form->{type} }->{employeenumber}{ndx} ] ) {
					$eth->execute(
						"$a[$form->{$form->{type}}->{employeenumber}{ndx}]");
				...
				if (
					!$ARAP{"$a[$form->{$form->{type}}->{$form->{ARAP}}{ndx}]"} )
				{
				...
				if ( $form->{"duedate_$i"} ) {
					$form->{"terms_$i"} =
					  $form->datediff( $myconfig, $form->{"transdate_$i"},
						$form->{"duedate_$i"} );
				}
				else {
					...
					if ( $form->{"terms_$i"} > 0 ) {
						$form->{"duedate_$i"} =
						  $form->add_date( $myconfig, $form->{"transdate_$i"},
							$form->{"terms_$i"}, 'days' );
					}
				}
			...
			while ( $ref = $pth->fetchrow_hashref(NAME_lc) ) {
				...
				if ( $customertax{ $ref->{accno} } ) {
					$form->{"tax_$j"} +=
					  $a[ $form->{ $form->{type} }->{sellprice}{ndx} ] *
					  $a[ $form->{ $form->{type} }->{qty}{ndx} ] *
					  $tax{ $ref->{accno} };
				}
			}
			$pth->finish;
			...
			$form->{"projectnumber_$i"} = qq|--$form->{"projectnumber_$i"}|
			  if $form->{"projectnumber_$i"};

			if ( !$parts_id ) {
				$form->{"customer_id_$j"} = 0;
				$form->{missingparts} .=
"$a[$form->{$form->{type}}->{invnumber}{ndx}] : $a[$form->{$form->{type}}->{partnumber}{ndx}]\n";
			}
...
sub import_sales_invoice {
	...
	( $form->{customernumber}, $language_code, $form->{city} ) =
	...
	for (@d) {
		...
		if (@a) {
			...
			if ( $customernumber ne
				   $a[ $form->{ $form->{type} }->{customernumber}{ndx} ]
				|| $ordnumber ne
				$a[ $form->{ $form->{type} }->{ordnumber}{ndx} ] )
			{
		...
		AND NOT quotation AND vendor_id = 0" );
			...
			if ( !$parts_id ) {
				$form->{"customer_id_$j"} = 0;
				$form->{missingparts} .=
"$a[$form->{$form->{type}}->{ordnumber}{ndx}] : $a[$form->{$form->{type}}->{partnumber}{ndx}]\n";
			}
...
sub purchase_order {
	...
	for (@d) {
		...
		if (@a) {
			...
			if ( $vendornumber ne
				   $a[ $form->{ $form->{type} }->{vendornumber}{ndx} ]
				|| $ordnumber ne
				$a[ $form->{ $form->{type} }->{ordnumber}{ndx} ] )
			{
...
sub import_purchase_order {
	...
	( $form->{vendornumber}, $language_code, $form->{city} ) =
	  $dbh->selectrow_array($query);
...
sub payments {
	...
	for (@d) {
		...
		if (@a) {
			...
			$amount = $form->format_amount(
				$myconfig,
				$a[ $form->{ $form->{type} }->{credit}{ndx} ] -
				  $a[ $form->{ $form->{type} }->{debit}{ndx} ],
				$form->{precision}
			);
			$am = 1;
			...
			if ( exists $form->{ $form->{type} }->{dcn} ) {
				...
				if ( $a[ $form->{ $form->{type} }->{dcn}{ndx} ] ) {
					...
					if ( $ref->{invnumber} ) {
						...
						$vc = $ref->{vc};
						for (
							qw(id invnumber description name companynumber vc arap city paymentmethod_id)
						  )
						{
							$form->{"${_}_$i"} = $ref->{$_};
						}
			...
			if ( $form->{currency} ne $form->{defaultcurrency} ) {
				$eth->execute(
					$a[ $form->{ $form->{type} }->{datepaid}{ndx} ] );
				( $buy, $sell ) = $eth->fetchrow_array;
				$eth->finish;
				( $form->{"exchangerate_$i"} ) =
				  ( $vc eq 'customer' ) ? $buy : $sell;
			}
...
sub partscustomer {
	...
	my $pquery =
	  qq|SELECT id, description FROM parts WHERE LOWER(partnumber) = ?|;
	my $psth = $dbh->prepare($pquery) || $form->dberror($pquery);

	my $cquery =
	  qq|SELECT id, name FROM customer WHERE LOWER(customernumber) = ?|;
	...
	for (@d) {
		...
		if (@a) {
			...
			$psth->execute(
				lc "$a[$form->{$form->{type}}->{partnumber}{ndx}]" );
			...
			$psth->finish;
			$csth->execute(
				lc "$a[$form->{$form->{type}}->{customernumber}{ndx}]" );
			...
			$csth->finish;
			$pgsth->execute(
				lc "$a[$form->{$form->{type}}->{pricegroup}{ndx}]" );
...
sub partsvendor {
	...
	for (@d) {
		...
		if (@a) {
			...
			$psth->finish;
			$vsth->execute(
				lc "$a[$form->{$form->{type}}->{vendornumber}{ndx}]" );
...
sub parts {
	...
	for (@d) {
		...
		if (@a) {
			...
			$gsth->execute(
				lc "$a[$form->{$form->{type}}->{partsgroup}{ndx}]" );
