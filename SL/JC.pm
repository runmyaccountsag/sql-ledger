...
sub retrieve_card {
	...
	if ( $form->{id} *= 1 ) {
		...
		$query =
		  qq|SELECT j.*, to_char(j.checkedin, 'HH24:MI:SS') AS checkedina,
		...
		if ( $form->{customer_id} ) {
			my $pmh =
			  price_matrix_query( $dbh, $form, $form->{project_id},
				$form->{customer_id} );
			%ref = ();
			$ref->{id} = $form->{parts_id};

			IS::exchangerate_defaults( $dbh, $myconfig, $form );
			IS::price_matrix( $pmh, $ref,
...
sub jcitems {
	...
	$where .= " AND j.checkedin >= '$form->{startdatefrom}'"
	  if $form->{startdatefrom};
	$where .= " AND j.checkedout < date '$form->{startdateto}' + 1"
	  if $form->{startdateto};
	...
	$sortorder = qq|employee, employeenumber, $sortorder|
	  if $form->{type} eq 'timecard';
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		if ( $ref->{parts_id} ) {
			$ref->{project} = 'job';
			$ref->{type} =
			  ( $ref->{inventory_accno_id} && $ref->{income_accno_id} )
			  ? 'storescard'
			  : 'timecard';
		}
		else {
...
sub company_defaults {
	...
	my %defaults =
	  $form->get_defaults( $dbh,
		\@{ [ 'company', 'address', 'tel', 'fax', 'businessnumber' ] } );
