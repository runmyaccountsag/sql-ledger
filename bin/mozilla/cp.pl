...
sub edit {
	...
	$form->error( $locale->text('Payment account missing!') )
	  unless $form->{"select$form->{ARAP}_paid"};

	# $locale->text('AR account missing!')
	# $locale->text('AP account missing!')
	$form->error( $locale->text("$form->{ARAP} account missing!") )
	  unless $form->{"select$form->{ARAP}"};
	...
	if ( @{ $form->{transactions} } ) {
		...
		foreach $ref ( @{ $form->{transactions} } ) {
			...
			for (
				qw(id invnumber invdescription transdate duedate calcdiscount discountterms cashdiscount)
			  )
			{
				$form->{"${_}_$i"} = $ref->{$_};
			}
			$ref->{exchangerate} ||= 1;
			$form->{"netamount_$i"} =
			  $form->round_amount( $ref->{netamount} / $ref->{exchangerate},
				$form->{precision} );
	...
	if ( !$form->{readonly} ) {
		if ( $form->{batch} ) {
			$form->{readonly} = 1
			  if $myconfig{acs} =~ /Vouchers--Payment Batch/
				  || $form->{approved};
		}
	}
...
"$form->{script}?action=payment&path=$form->{path}&login=$form->{login}&all_vc=$form->{all_vc}&type=$form->{type}"
	...
	foreach $item (qw(business paymentmethod)) {
		if ( @{ $form->{"all_$item"} } ) {
			$form->{"select$item"} = "\n";
			$form->{$item} = qq|$form->{$item}--$form->{"${item}_id"}|
			  if $form->{$item};

			for ( @{ $form->{"all_$item"} } ) {
				$form->{"select$item"} .= qq|$_->{description}--$_->{id}\n|;
			}
		}
	}
	...
	if ( @{ $form->{all_department} } ) {
		if ( $myconfig{department_id} and $myconfig{role} eq 'user' ) {
			...
			$form->{selectdepartment} = "\n";
			$form->{department} =
			  qq|$form->{department}--$form->{department_id}|
			  if $form->{department};
	...
	for (qw(currency department business language account paymentmethod)) {
		$form->{"select$_"} = $form->escape( $form->{"select$_"}, 1 );
	}
...
sub prepare_payments_header {
	...
	for $i ( 1 .. $form->{rowcount} ) {
		if ( $form->{"detail_$i"} ) {
			...
			for (qw(datepaid duedatefrom duedateto)) {
				$form->{"old$_"} = $form->{$_};
			}
			last;
		}
	...
	foreach $ref ( @{ $form->{PR} } ) {
		...
		for (
			qw(id invnumber invdescription transdate duedate calcdiscount discountterms cashdiscount netamount)
		  )
		{
			$form->{"${_}_$i"} = $ref->{$_};
		}
		$ref->{exchangerate} ||= 1;
		$due =
		  ( $form->{edit} ) ? $ref->{amount} : $ref->{amount} - $ref->{paid};
		$due =
		  $form->round_amount( $due / $ref->{exchangerate},
			$form->{precision} );
		$netamount =
		  $form->round_amount( $ref->{netamount} / $ref->{exchangerate},
			$form->{precision} );
		...
		if ( $ref->{calcdiscount} ) {
			$discount = $form->round_amount( $netamount * $ref->{cashdiscount},
				$form->{precision} );
		}
		...
		$form->{"due_$i"} =
		  $form->format_amount( \%myconfig, $due, $form->{precision} );
		$form->{"discount_$i"} =
		  $form->format_amount( \%myconfig, $discount, $form->{precision} );
		$form->{"amount_$i"} =
		  $form->format_amount( \%myconfig,
			$ref->{amount} / $ref->{exchangerate},
			$form->{precision} );
		$form->{"netamount_$i"} =
		  $form->format_amount( \%myconfig, $netamount, $form->{precision} );

		$form->{"olddiscount_$i"} = $form->{"discount_$i"};

		$form->{"checked_$i"} = 1;
		$form->{"paid_$i"} = $form->format_amount( \%myconfig, $due - $discount,
			$form->{precision} );
		$form->{"total_$i"} =
		  $form->format_amount( \%myconfig, $due, $form->{precision} );
	}
...
sub payments {
	...
	$form->{callback} =
"$form->{script}?action=payments&path=$form->{path}&login=$form->{login}&type=$form->{type}"
	  unless $form->{callback};
...
|;
	...
	$form->hide_form( map { "old$_" }
		  qw(currency datepaid duedatefrom duedateto department business paymentmethod)
	);
	$form->hide_form( map { "old$_" }
		  ( "$form->{ARAP}", "$form->{vc}", "$form->{vc}number" ) );
	$form->hide_form( map { "select$_" }
		  qw(currency department business language account paymentmethod) );
	$form->hide_form( map { "select$_" }
		  ( "$form->{ARAP}", "$form->{ARAP}_paid", "$form->{ARAP}_discount" ) );
...
<table width=100%>
  ...
  <tr>
    <td>
	  ...
	  . $form->select_option( $form->{"select$form->{ARAP}_paid"},
		$form->{"$form->{ARAP}_paid"} )
	  . qq|</select>
		</td>
	      </tr>
	      $paymentmethod
...
|;
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		$column_data{paid} =
qq|<td align=right><input name="paid_$i" size=11 value=$form->{"paid_$i"}></td>|;
...
qq|<td align=center><input name="detail_$i" type=checkbox class=checkbox $form->{"detail_$i"}></td>|;
		...
		$column_data{"$form->{vc}number"} =
		  qq|<td>$form->{"$form->{vc}number_$i"}</td>|;
		$column_data{name} = qq|<td>$form->{"name_$i"}</td>|;

		$column_data{memo} =
		    qq|<td align=right><input name="memo_$i" size=20 value="|
		  . $form->quote( $form->{"memo_$i"} )
		  . qq|"></td>|;
		$column_data{source} =
		    qq|<td align=right><input name="source_$i" size=10 value="|
		  . $form->quote( $form->{"source_$i"} )
		  . qq|"></td>|;

		if ( $form->{selectlanguage} ) {
			$column_data{language} = qq|<td><select name="language_code_$i">|
			  . $form->select_option(
				$form->{selectlanguage},
...
|;
	...
	$column_data{amount} = qq|<th class=listtotal align=right>|
	  . $form->format_amount( \%myconfig, $totalamount, $form->{precision},
		"&nbsp;" )
	  . qq|</th>|;
	$column_data{due} = qq|<th class=listtotal align=right>|
	  . $form->format_amount( \%myconfig, $totaldue, $form->{precision},
		"&nbsp;" )
	  . qq|</th>|;
	$column_data{paid} = qq|<th class=listtotal align=right>|
...
|;
	...
	%button = (
		...
		'Select all' =>
		  { ndx => 3, key => 'A', value => $locale->text('Select all') },
		'Deselect all' =>
		  { ndx => 4, key => 'A', value => $locale->text('Deselect all') },
...
sub select_all {
	...
	for ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"calcdiscount_$_"} ) {
			$form->{"discount_$_"} =
			  $form->parse_amount( \%myconfig, $form->{"netamount_$_"} ) *
			  $form->{"cashdiscount_$_"};
		}

		$form->{"paid_$_"} =
		  $form->format_amount( \%myconfig, $due - $form->{"discount_$_"},
			$form->{precision} );
	}
...
sub update {
	...
	$form->isvaldate( \%myconfig, $form->{datepaid},
...
sub update_payments {
	...
	$form->{exchangerate} =
	  $form->check_exchangerate( \%myconfig, $form->{currency},
		$form->{datepaid}, $buysell );
	for (
		"datepaid", "duedatefrom", "duedateto",     "department",
		"business", "currency",    "$form->{ARAP}", "paymentmethod"
	  )
	{
	...
	if ( $form->{redo} ) {
		...
		foreach $ref ( @{ $form->{PR} } ) {
			...
			$amount =
			  $form->round_amount( $ref->{amount} / $ref->{exchangerate},
				$form->{precision} );
			$paid = $form->round_amount( $ref->{paid} / $ref->{exchangerate},
				$form->{precision} );
			...
			for (qw(_id number)) {
				$form->{"$form->{vc}${_}_$i"} = $ref->{"$form->{vc}$_"};
			}
			$form->{"checked_$i"} = 1;
...
sub update_payment {
	...
	for (
		"duedatefrom",   "duedateto", "department", "business",
		"$form->{ARAP}", "currency",  "paymentmethod"
	  )
	{
		if ( $form->{$_} ne $form->{"old$_"} ) {
			$form->{redo} = 1;
...
qq|$form->{"all_$form->{vc}"}[0]->{name}--$form->{"all_$form->{vc}"}[0]->{id}|;
		...
		if ( @{ $form->{all_language} } ) {
			...
			$form->{selectlanguage} =
			  $form->escape( $form->{selectlanguage}, 1 );
		}
	...
	if ( $form->{redo} ) {
		...
		foreach $ref ( @{ $form->{PR} } ) {
			...
			$due =
			  ( $form->{edit} )
			  ? $ref->{fxamount}
			  : $ref->{fxamount} - $ref->{fxpaid};

			$form->{"due_$i"} =
			  $form->format_amount( \%myconfig, $due, $form->{precision} );
			$form->{"netamount_$i"} =
			  $form->format_amount( \%myconfig,
				$ref->{netamount} / $ref->{exchangerate},
				$form->{precision} );

			$form->{"amount_$i"} =
			  $form->format_amount( \%myconfig, $ref->{fxamount},
				$form->{precision} );
			$form->{"paid_$i"} =
			  $form->format_amount( \%myconfig, $ref->{fxpaid},
				$form->{precision} );
		...
		if ( $form->{"checked_$i"} ) {
			...
			if ( $form->{"calcdiscount_$i"} ) {
				if ( !$form->{"olddiscount_$i"} ) {
					$form->{"discount_$i"} =
					  $form->parse_amount( \%myconfig, $form->{"netamount_$i"} )
					  * $form->{"cashdiscount_$i"};
					$form->{"olddiscount_$i"} = $form->{"discount_$i"};
				}
			}
...
|;
  ...
  |;
			...
			$vc .=
			    qq|<td><input name="$form->{vc}" size=35 value="|
			  . $form->quote( $form->{ $form->{vc} } )
			  . qq|"></td>
		</tr>
		<tr>
		<th align=right>| . $locale->text( $vc{ $form->{vc} }{number} ) . qq|</th>
		<td><input name="$form->{vc}number" size=35 value="|
...
|;
	...
	$form->hide_form( map { "old$_" }
		  ( "$form->{ARAP}", "$form->{vc}", "$form->{vc}number" ) );
	$form->hide_form( map { "select$_" }
		  qw(currency department business paymentmethod) );
	$form->hide_form(
		map { "select$_" } (
			"$form->{ARAP}",          "$form->{ARAP}_paid",
			"$form->{ARAP}_discount", "$form->{vc}"
		)
	);
  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		<td colspan=2><input name="memo" size=30 value="|
	  . $form->quote( $form->{memo} )
	  . qq|"></td>
	      </tr>
	    </table>
	  </td>
	  ...
	  . qq|</select>
		...
		<td colspan=3><input name=source value="|
	  . $form->quote( $form->{source} )
	  . qq|" size=10></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Amount') . qq|</th>
		<td colspan=3><input name=amount size=11 value=|
	  . $form->format_amount( \%myconfig, $form->{amount}, $form->{precision} )
	  . qq|></td>
		<input type=hidden name=oldamount value=|
	  . $form->round_amount( $form->{amount}, $form->{precision} ) . qq|>
	      </tr>
	    </table>
	  </td>
	</tr>
...
sub list_invoices {
	...
	@column_index =
	  qw(invnumber transdate duedate amount due checked paid discount total);
...
|;
	...
	$column_data{transdate} =
	  qq|<th>| . $locale->text('Invoice Date') . "</th>";
...
|;
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		$column_data{invnumber} =
		  qq|<td width=30%>$form->{"invnumber_$i"}</td>|;
		$column_data{transdate} =
		  qq|<td width=30% nowrap>$form->{"transdate_$i"}</td>|;
		$column_data{duedate} =
		  qq|<td width=30% nowrap>$form->{"duedate_$i"}</td>|;
...
qq|<td align=right><input name="discount_$i" size=11 value=$form->{"discount_$i"}></td>|;
		}
		else {
...
|;
	...
	$column_data{paid} = qq|<th class=listtotal align=right>|
	  . $form->format_amount( \%myconfig, $totalpaid, $form->{precision},
		"&nbsp;" )
	  . qq|</th>|;
	$column_data{discount} = qq|<th class=listtotal align=right>|
	...
	$column_data{amount} = qq|<th class=listtotal align=right>|
	  . $form->format_amount( \%myconfig, $totalamount, $form->{precision},
		"&nbsp;" )
	  . qq|</th>|;
	$column_data{total} = qq|<th class=listtotal align=right>|
...
|;
		...
		%button = (
			...
			'Print' =>
			  { ndx => 5, key => 'P', value => $locale->text('Print') },
			'Post' => { ndx => 6, key => 'O', value => $locale->text('Post') },
...
sub post_payments {
	...
	foreach $ref ( @{ $form->{PR} } ) {
		...
		for (qw(checked source memo)) {
			$form->{"${_}_$i"} = $oldform{"${_}_$j"};
		}

		for (qw(id invnumber transdate duedate)) {
			$form->{"${_}_$i"} = $ref->{$_};
		}
			...
			if ( $ref->{calcdiscount} ) {
				$netamount =
				  $form->round_amount( $ref->{netamount} / $ref->{exchangerate},
					$form->{precision} );
				$form->{"discount_$i"} =
				  $form->round_amount( $netamount * $ref->{cashdiscount},
					$form->{precision} );
			}
		}

		$due = $form->round_amount(
		...
		$paid =
		  $form->round_amount( $paid - $form->{"paid_$i"}, $form->{precision} );

		$form->{"paid_$i"} =
		  $form->format_amount( \%myconfig,
			$form->{"paid_$i"} - $form->{"discount_$i"},
			$form->{precision} );
		$form->{"discount_$i"} =
	...
	for $i ( 1 .. $rowcount ) {
		...
		if ( $form->{"$form->{vc}_id_$i"} ne $form->{"$form->{vc}_id"} ) {
			...
			if ($ok) {
				$form->{amount} =
				  $form->format_amount( \%myconfig, $amount,
					$form->{precision} );
				if ( $form->{batch} ) {
				...
				$l = $k - 1;
				$form->info(
qq|$msg $oldform{"paid_$l"}, $form->{name} $form->{"$form->{vc}number"}\n|
				);
			}
		...
		if ( $form->{"checked_$i"} ) {
			$j++;
			$ok = 1;
			for (qw(id invnumber checked paid)) {
	...
	if ($ok) {
		$form->{amount} =
		  $form->format_amount( \%myconfig, $amount, $form->{precision} );
		if ( $form->{batch} ) {
			$batchid = $form->{batchid};
			VR->post_transaction( \%myconfig, \%$form );
		}
		...
		$oldform{header} = 1;
		$form->info(
qq|$msg $oldform{"paid_$k"}, $form->{name} $form->{"$form->{vc}number"}\n|
		);
	}
...
sub post_payment {
	...
	if ( $form->{batch} ) {
		...
		if ( $rc = VR->post_transaction( \%myconfig, \%$form ) ) {
			if ( $form->{callback} ) {
				$form->{callback} .= "&batch=$form->{batch}&batchdescription="
				  . $form->escape( $form->{batchdescription}, 1 );
				if ( !$batchid ) {
					$form->{callback} .=
					  "&batchid=$form->{batchid}&type=$form->{type}";
				}
			}
...
sub print_payments {
	...
	foreach $ref ( @{ $form->{PR} } ) {
		...
		for (qw(checked source memo language_code)) {
			$form->{"${_}_$i"} = $oldform{"${_}_$j"};
		}

		for (qw(id invnumber invdescription transdate duedate)) {
			$form->{"${_}_$i"} = $ref->{$_};
		}
		$form->{"$form->{vc}_id_$i"} = $ref->{"$form->{vc}_id"};
	...
	$msg =
	  ( $form->{vc} eq 'vendor' )
	  ? $locale->text('Printing check for')
	  : $locale->text('Printing receipt for');
	...
	for $i ( 1 .. $temp{rowcount} ) {
		...
		if ( $form->{"$form->{vc}_id_$i"} ne $form->{"$form->{vc}_id"} ) {
			...
			if ($ok) {
				...
				$oldform{header} = 1;
				$form->info(
					qq|$msg $form->{name} $form->{"$form->{vc}number"}\n|);
			}

			$ok             = 0;
			$j              = 0;
			$form->{amount} = 0;
			for (qw(invnumber invdescription invdate due paid)) {
				@{ $form->{$_} } = ();
			}
		...
		if ( $form->{"checked_$i"} ) {
			...
			$form->{amount} =
			  $form->parse_amount( \%myconfig, $oldform{"paid_$k"} );
			$form->{"$form->{vc}_id"} = $form->{"$form->{vc}_id_$i"};
		}
...
sub print_payment {
	...
	@a =
	  qw(name text_amount text_decimal address1 address2 city state zipcode country memo);
...
sub check_form {
	...
	if ( $form->round_amount( $amount, $form->{precision} ) != 0 ) {
		...
		push(
			@{ $form->{due} },
			$form->format_amount( \%myconfig, 0, $form->{precision} )
		);
		push(
			@{ $form->{discount} },
			$form->format_amount( \%myconfig, 0, $form->{precision} )
