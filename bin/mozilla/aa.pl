...
sub edit {
	...
	$form->{firsttime} = 1
	  ; # do not use parse_amount if is first time and amounts are not formatted.
	&update;
...
sub create_links {
	...
	@a =
	  qw(duedate taxincluded terms cashdiscount discountterms payment_accno payment_method);
	push @a, $form->{ARAP};
	for (@a) { $temp{$_} = $form->{$_} }

	if ( exists $form->{oldinvtotal} && $form->{oldinvtotal} < 0 ) {
		$form->{type} =
		  ( $form->{vc} eq 'customer' ) ? 'credit_note' : 'debit_note';
	...
	$form->{selectformname} =
	  qq|$form->{type}--| . $locale->text( $selectform{ $form->{type} } );
	...
	if ( @{ $form->{"all_$form->{vc}"} } ) {
		...
		for ( @{ $form->{"all_$form->{vc}"} } ) {
			$form->{"select$form->{vc}"} .=
			  qq|$_->{name} ($_->{"$form->{vc}number"})--$_->{id}\n|;
	...
	if ( @{ $form->{all_department} } ) {
		if ( $myconfig{department_id} and $myconfig{role} eq 'user' ) {
			$form->{selectdepartment} =
			  qq|$myconfig{department}--$myconfig{department_id}\n|;
		}
		else {
			$form->{selectdepartment} = "\n";
	...
	if ( @{ $form->{all_project} } ) {
		...
		for ( @{ $form->{all_project} } ) {
			$form->{selectprojectnumber} .= qq|$_->{projectnumber}--$_->{id}\n|;
		}
	}
	...
	for (
		qw(formname currency department employee projectnumber language paymentmethod)
	  )
	{
	...
	my ($linetax) = $dbh->selectrow_array(
		"SELECT fldvalue FROM defaults WHERE fldname='linetax'");
	if ( $linetax and $form->{id} ) {
		($linetax) = $dbh->selectrow_array(
"SELECT 1 FROM acc_trans WHERE trans_id = $form->{id} AND tax <> '' LIMIT 1"
		);
	}
	if ($linetax) {
		$form->{selecttax} = "\n";
		my $query =
qq|SELECT accno, description FROM chart WHERE link LIKE '%$form->{ARAP}_tax%' ORDER BY accno|;
		...
		foreach $ref ( @{ $form->{"$form->{ARAP}_links"}{$key} } ) {
			if ( $key eq "$form->{ARAP}_tax" ) {
				$form->{"select$form->{ARAP}_tax_$ref->{accno}"} =
				  $form->escape( "$ref->{accno}--$ref->{description}", 1 );
				next;
			}
...
"$form->{acc_trans}{$key}->[$i-1]->{accno}--$form->{acc_trans}{$key}->[0]->{description}";
				$form->{"discount_paid"} =
				  $form->{acc_trans}{$key}->[0]->{amount} * -1 * $ml;
				$form->{"discount_datepaid"} =
				  $form->{acc_trans}{$key}->[0]->{transdate};
				$form->{"olddiscount_datepaid"} =
				  $form->{acc_trans}{$key}->[0]->{transdate};
				$form->{"discount_source"} =
				  $form->{acc_trans}{$key}->[0]->{source};
				$form->{"discount_memo"} =
				  $form->{acc_trans}{$key}->[0]->{memo};

				$form->{"discount_exchangerate"} =
				  $form->{acc_trans}{$key}->[0]->{exchangerate};
				$form->{"discount_cleared"} =
				  $form->{acc_trans}{$key}->[0]->{cleared};
...
"$form->{acc_trans}{$key}->[0]->{paymentmethod_id}--$form->{acc_trans}{$key}->[0]->{paymentmethod}";
				...
				if ( $key eq "$form->{ARAP}_tax" ) {
					if ( !$form->{acc_trans}{$key}->[ $i - 1 ]->{id} ) {
						$form->{
							"${key}_$form->{acc_trans}{$key}->[$i-1]->{accno}"}
						  = "$form->{acc_trans}{$key}->[$i-1]->{accno}--$form->{acc_trans}{$key}->[$i-1]->{description}";
					...
					if ( $akey eq 'amount' ) {
						$form->{"description_$i"} =
						  $form->{acc_trans}{$key}->[ $i - 1 ]->{memo};
						$form->{"tax_$i"} =
						  $form->{acc_trans}{$key}->[ $i - 1 ]->{tax};
						$form->{"linetaxamount_$i"} =
						  $form->{acc_trans}{$key}->[ $i - 1 ]->{taxamount} *
						  -1;
						$form->{rowcount}++;
...
"$form->{acc_trans}{$key}->[$i-1]->{accno}--$form->{acc_trans}{$key}->[$i-1]->{description}";
	...
	$form->{"$form->{ARAP}_paid_$i"} = $form->{payment_accno}
	...
	if ( $form->{type} =~ /_note/ ) {
		...
		$form->{cd_available} =
		    ( $form->{taxincluded} )
		  ? ( $netamount + $tax ) * $form->{cashdiscount}
	...
	if ( $form->{id} ) {
		...
		for (@taxaccounts) {
			$form->{"calctax_$_"} = !$linetax;
		}    # Uncheck summary tax accounts by default when linetax is enabled.
	}

	for (qw(payment discount)) {
		$form->{"${_}_accno"} = $form->escape( $form->{"${_}_accno"}, 1 );
	}
	...
	if ( !$form->{readonly} ) {
		if ( $form->{batch} ) {
			$form->{readonly} = 1
			  if $myconfig{acs} =~ /Vouchers--Payable Batch/
				  || $form->{approved};
		}
		else {
			$form->{readonly} = 1
			  if $myconfig{acs} =~ /$form->{ARAP}--(Add Transaction| Note)/;
		}
	}
...
qq|<textarea name=notes rows=$rows cols=50 wrap=soft>$form->{notes}</textarea>|;
	...
	$intnotes =
qq|<textarea name=intnotes rows=$rows cols=50 wrap=soft>$form->{intnotes}</textarea>|;
...
|;
	...
	if ( $form->{"$form->{vc}"} ) {
		$vcdetail =
qq|<a href="ct.pl?login=$form->{login}&path=$form->{path}&action=edit&db=$form->{vc}&id=$form->{"$form->{vc}_id"}" target="_blank">?</a>|;
	}
	if ( $form->{"select$form->{vc}"} ) {

		# Add customer/vendor link
		$addvc =
"ct.pl?action=add&db=$form->{vc}&path=$form->{path}&login=$form->{login}&addvc=1";
...
|;
	...
	if ( $form->{"select$form->{ARAP}_discount"} ) {
		...
		<input name=cashdiscount size=3 value=|
		  . $form->format_amount( \%myconfig, $form->{cashdiscount} ) . qq|> / 
		<input name=discountterms size=3 value=$form->{discountterms}> |
		  . $locale->text('Net') . qq|
...
|;
	...
	if ( $form->{vc} eq 'vendor' ) {
		...
		<td><input name=dcn size=60 value="|
		  . $form->quote( $form->{dcn} )
		  . qq|"></td>
	      </tr>
|;
...
|;
	...
	$form->hide_form( map { "select$_" }
		  qw(formname currency department employee projectnumber language paymentmethod tax)
	);
	$form->hide_form( "old$form->{vc}", "$form->{vc}_id",
		"old$form->{vc}number" );
	$form->hide_form(
		map { "select$_" } (
			"$form->{ARAP}_amount", "$form->{ARAP}",
			"$form->{ARAP}_paid",   "$form->{ARAP}_discount"
		)
	);
  ...
  <tr valign=top>
    <td>
	  ...
	  . qq|"></td>
		...
		<td><input name=ordnumber size=20 value="|
	  . $form->quote( $form->{ordnumber} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>|
	  ...
	  . qq| <font color=red>*</font></th>
		...
		<td><input name=ponumber size=20 value="|
	  . $form->quote( $form->{ponumber} )
	  . qq|"></td>
	      </tr>
	      $terms
	    </table>
...
|;
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{selecttax} ) {
			...
			$line1 .= qq|<td><input name="amount_$i" size=11 value="|
			  . $form->format_amount( \%myconfig, $form->{"amount_$i"},
				$form->{precision} )
			  . qq|" accesskey="$i"></td>
                            <input type=hidden name="oldamount_$i" value="$form->{"amount_$i"}"><td></td>|;
			$line1 .= qq|<td><select name="$form->{ARAP}_amount_$i">|
			  . $form->select_option(
				$form->{"select$form->{ARAP}_amount"},
				$form->{"$form->{ARAP}_amount_$i"}
			  ) . qq|</select>|;
			$line1 .=
			    qq|<td><select name="tax_$i">|
			  . $form->select_option( $form->{selecttax}, $form->{"tax_$i"} )
			  . qq|</select>
                             <input type=hidden name="oldtax_$i" value='$form->{"tax_$i"}'></td>|;
			$line1 .=
qq|<td align="right"><input type=text name="linetaxamount_$i" size=10 value="|
			  . $form->format_amount( \%myconfig, $form->{"linetaxamount_$i"},
				$form->{precision} )
			  . qq|"></td>|;
			$line1 .= qq|</tr>|;
			...
			{
				$line2 .=
qq|<td><textarea name="description_$i" rows=$rows cols=40 title="|
				  . $locale->text('Description')
				  . qq|">$form->{"description_$i"}</textarea></td>|;
			}
			else {
				$line2 .=
				    qq|<td><input name="description_$i" size=40 value="|
			...
			if ( $form->{selectprojectnumber} ) {
				$line2 .= qq|<td><select name="projectnumber_$i">|
				  . $form->select_option( $form->{selectprojectnumber},
					$form->{"projectnumber_$i"}, 1 )
				  . qq|</select></td>|;
			}
			else {
				$line2 .= qq|<td></td>|;
...
qq|<td><textarea name="description_$i" rows=$rows cols=40 title="|
			...
			if ( $form->{selectprojectnumber} ) {
				$line1 .= qq|<td><select name="projectnumber_$i">|
				  . $form->select_option( $form->{selectprojectnumber},
					$form->{"projectnumber_$i"}, 1 )
				  . qq|</select></td>|;
			}
...
|;
	...
	if ( !$form->{"$form->{ARAP}_discount_paid"} ) {
		$form->{"$form->{ARAP}_discount_paid"} =
		  $form->unescape( $form->{discount_accno} );
	}
	...
	$column_data{exchangerate} =
	  "<th>" . $locale->text('Exch') . " <font color=red>*</font></th>";
  ...
  <tr>
    <td><b>| 
		  ...
		  . qq|:</b> |
		  . $form->format_amount( \%myconfig, $form->{cd_available},
			$form->{precision} )
		  . qq|</td>
  </tr>
|;
		...
		if ( $form->{currency} ne $form->{defaultcurrency} ) {
			$form->{discount_exchangerate} =
			  $form->format_amount( \%myconfig,
				$form->{discount_exchangerate} );
			$exchangerate =
qq|<input name="discount_exchangerate" size=10 value=$form->{"discount_exchangerate"}>|
			  . $form->hide_form(qw(olddiscount_datepaid));
		}

		$column_data{paid} =
		  qq|<td align=center><input name="discount_paid" size=11 value=|
...
|;
	...
	if ( $form->{discount_paid} && $form->{cdt} ) {
		...
		for ( split / /, $form->{taxaccounts} ) {
			$cd_tax += $form->round_amount( $form->{"tax_$_"} * $cdtp,
				$form->{precision} );
		}
	}
	...
	$form->{subtotal} =
	  $form->format_amount( \%myconfig,
		$form->{subtotal} - $form->{discount_paid},
		$form->{precision} );
	$form->{invtotal} =
	  $form->format_amount( \%myconfig, $form->{invtotal}, $form->{precision} );
...
|;
	...
	$form->{"$form->{ARAP}_paid_$form->{paidaccounts}"} =
	  $form->unescape( $form->{payment_accno} );
	$form->{"paymentmethod_$form->{paidaccounts}"} =
	  $form->unescape( $form->{payment_method} );
...
qq|<input name="exchangerate_$i" size=10 value=$form->{"exchangerate_$i"}>|
		...
		$column_data{paid} =
		  qq|<td align=center><input name="paid_$i" size=11 value=|
		...
		$column_data{exchangerate} = qq|<td align=center>$exchangerate</td>|;
		$column_data{datepaid} =
qq|<td align=center><input name="datepaid_$i" size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)" value=$form->{"datepaid_$i"}></td>|;
		$column_data{source} =
		    qq|<td align=center><input name="source_$i" size=11 value="|
		  . $form->quote( $form->{"source_$i"} )
		  . qq|"></td>|;
		$column_data{memo} =
		    qq|<td align=center><input name="memo_$i" size=11 value="|
		  . $form->quote( $form->{"memo_$i"} )
		  . qq|"></td>|;
		$column_data{paymentmethod} =
		  qq|<td align=center><select name="paymentmethod_$i">|
		  . $form->select_option( $form->{"selectpaymentmethod"},
			$form->{"paymentmethod_$i"}, 1 )
		  . qq|</select></td>|;
...
";
	...
	$outstanding = $form->round_amount( $form->{oldinvtotal} - $totalpaid,
		$form->{precision} );
	...
	if ($outstanding) {
	  ...
	  <tr>
            <td colspan=4><b>| 
			  ...
			  . ":</b> "
			  . $form->format_amount( \%myconfig, $outstanding,
				$form->{precision} )
			  . qq|</td>
	  </tr>
|;
	  ...
	  <tr>
            <td colspan=4><b>| 
			  ...
			  . ":</b> "
			  . $form->format_amount( \%myconfig, $outstanding * -1,
				$form->{precision} )
			  . qq|</td>
	  </tr>
|;
	...
	$form->hide_form(
		qw(city state country paidaccounts payment_accno discount_accno payment_method)
	);
...
sub form_footer {
	...
	if ( $form->{readonly} ) {
		...
		%button = (
			...
			'New Number' =>
			  { ndx => 10, key => 'M', value => $locale->text('New Number') },
			'Delete' =>
		...
		if ( $form->{id} ) {
			...
			if ( $form->{locked} || $transdate <= $form->{closedto} ) {
				for ( "Post", "Print and Post", "Delete" ) {
					delete $button{$_};
				}
			}

			if ( !$latex ) {
				for ( "Print and Post", "Print and Post as new" ) {
					delete $button{$_};
				}
			}
			...
			for ( "Post as new", "Print and Post as new", "Delete" ) {
				delete $button{$_};
			}
	...
	if ( $form->{id} and $debits_credits_footer ) {
		...
		$table->map_cell(
			sub { return $form->format_amount( \%myconfig, shift, 4 ) },
			'amount' );
...
sub update {
	...
	if ( !$display ) {
		...
		for ( 1 .. $form->{rowcount} ) {
			...
			if ( $form->{"tax_$_"} ) {
				...
				if (  !$form->{"linetaxamount_$_"}
					|| $form->{"tax_$_"} ne $form->{"oldtax_$_"}
					|| $form->{"amount_$_"} != $form->{"oldamount_$_"}
					|| $form->{taxincluded} ne $form->{oldtaxincluded} )
				{
					if ( $form->{"amount_$_"} )
					{ # Calculate only when there is amount. Otherwise leave the user entered amount as it is.
						if ( $form->{taxincluded} ) {
							$form->{"linetaxamount_$_"} =
							  $form->{"amount_$_"} -
							  $form->{"amount_$_"} /
							  ( 1 + $form->{"${taxaccno}_rate"} );
						}
						else {
							$form->{"linetaxamount_$_"} =
							  $form->{"amount_$_"} *
		...
		$form->{cashdiscount} =
		  ...
		  if !$form->{firsttime};
		$form->{discount_paid} =
		  $form->parse_amount( \%myconfig, $form->{discount_paid} )
		  if !$form->{firsttime};
		...
		if ( $form->{oldterms} != $form->{terms} ) {
			$form->{duedate} =
			  $form->add_date( \%myconfig, $form->{transdate}, $form->{terms},
				'days' );
			$newterms           = 1;
		...
		if ( $form->{duedate} ne $form->{oldduedate} ) {
			$form->{terms} =
			  $form->datediff( \%myconfig, $form->{transdate},
				$form->{duedate} );
			$newterms           = 1;
			$form->{oldterms}   = $form->{terms};
			$form->{oldduedate} = $form->{duedate};
		...
		if ( $form->{transdate} ne $form->{oldtransdate} ) {
			...
			if ( @{ $form->{all_employee} } ) {
				...
				$form->{selectemployee} =
				  $form->escape( $form->{selectemployee}, 1 );
			}
		}
	}
		...
		for ( 0 .. 1 ) {
			...
			foreach $item (@taxaccounts) {
				if ( ( $form->{"${item}_rate"} * $ml ) > 0 ) {
					...
					$form->{"select$form->{ARAP}_tax_$item"} =
					  qq|$item--$form->{"${item}_description"}|;
		...
		foreach $item (@taxaccounts) {
			...
			if ( $form->{"calctax_$item"} ) {
				$a =
				  ...
				  : $form->{invtotal};
				$form->{"tax_$item"} =
				  $form->round_amount( $a * $form->{"${item}_rate"},
					$form->{precision} );
			}
			$form->{"select$form->{ARAP}_tax_$item"} =
			  qq|$item--$form->{"${item}_description"}|;
	...
	if ( $form->{discount_paid} ) {
		if (   $form->{discount_datepaid} ne $form->{olddiscount_datepaid}
			|| $form->{currency} ne $form->{oldcurrency} )
		{
			$form->{discount_exchangerate} = $form->check_exchangerate(
	...
	for $i ( 1 .. $form->{paidaccounts} ) {
		if ( $form->{"paid_$i"} ) {
			for (qw(olddatepaid datepaid source memo cleared paymentmethod)) {
			...
			if (   $form->{"datepaid_$j"} ne $form->{"olddatepaid_$j"}
				|| $form->{currency} ne $form->{oldcurrency} )
			{
				$form->{"exchangerate_$j"} = $form->check_exchangerate(
					\%myconfig, $form->{currency},
					$form->{"datepaid_$j"},
					( $form->{ARAP} eq 'AR' ) ? 'buy' : 'sell'
				);
			...
			if ( $j++ != $i ) {
				for (
					qw(olddatepaid datepaid source memo paid exchangerate cleared)
				  )
				{
					delete $form->{"${_}_$i"};
				}
			...
			for (qw(olddatepaid datepaid source memo paid exchangerate cleared))
			{
				delete $form->{"${_}_$i"};
			}
		}
	...
	$form->{payment_accno} =
	  $form->escape( $form->{"$form->{ARAP}_paid_$form->{paidaccounts}"}, 1 );
	$form->{payment_method} =
	  $form->escape( $form->{"paymentmethod_$form->{paidaccounts}"}, 1 );
...
sub post {
	...
	$form->isvaldate( \%myconfig, $form->{transdate},
		$locale->text('Invalid transdate...') );
	...
	if ( $form->{discount_paid} ) {
		...
		for (qw(paid datepaid source memo exchangerate cleared)) {
			$form->{"${_}_$i"} = $form->{"discount_$_"};
		}
		...
		if ( $form->{"paid_$i"} ) {
			$datepaid = $form->datetonum( \%myconfig, $form->{"datepaid_$i"} );
			$expired = $form->datetonum(
				\%myconfig,
				$form->add_date(
					\%myconfig,             $form->{transdate},
					$form->{discountterms}, 'days'
				)
			);

			$form->isblank( "datepaid_$i",
				$locale->text('Cash Discount date missing!') );

			$form->error(
				$locale->text('Cannot post cash discount for a closed period!')
			) if ( $datepaid <= $form->{closedto} );

			$form->error( $locale->text('Date for cash discount past due!') )
			  if ( $datepaid > $expired );

			$form->error(
				$locale->text('Cash discount exceeds available discount!') )
			  if $form->parse_amount( \%myconfig, $form->{"paid_$i"} ) >
				  ( $form->{oldinvtotal} * $form->{cashdiscount} );

			if ( $form->{currency} ne $form->{defaultcurrency} ) {
				$form->{"exchangerate_$i"} = $form->{exchangerate}
				  if ( $transdate == $datepaid );
				$form->isblank( "exchangerate_$i",
					$locale->text('Exchange rate for cash discount missing!') );
			}
		}
	}
	...
	if ( $form->{callback} ) {
		...
		$form->{callback} .=
"&batch=$form->{batch}&batchid=$form->{batchid}&transdate=$form->{transdate}&batchdescription="
...
sub search {
	...
	$form->error( $locale->text('Access denied!') )
	  if $myconfig{acs} =~ $form->{level};

	my $old_number = $form->{"$form->{vc}number"
	  };    # customer/vendor number is changed in $form->create_links
	$form->create_links( $form->{ARAP}, \%myconfig, $form->{vc} );
	...
	for ( @{ $form->{"$form->{ARAP}_links"}{ $form->{ARAP} } } ) {
		$form->{"select$form->{ARAP}"} .=
		  "<option>" . $form->quote("$_->{accno}--$_->{description}") . "\n";
	}
...
qq|<input name="l_name" class=checkbox type=checkbox value=Y checked> $vclabel|;
	$l_customernumber =
qq|<input name="l_customernumber" class=checkbox type=checkbox value=Y checked> $vcnumber|;
	$l_till = qq|<input name="l_till" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Till');
	...
	if ( @{ $form->{"all_$form->{vc}"} } ) {
		...
		for ( @{ $form->{"all_$form->{vc}"} } ) {
			...
			$form->{"select$form->{vc}"} .=
			    qq|<option value="|
			  . $form->quote( $_->{name} )
			  . qq|--$_->{id}" $selected>$_->{name} ($_->{"$form->{vc}number"})\n|;
		}
		$vc = qq|
              <tr>
	...
	if ( @{ $form->{all_warehouse} } ) {
		if ( $myconfig{warehouse} and $myconfig{role} eq 'user' ) {
			$form->{selectwarehouse} =
			  qq|$myconfig{warehouse}--$myconfig{warehouse_id}\n|;
		}
		else {
			$form->{selectwarehouse} = "\n";
			...
			for ( @{ $form->{all_warehouse} } ) {
				$form->{selectwarehouse} .= qq|$_->{description}--$_->{id}\n|;
			}
		}
...
|;
	...
	$openclosed = qq|
	  ...
	  . $locale->text('Open')
	  . qq|</td>
		<td nowrap><input name=closed class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Closed')
	  . qq|</td>
		<td nowrap><input name=onhold class=checkbox type=checkbox value=Y> |
	  . $locale->text('On Hold')
	  . qq|</td>
		<td nowrap><input name=paidlate class=checkbox type=checkbox value=Y> |
	  . $locale->text('Paid Late')
	  . qq|</td>
		<td nowrap><input name=paidearly class=checkbox type=checkbox value=Y> |
	  . $locale->text('Paid Early')
	  . qq|</td>
	      </tr>
|;
...
qq|<input name="l_transdate" class=checkbox type=checkbox value=Y checked> |
	...
	push @a, qq|<input name="l_address" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Address');
	...
	push @a,
	  qq|<input name="l_paymentdiff" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Payment Difference');
	push @a,
	  qq|<input name="l_paid" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Paid');
	push @a,
	  qq|<input name="l_paymentmethod" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Payment Method');
	push @a, qq|<input name="l_duedate" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Due Date');
	push @a,
qq|<input name="l_due" class=checkbox type=checkbox value=Y $form->{due_checked}> |
	  . $locale->text('Due');
	...
	push @a, qq|<input name="l_notes" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Notes');
	push @a, qq|<input name="l_intnotes" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Internal Notes');
	...
	push @a, qq|<input name="l_waybill" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Waybill');
	push @a, qq|<input name="l_dcn" class=checkbox type=checkbox value=Y> |
	  . $locale->text('DCN');
...
sub transactions {
	...
	for (qw(direction oldsort till outstanding path login summary)) {
		$href .= qq|&$_=$form->{$_}|;
	}
	...
	if ( $form->{"$form->{vc}number"} ) {
		$callback .= "&$form->{vc}number="
		  . $form->escape( $form->{"$form->{vc}number"}, 1 );
		$href .=
		  "&$form->{vc}number=" . $form->escape( $form->{"$form->{vc}number"} );
		$option .= "\n<br>" if ($option);
		$name =
		  ( $form->{vc} eq 'customer' )
	...
	if ( $form->{shippingpoint} ) {
		$callback .=
		  "&shippingpoint=" . $form->escape( $form->{shippingpoint}, 1 );
		$href .= "&shippingpoint=" . $form->escape( $form->{shippingpoint} );
		$option .= "\n<br>" if ($option);
		$option .=
		  $locale->text('Shipping Point') . " : $form->{shippingpoint}";
	}
	if ( $form->{shipvia} ) {
	...
	if ( !$form->{summary} ) {
		...
		@column_index =
		  ( @a, (qw(source debit credit accno memo projectnumber)) );
	}
	...
	$column_data{$namefld} =
	  "<th><a class=listheading href=$href&sort=$namefld>$namenumber</a></th>";
	$column_data{address} =
	  "<th class=listheading>" . $locale->text('Address') . "</th>";
	$column_data{netamount} =
	  "<th align=right class=listheading>" . $locale->text('Amount') . "</th>";
	$column_data{tax} =
	  "<th align=right class=listheading>" . $locale->text('Tax') . "</th>";
	$column_data{amount} =
	  "<th align=right class=listheading>" . $locale->text('Total') . "</th>";
	$column_data{paid} =
	  "<th align=right class=listheading>" . $locale->text('Paid') . "</th>";
	$column_data{paid} =
	    "<th align=right><a class=listheading href=$href&sort=paid>"
	  . $locale->text('Paid')
	  . "</a></th>";
	$column_data{paymentmethod} =
	    "<th><a class=listheading href=$href&sort=paymentmethod>"
	  . $locale->text('Payment Method')
	  . "</a></th>";
	$column_data{datepaid} =
	...
	$column_data{intnotes} =
	  "<th class=listheading>" . $locale->text('Internal Notes') . "</th>";
	$column_data{employee} =
	...
	$column_data{shipvia} =
	  ...
	  . "</a></th>";
	$column_data{waybill} =
	    "<th><a class=listheading href=$href&sort=waybill>"
	  . $locale->text('Waybill')
	  . "</a></th>";
	$column_data{dcn} =
	    "<th><a class=listheading href=$href&sort=dcn>"
	  . $locale->text('DCN')
	  . "</a></th>";
	$column_data{paymentdiff} =
	    "<th><a class=listheading href=$href&sort=paymentdiff>"
	  . $locale->text('+/-')
	  . "</a></th>";
	...
	$column_data{description} =
	  ...
	  . "</a></th>";
	$column_data{memo} =
	  "<th class=listheading>" . $locale->text('Line Item') . "</th>";
...
|;
	...
	foreach $ref ( @{ $form->{transactions} } ) {
		...
		if ( $form->{l_curr} ) {
			...
			for (qw(netamount amount paid)) {
				$column_data{"fx_$_"} = "<td align=right>"
				  . $form->format_amount( \%myconfig, $ref->{"fx_$_"},
					$form->{precision}, "&nbsp;" )
				  . "</td>";
			}
		...
		$column_data{invnumber} =
"<td align=left><a href=$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>";

		for (qw(notes intnotes description memo)) {
			$ref->{$_} =~ s/\r?\n/<br>/g;
		}
		for (qw(transdate datepaid duedate)) {
			$column_data{$_} = "<td align=left nowrap>$ref->{$_}&nbsp;</td>";
		}
		for (
			qw(department ordnumber ponumber notes intnotes warehouse shippingpoint shipvia waybill employee manager till source memo description projectnumber address dcn paymentmethod)
		  )
		{
		...
		if ( $ref->{paymentdiff} <= 0 ) {
			$column_data{paymentdiff} =
			  qq|<td class="plus1" align=right>$ref->{paymentdiff}&nbsp;</td>|;
		}
		else {
			$column_data{paymentdiff} =
			  qq|<td class="plus0" align=right>+$ref->{paymentdiff}&nbsp;</td>|;
		}
...
qq|<td align=left><a href=ca.pl?path=$form->{path}&login=$form->{login}&action=list_transactions&accounttype=standard&accno=$ref->{accno}&fromdate=$form->{transdatefrom}&todate=$form->{transdateto}&sort=transdate&l_subtotal=$form->{l_subtotal}&prevreport=$callback>$ref->{accno}</a></td>|;
		...
		$column_data{name} =
qq|<td align=left><a href=ct.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{"$form->{vc}_id"}&db=$form->{vc}&callback=$callback>$ref->{name}</a></td>|;
...
|;
	...
	$column_data{due} = "<th class=listtotal align=right>"
	  . $form->format_amount( \%myconfig, $totalamount - $totalpaid,
		$form->{precision}, "&nbsp;" )
	  . "</th>";
	...
	if ( $form->{l_curr} && $form->{sort} eq 'curr' && $form->{l_subtotal} ) {
		$column_data{fx_netamount} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totalfxnetamount,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
		...
		$column_data{fx_amount} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totalfxamount,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
		$column_data{fx_paid} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totalfxpaid, $form->{precision},
			"&nbsp;" )
		  . "</th>";
	...
	if ( $myconfig{acs} !~ /$form->{ARAP}--$form->{ARAP}/ ) {
		...
		if ( $form->{ARAP} eq 'AR' ) {
			$button{'AR--Add Transaction'}{code} =
			  qq|<input class="submit noprint" type=submit name=action value="|
...
sub transactions_to_csv {
	...
	foreach $ref ( @{ $form->{transactions} } ) {
		...
		$column_data{tax} =
		  $form->format_amount( \%myconfig, $ref->{amount} - $ref->{netamount},
			$form->{precision}, " " );
		$column_data{due} =
		  $form->format_amount( \%myconfig, $ref->{amount} - $ref->{paid},
			$form->{precision}, " " );
	...
	if ( $form->{l_curr} && $form->{sort} eq 'curr' && $form->{l_subtotal} ) {
		$column_data{fx_netamount} =
		  $form->format_amount( \%myconfig, $totalfxnetamount,
			$form->{precision}, " " );
		$column_data{fx_tax} =
		  $form->format_amount( \%myconfig, $totalfxamount - $totalfxnetamount,
			$form->{precision}, " " );
		$column_data{fx_amount} =
		  $form->format_amount( \%myconfig, $totalfxamount, $form->{precision},
			" " );
		$column_data{fx_paid} =
		  $form->format_amount( \%myconfig, $totalfxpaid, $form->{precision},
			" " );
...
sub subtotal_csv {
	...
	$column_data{tax} =
	  $form->format_amount( \%myconfig, $subtotalamount - $subtotalnetamount,
		$form->{precision}, " " );
	$column_data{amount} =
	  $form->format_amount( \%myconfig, $subtotalamount, $form->{precision},
		" " );
	$column_data{paid} =
	...
	if ( $form->{l_curr} && $form->{sort} eq 'curr' && $form->{l_subtotal} ) {
		$column_data{fx_tax} =
		  $form->format_amount( \%myconfig,
			...
			$form->{precision}, " " );
		$column_data{fx_amount} =
		  $form->format_amount( \%myconfig, $subtotalfxamount,
			$form->{precision}, " " );
		$column_data{fx_paid} =
		  $form->format_amount( \%myconfig, $subtotalfxpaid, $form->{precision},
			" " );
		$column_data{fx_due} =
		  $form->format_amount( \%myconfig, $subtotalfxmount - $subtotalfxpaid,
			$form->{precision}, " " );
	}
...
sub subtotal {
	...
	if ( $form->{l_curr} && $form->{sort} eq 'curr' && $form->{l_subtotal} ) {
		...
		$column_data{fx_paid} = "<th class=listsubtotal align=right>"
		  . $form->format_amount( \%myconfig, $subtotalfxpaid,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
...
sub view {
	...
	$table->modify( td => { align => 'right' }, 'amount' );
	$table->map_cell(
		sub { return $form->format_amount( \%myconfig, shift, 4 ) }, 'amount' );
