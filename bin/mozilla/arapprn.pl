...
sub print_check {
	...
	push(
		@{ $form->{due} },
		$form->format_amount(
			\%myconfig, $form->{oldinvtotal}, $form->{precision}
		)
	);
	...
	for (qw(employee paymentmethod)) {
		( $form->{$_}, $form->{"${_}_id"} ) = split /--/, $form->{$_};
	}

	push @a,
	  qw(employee paymentmethod notes intnotes companyemail companywebsite company address tel fax businessnumber);
...
sub print_transaction {
		...
		if ( $form->{"tax_$_"} ) {
			...
			if ( $form->{cdt} ) {
				$cdt =
				  ( $form->{discount_paid} )
				  ? $form->{"tax_$_"}
				  : $amount * ( 1 - $cashdiscount );
				$cd_tax += $form->round_amount( $cdt, $form->{precision} );
				push(
					@{ $form->{cd_tax} },
					$form->format_amount(
						\%myconfig, $cdt, $form->{precision}
					)
				);
			}
			else {
	...
	if ( $form->{formname} eq 'remittance_voucher' ) {
		$form->isblank( "dcn",
			qq|$form->{"$form->{ARAP}_paid_$form->{paidaccounts}"} : |
	...
	for (qw(cd_subtotal cd_amount cd_invtotal invtotal subtotal paid total)) {
		$form->{$_} =
	...
	for (qw(description notes intnotes)) {
		$form->{$_} =~ s/<%(.*?)%>/$fld = lc $1; $form->{$fld}/ge;
	}

	@a =
	  qw(employee paymentmethod invnumber transdate duedate notes intnotes ndcn rvc);

	push @a,
	  qw(companyemail companywebsite company address tel fax businessnumber text_amount text_decimal text_out_decimal text_out_amount);
