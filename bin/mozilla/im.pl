...
sub import {
	...
	if (   $form->{type} eq 'sales_invoice'
		or $form->{type} eq 'payment'
		or $form->{type} eq 'transactions' )
	{
		...
		if ( @{ $form->{all_paymentaccount} } ) {
			...
			for ( @{ $form->{all_paymentaccount} } ) {
				$selectpaymentaccount .= qq|$_->{accno}--$_->{description}\n|;
			}
			if (   ( $form->{type} eq 'sales_invoice' )
				or ( $form->{type} eq 'transactions' and $form->{ARAP} eq 'AR' )
			  )
			{
				$paymentaccount = qq|
	  <tr>
	...
	<tr>
	  ...
	  <td><select name=currency>|
			...
			for ( @{ $form->{arap_accounts} } ) {
				$selectarapaccounts .= qq|$_->{accno}--$_->{description}\n|;
			}
			$arapaccounts = qq|
         <tr>
	  <th align=right>| . $locale->text("$form->{ARAP} Account") . qq|</th>
	  <td>
	    <select name=arapaccount>|
			  . $form->select_option($selectarapaccounts)
			  . qq|</select>
	  </td>
	</tr>| if $form->{ARAP};
			...
			if ( $form->{type} ne 'transactions' ) {
				...
				for ( @{ $form->{income_accounts} } ) {
					$selectincomeaccounts .=
					  qq|$_->{accno}--$_->{description}\n|;
				}
				$incomeaccounts = qq|
         <tr>
	  ...
	  <td>
	    <select name=incomeaccount>|
				  . $form->select_option($selectincomeaccounts)
				  . qq|</select>
	  </td>
	  ...
	  <td>
	    <select name=expenseaccount>|
				  . $form->select_option($selectexpenseaccounts)
				  . qq|</select>
	  </td>
		...
		my $tax;
		$tax =
		  qq|<input type=hidden name=taxaccounts value='$form->{taxaccounts}'>|;
	  ...
	  <td>
	    <select name=IC_inventory>|
			  . $form->select_option( $form->{"selectIC"} )
			  . qq|</select>
	  </td>
	</tr>
         <tr>
	  ...
	  <td>
	    <select name=IC_income>|
			  . $form->select_option( $form->{"selectIC_income"} )
			  . qq|</select>
	  </td>
	</tr>
         <tr>
	  ...
	  <td>
	    <select name=IC_expense>|
			  . $form->select_option( $form->{"selectIC_expense"} )
			  . qq|</select>
	  </td>
	</tr>|;
		}
	   ...
	   <td>$tax</td>
		...
		foreach $item (qw(arap discount payment)) {
			if ( @{ $form->{"${item}_accounts"} } ) {
				...
				$form->{"select$item"} =
				  $form->escape( $form->{"select$item"}, 1 );
			}
		}
  ...
  <tr>
	...
	<tr valign=top>
	  ...
	  <td>
	    <table>
	      <tr>
	        <td><input name=filetype type=radio class=radio value=CSV checked>&nbsp;|
	  . $locale->text('CSV')
	  . qq|</td>
		<td width=20></td>
	...
	<tr>
	  ...
	  <td><input name=mapfile type=radio class=radio value=1>&nbsp;|
	  . $locale->text('Yes')
	  . qq|&nbsp;
	      <input name=mapfile type=radio class=radio value=0 checked>&nbsp;|
	  . $locale->text('No') . qq|
	  </td>
	</tr>
      </table>
...
|;
	...
	$form->hide_form(
		qw(ARAP vc db defaultcurrency title type action nextsub login path));
...
sub export {
	...
	if ( $form->{type} eq 'payment' ) {
	  ...
	  <td>
	    <select name=paymentaccount>|
			  . $form->select_option( $form->{selectpaymentaccount} )
			  . qq|</select>
	  </td>
	</tr>
	<tr>
...
qq|<input name="l_invnumber" class=checkbox type=checkbox value=Y checked> |
	...
	push @a,
	  qq|<input name="l_description" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Description');
	push @a,
	  qq|<input name="l_dcn" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('DCN');
	push @a,
	  qq|<input name="l_name" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Company Name');
	push @a,
	  qq|<input name="l_companynumber" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Company Number');
	push @a,
qq|<input name="l_datepaid" class=checkbox type=checkbox value=Y checked> |
...
sub im_sales_invoice {
	...
	@column_index =
	  qw(ndx transdate invnumber customer customernumber city dcn invoicedescription total curr exchangerate totalqty unit duedate employee department warehouse);
	@flds = @column_index;
	shift @flds;
	push @flds,
...
|;
			...
			$column_data{total} = qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $form->{"total_$i"},
				$form->{precision} )
			  . qq|</td>|;
			$column_data{totalqty} =
			    qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $form->{"totalqty_$i"} )
			  . qq|</td>|;
			...
			if ( $form->{"customer_id_$i"} ) {
				$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox checked></td>|;
			}
			else {
...
|;
    ...
    <tr>
      <td>|;
		$form->info(
			$locale->text('The following parts could not be found:') . "\n\n" );
		for ( split /\n/, $form->{missingparts} ) {
			$form->info("$_\n");
		}
		print qq|
...
|;
	...
	$form->hide_form(
		qw(vc rowcount ndx type login path callback markpaid paymentaccount));

	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Sales Invoices') . qq|">
</form>
...
sub im_sales_order {
	...
	@column_index =
	  qw(ndx transdate ordnumber customer customernumber city orderdescription total curr totalqty unit duedate employee);
	@flds = @column_index;
	shift @flds;
	push @flds,
...
|;
			...
			if ( $form->{"customer_id_$i"} ) {
				$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox $form->{"checked_$i"}></td>|;
			}
			else {
				$column_data{ndx} = qq|<td>&nbsp;</td>|;
			}
...
|;
	...
	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Sales Orders') . qq|">
</form>
...
sub im_purchase_order {
	...
	@column_index =
	  qw(ndx transdate ordnumber vendor vendornumber city orderdescription total curr totalqty unit duedate employee);
	@flds = @column_index;
	shift @flds;
	push @flds,
...
|;
	...
	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Purchase Orders') . qq|">
</form>
	...
	for $k ( keys %ndx ) {
		...
		if ( $form->{"ndx_$k"} ) {
			...
			for $i ( @{ $ndx{$k} } ) {
				...
				for (
					qw(description unit deliverydate serialnumber itemnotes projectnumber)
				  )
				{
					$newform->{"${_}_$j"} = $form->{"${_}_$i"};
				}
				$newform->{"sellprice_$j"} =
			...
			if ( IM->import_sales_invoice( \%myconfig, \%$newform ) ) {
				$form->info(
qq| $newform->{invnumber}, $newform->{description}, $newform->{customernumber}, $newform->{name}, $newform->{city}, |
				);
				$myconfig{numberformat} = $numberformat;
				$form->info(
					$form->format_amount(
						\%myconfig, $form->{"total_$k"}, $form->{precision}
					)
				);
				$myconfig{numberformat} = "1000.00";
	...
	for $k ( keys %ndx ) {
		...
		if ( $form->{"ndx_$k"} ) {
			...
			if ( IM->import_sales_order( \%myconfig, \%$newform ) ) {
				$form->info(
qq| $newform->{ordnumber}, $newform->{description}, $newform->{customernumber}, $newform->{name}, $newform->{city}, |
				);
				$myconfig{numberformat} = $numberformat;
				$form->info(
					$form->format_amount(
...
sub import_purchase_orders {
	...
	for $k ( keys %ndx ) {
		...
		if ( $form->{"ndx_$k"} ) {
			...
			for (
				qw(ordnumber quonumber transdate vendor vendor_id reqdate shippingpoint shipvia waybill terms notes intnotes curr language_code ponumber cashdiscount discountterms AP taxincluded)
			  )
			{
				$newform->{$_} = $form->{"${_}_$k"};
			}
			...
			if ( IM->import_purchase_order( \%myconfig, \%$newform ) ) {
				$form->info(
qq| $newform->{ordnumber}, $newform->{description}, $newform->{vendornumber}, $newform->{name}, $newform->{city}, |
				);
				$myconfig{numberformat} = $numberformat;
				$form->info(
					$form->format_amount(
...
sub im_payment {
	...
	@column_index =
	  qw(runningnumber ndx invnumber description dcn name companynumber city datepaid amount);
	push @column_index, "exchangerate"
...
|;
		...
		$column_data{exchangerate} =
		    qq|<td><input name="exchangerate_$i" size=10 value=|
...
|;
	...
	$form->hide_form(
		qw(precision rowcount type paymentaccount currency defaultcurrency login path callback)
	);

	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Payments') . qq|">
</form>
...
sub import_payments {
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"ndx_$i"} ) {
			...
			for (qw(precision currency defaultcurrency)) {
				$newform->{$_} = $form->{$_};
			}
			for (
				qw(vc arap exchangerate datepaid amount source memo paymentmethod)
			  )
			{
				$newform->{$_} = $form->{"${_}_$i"};
			}
			...
			if ( CP->post_payment( \%myconfig, \%$newform ) ) {
				$form->info(
qq| $form->{"invnumber_$i"}, $form->{"description_$i"}, $form->{"companynumber_$i"}, $form->{"name_$i"}, $form->{"city_$i"}, |
				);
...
sub ex_payment {
	...
	$column_data{runningnumber} = "&nbsp;";
	$column_data{ndx} =
qq|<input name="allbox" type=checkbox class=checkbox value="1" checked onChange="CheckAll();">|;
...
|;
	...
	$form->hide_form(
		qw(column_index rowcount file filetype delimiter tabdelimited includeheader type paymentaccount paymentmethod currency defaultcurrency login path callback)
	);

	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Export Payments') . qq|">
</form>
...
sub im_parts {
	...
	@column_index =
	  qw(partnumber description unit partsgroup newpartsgroup listprice sellprice lastcost rop bin image drawing notes);
	@flds = @column_index;
	push @flds,
	  qw(parts_id partsgroup_id microfiche barcode tarrif_hscode countryorigin toolnumber);
...
|;
		...
		$column_data{runningnumber} = qq|<td align=right>$i</td>|;
		$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox value='1' checked></td>|;
...
|;
	...
	$form->hide_form(
		qw(taxaccounts IC_inventory IC_income IC_expense precision rowcount type login path callback)
	);

	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Parts') . qq|">
</form>
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"ndx_$i"} ) {
			...
			if ( $form->{"partsgroup_$i"} and !$form->{"partsgroup_id_$i"} ) {
				...
				$query =
qq|SELECT id FROM partsgroup WHERE partsgroup='$form->{"partsgroup_$i"}'|;
		...
		WHERE parts.id = $form->{"parts_id_$i"}|;
			...
			for ( split / /, $form->{taxaccounts} ) {
				$newform->{"IC_tax_$_"} = $form->{"IC_tax_$_"};
			}
			...
			if ( IC->save( \%myconfig, \%$newform ) ) {
				$form->info(
					qq| $form->{"partnumber_$i"}, $form->{"description_$i"}|);
				$form->info( " ... " . $locale->text('ok') . "\n" );
			}
			else {
...
sub im_partscustomer {
	...
	$form->error( $locale->text('Import File missing!') ) if !$form->{data};
	@column_index =
	  qw(partnumber description customernumber name pricegroup pricebreak sellprice validfrom validto curr);
	@flds = @column_index;
	...
	print
	  qq|<tr height="5"></tr><tr><td><table width=100%><tr class=listheading>|;
	for (@column_index) { print "\n<th>$column_data{$_}</th>" }
	print qq|</tr>|;
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		$column_data{runningnumber} = qq|<td align=right>$i</td>|;
		if ( $form->{"parts_id_$i"} )
		{    # and ($form->{"customer_id_$i"} or $form->{"pricegroup_id_$i"})){
			$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox value='1' checked></td>|;
		}
		else {
	...
	for (@column_index) { print "\n$column_data{$_}" }
	print
	  qq|</tr></table></td></tr><tr><td><hr size=3 noshade></td></tr></table>|;
...
sub im_partsvendor {
	...
	$form->error( $locale->text('Import File missing!') ) if !$form->{data};
	@column_index =
	  qw(partnumber description vendornumber name vendorpartnumber lastcost curr leadtime);
	@flds = @column_index;
...
sub im_vc {
	...
	push @column_index,
	  qw(id name salutation firstname lastname contacttitle phone fax email notes address1 address2 city state zipcode country);
	@flds = @column_index;
	push @flds,
...
|;
	...
	$form->hide_form(
		qw(arap_accno payment_accno taxaccounts taxincluded precision rowcount nextsub db type login path callback)
	);
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"ndx_$i"} ) {
			...
			$newform->{typeofcontact} = 'company'
			  if $newform->{typeofcontact} ne 'person';
			$newform->{taxaccounts}   = $form->{taxaccounts};
			...
			if ( CT->save( \%myconfig, \%$newform ) ) {
				$form->info(
					qq| $form->{"$form->{db}number_$i"}, $form->{"name_$i"}|);
				$form->info( " ... " . $locale->text('ok') . "\n" );
			}
			else {
...
|;
		...
		if ( $form->{"ndx_$i"} ) {
			...
			$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox value='Y' checked></td>|;
		}
...
|;
	...
	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Accounts') . qq|">
</form>
	...
	for my $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $form->{"ndx_$i"} ) {
			...
			if ( AM->save_account( \%myconfig, \%$newform ) ) {
				$form->info(
					qq| $form->{"account_$i"}, $form->{"description_$i"}|);
				$form->info( " ... " . $locale->text('ok') . "\n" );
			}
			else {
				$form->error( $locale->text('Saving failed!') );
...
sub im_gl {
	...
	@column_index =
	  qw(reference department department_id description transdate notes currency exchangerate accno accdescription debit credit source memo);
	@flds = @column_index;
	unshift @column_index, qw(runningnumber ndx);
...
|;
		...
		$column_data{debit} = qq|<td>|
		  . $form->format_amount( \%myconfig, $form->{"debit_$i"},
			$form->{precision} )
		  . qq|</td>|;
		$column_data{credit} = qq|<td>|
		  . $form->format_amount( \%myconfig, $form->{"credit_$i"},
			$form->{precision} )
		  . qq|</td>|;
...
|;
	...
	$column_data{debit} =
	    qq|<td>|
	  . $form->format_amount( \%myconfig, $debit_total, $form->{precision} )
	  . qq|</td>|;
	$column_data{credit} =
	    qq|<td>|
...
|;
	...
	if ( $debit_total eq $credit_total ) {
		print qq|
<input name=action class=submit type=submit value="|
		  . $locale->text('Import GL') . qq|">|;
	}
	else {
		$form->error(
			$locale->text('Debits and credits are not equal. Cannot import') );
	}
...
sub im_transactions {
	...
	if ( $form->{vc} eq 'customer' ) {
		@column_index =
		  qw(ndx invnumber customernumber name transdate account account_description amount description notes source memo);
	}
	else {
		@column_index =
		  qw(ndx invnumber vendornumber name transdate account account_description amount description notes source memo);
	}
	@flds = @column_index;
...
|;
		...
		$column_data{amount} = qq|<td align=right>|
...
|;
	...
	$column_data{amount} =
	    qq|<td align=right>|
	  . $form->format_amount( \%myconfig, $total_amount, $form->{precision} )
	  . qq|</td>|;
...
|;
	...
	$form->hide_form(
		qw(markpaid ARAP vc currency arapaccount incomeaccount paymentaccount expenseaccount precision rowcount type login path callback)
	);

	print qq|
<input name=action class=submit type=submit value="|
	  . $locale->text('Import Transactions') . qq|">
</form>
		...
		if ( $form->{"ndx_$i"} ) {
			...
			if ( $newform->{invnumber} ne $form->{"invnumber_$i"} ) {
				if ( $newform->{invnumber} ne 'null' ) {
					$form->info(
						"${m}. " . $locale->text('Add transaction ...') );
					if ( AA->post_transaction( \%myconfig, \%$newform ) ) {
						$form->info(
qq| $newform->{invnumber}, $form->{"$form->{vc}number_$i"}|
						);
						$form->info( " ... " . $locale->text('ok') . "\n" );
					}
					else {
				...
				$newform->{ $form->{vc} } =
				  qq|$form->{"name_$i"}--$form->{"$form->{vc}_id_$i"}|;
				...
				$newform->{employee} =
				  qq|$form->{"employee_$i"}--$form->{"employee_id_$i"}|;
				$linenum = 0;
			}
			$linenum += 1;
			$newform->{transdate} = $form->{"transdate_$i"};
