...
sub search {
	...
	%label = (
		invoice => { title => 'Invoices', name => [ 'Customer', 'Vendor' ] },
		remittance_voucher =>
		  { title => 'Remittance Vouchers', name => ['Customer'] },
		packing_list =>
		  { title => 'Packing Lists', name => [ 'Customer', 'Vendor' ] },
		pick_list =>
		  { title => 'Pick Lists', name => [ 'Customer', 'Vendor' ] },
...
|;
	...
	$form->{title} =
	    $locale->text( $label{ $form->{batch} }{title} ) . " "
	  . $locale->text( $label{ $form->{type} }{title} );

	if ( $form->{batch} ne 'queue' ) {
		$onhold = qq|
		<input name=onhold class=checkbox type=checkbox value=Y> |
		  . $locale->text('On Hold');
		...
		$openclosed = qq| 
              <tr>
	        <td></td>
	        <td colspan=3 nowrap><input name=open class=checkbox type=checkbox value=Y checked> |
		  . $locale->text('Open') . qq|
		<input name=closed class=checkbox type=checkbox value=Y> |
		  . $locale->text('Closed') . qq|
		$onhold
		<input name="printed" class=checkbox type=checkbox value=Y> |
		  . $locale->text('Printed') . qq|
		<input name="notprinted" class=checkbox type=checkbox value=Y $checked{print}> |
		  . $locale->text('Not Printed') . qq|
		<input name="emailed" class=checkbox type=checkbox value=Y> |
		  . $locale->text('E-mailed') . qq|
		<input name="notemailed" class=checkbox type=checkbox value=Y $checked{email}> |
		  . $locale->text('Not E-mailed') . qq|
		</td>
              </tr>
|;
	}
...
|;
	...
	<tr>
	  ...
	  <td colspan=3><select name=$vcn><option>\n|;
				...
				$form->{"select$vcn"} .=
				  qq|<option value="1--0">| . $locale->text('None')
				  if $k > 1;
...
sub print {
	...
	for my $i ( 1 .. $myform->{rowcount} ) {
		...
		if ( $myform->{"ndx_$i"} ) {
			...
			for (
				qw(login path media sendmode subject message format type header)
			  )
			{
				$form->{$_} = $myform->{$_};
			}
			...
			if ( $myform->{"module_$i"} eq 'oe' ) {
				...
				$form->{"${arap}_paid_$form->{paidaccounts}"} =
				  $form->{payment_accno};
				$inv = 'inv';
			}
			...
			$myform->info( "${r}. "
				  . $locale->text( $msg{ $myform->{batch} } )
				  . qq| ... $myform->{"reference_$i"}| );
			$myform->info(qq|, $myform->{description}|)
			  if $myform->{description};

			if ( $myform->{"module_$i"} ne 'jc' ) {
				if ( $form->{formname} =~ /_invoice/ ) {
					$total -=
					  $form->parse_amount( \%myconfig, $form->{"${inv}total"} );
				}
				else {
					$total +=
					  $form->parse_amount( \%myconfig, $form->{"${inv}total"} );
				...
				$myform->info(
qq|, $form->{"${inv}total"}, $form->{"$form->{vc}number"}, $form->{"$form->{vc}"} $form->{city}|
				);
			}
	...
	$myform->info( $locale->text('Total') . ": "
		  . $form->format_amount( \%myconfig, $total, $myform->{precision} ) )
	  if $total;
...
sub list_spool {
	...
	@a =
	  qw(direction oldsort path login type printcustomer printvendor batch allbox);
	...
	for (qw(customer vendor employee)) {
		...
		if ( $form->{"${_}number"} ) {
			$callback .= qq|&${_}number=|
			  . $form->escape( $form->{ $form->{"${_}number"} }, 1 );
			$href .= qq|&${_}number=|
			  . $form->escape( $form->{ $form->{"${_}number"} } );
			$option .= "\n<br>" if ($option);
			$option .=
			  $locale->text( $vc{$_}{number} ) . qq| : $form->{"${_}number"}|;
		}
...
qq|<th class=listheading width=1%><input name="allbox" type=checkbox class=checkbox value="1" $form->{allbox} onChange="CheckAll(); javascript:document.forms[0].submit()"><input type=hidden name=action value="$action"></th>|;
	...
	$column_header{vcnumber} =
	    "<th><a class=listheading href=$href&sort=vcnumber>"
	  . $locale->text('Number')
	  . "</a></th>";

	$column_header{email} =
	  "<th class=listheading>" . $locale->text('E-mail') . "</th>";
	$column_header{city} =
	  "<th class=listheading>" . $locale->text('City') . "</th>";
	$column_header{id} =
...
|;
	...
	foreach $ref ( @{ $form->{SPOOL} } ) {
		...
		$column_data{ndx} =
qq|<td><input name="ndx_$i" type=checkbox class=checkbox $form->{"ndx_$i"} $form->{"ndx_$i"}></td>|;
		...
		for (
			qw(description email city id invnumber ordnumber quonumber vcnumber)
		  )
		{
		...
		$column_data{name} =
qq|<td><a href=ct.pl?action=edit&id=$ref->{vc_id}&db=$ref->{db}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</a></td>|;

		if ( $ref->{module} eq 'oe' ) {
			$column_data{invnumber} = qq|<td>&nbsp</td>|;
			$column_data{ordnumber} =
qq|<td><a href=$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=$form->{type}&callback=$callback>$ref->{ordnumber}</a></td>
      <input type=hidden name="reference_$i" value="|
			  . $form->quote( $ref->{ordnumber} ) . qq|">|;

			$column_data{quonumber} =
qq|<td><a href=$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=$form->{type}&callback=$callback>$ref->{quonumber}</a></td>
    <input type=hidden name="reference_$i" value="|
			  . $form->quote( $ref->{quonumber} ) . qq|">|;

		}
		elsif ( $ref->{module} eq 'jc' ) {
			$column_data{id} =
qq|<td><a href=$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=$form->{type}&callback=$callback>$ref->{id}</a></td>
    <input type=hidden name="reference_$i" value="$ref->{id}">|;

			$column_data{name} =
qq|<td><a href=hr.pl?action=edit&id=$ref->{employee_id}&db=employee&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</a></td>|;
		}
		else {
			$column_data{invnumber} =
qq|<td><a href=$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=$form->{type}&callback=$callback>$ref->{invnumber}</a></td>
    <input type=hidden name="reference_$i" value="|
			  . $form->quote( $ref->{invnumber} ) . qq|">|;
		}
...
|;
	...
	$form->hide_form(
		qw(callback title type sort path login printcustomer dispatch printvendor customer customernumber vendor vendornumber employee employeenumber batch invnumber ordnumber quonumber description transdatefrom transdateto open closed onhold printed emailed notprinted notemailed precision)
	);
...
|;
	...
	if ( $form->{batch} eq 'email' ) {
		$message = qq|<tr>
                    <td nowrap><b>|
		  . $locale->text('Subject')
		  . qq|</b>&nbsp;<input name=subject size=30></td>
                  </tr>
		  <tr>
                    <td><b>|
		  . $locale->text('Message')
		  . qq|<br><textarea name=message rows=15 cols=60 wrap=soft>$form->{message}</textarea></td>
      </tr>|;
...
|;
	...
	%button = (
		...
		'Download zip file' =>
		  { ndx => 7, key => 'R', value => $locale->text('Download zip file') },
	);
