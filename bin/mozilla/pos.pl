...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=$form->{nextsub}&path=$form->{path}&login=$form->{login}"
...
sub openinvoices {
	...
	for (
		qw(open l_invnumber l_transdate l_name l_amount l_curr l_till l_subtotal summary)
	  )
	{
		$form->{$_} = 'Y';
	}
...
| . $form->hide_form(qw(forex));
	...
	$addvc =
	  "ct.pl?action=add&db=customer&path=$form->{path}&login=$form->{login}";
	...
	if ( $form->{selectcustomer} ) {
		...
		<td colspan=3><select name=customer onChange="javascript:document.forms[0].submit()">|
		  . $form->select_option( $form->{selectcustomer}, $form->{customer},
			1 )
		  . qq|</select>
		$addvc
		</td>
...
|;
		...
		$customer .= qq|
		  ...
		  . $locale->text('Customer')
		  . qq| <font color=red>*</font></th>
		<td colspan=3><input name=customer value="|
		  . $form->quote( $form->{customer} )
		  . qq|" size=35>
		$addvc
		</td>
	      </tr>
		...
		<td colspan=3><input name=customernumber value="|
		  . $form->quote( $form->{customernumber} )
		  . qq|" size=35>
		</td>
	      </tr>
|;
...
|;
	...
	$form->hide_form( map { "select$_" }
		  qw(currency customer department warehouse employee language AR AR_paid)
	);
	$form->hide_form(
		qw(id till type format printed title discount creditlimit creditremaining tradediscount business closedto locked customer_id payment_accno precision)
	);
	$form->hide_form( map { "old$_" }
		  qw(transdate customer language customernumber) );
...
|;
	...
	$form->hide_form(
		qw(city state country taxaccounts duedate invnumber transdate defaultcurrency)
	);
...
|;
	...
	for $i ( 1 .. $form->{paidaccounts} ) {
		...
		$column_data{source} =
		    qq|<td><input name="source_$i" size=10 value="|
		  . $form->quote( $form->{"source_$i"} )
		  . qq|"></td>|;
		$column_data{memo} =
		    qq|<td><input name="memo_$i" size=20 value="|
		  . $form->quote( $form->{"memo_$i"} )
		  . qq|"></td>|;
		$column_data{AR_paid} =
		  qq|<td><select name="AR_paid_$i">|
...
|;
	...
	$form->{change} =
	  $form->format_amount( \%myconfig, $form->{change}, $form->{precision},
		0 );
	$form->{totalpaid} =
...
|;
	...
	if ( $form->{readonly} ) {
		...
		%button = (
			...
			'Delete' =>
			  { ndx => 6, key => 'D', value => $locale->text('Delete') },
		);
...
<input type=hidden name=nextsub value=lookup_partsgroup>|;
				...
				foreach $item ( split /\n/, $form->{partsgroup} ) {
					...
					print
qq| <input class=submit type=submit name=action value="$spc|
					  . $form->quote($item)
					  . qq|">\n|
					  if $item;
				}
...
sub post {
	...
	$form->{"paid_1"} = $form->format_amount(
		\%myconfig,
		$form->parse_amount( \%myconfig, $form->{"paid_1"} ) -
		  ( $paid - $total ),
		$form->{precision}
	) if $paid > $total;
...
sub display_row {
	...
	$column_data{linetotal} =
	  qq|<th class=listheading>| . $locale->text('Extended') . qq|</th>|;
...
|;
	...
	for $i ( 1 .. $numrows ) {
		...
		$column_data{partnumber} =
		    qq|<td><input name="partnumber_$i" size=20 value="|
		  . $form->quote( $form->{"partnumber_$i"} )
		  . qq|" accesskey="$i" title="[Alt-$i]"></td>|;
...
qq|<td><textarea name="description_$i" rows=$rows cols=46 wrap=soft>$form->{"description_$i"}</textarea></td>|;
		...
		$column_data{qty} =
		    qq|<td align=right><input name="qty_$i" size=8 value=|
...
sub print_form {
	...
	push @a,
	  qw(companyemail companywebsite company address tel fax businessnumber username);
	$form->format_string(@a);
...
|;
	...
	@a =
	  qw(transdate reference name customernumber description paid curr source till);
	for (@a) { $form->{"l_$_"} = "Y" }
