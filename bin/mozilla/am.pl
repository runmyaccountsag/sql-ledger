...
sub add_account {
	...
	$form->{callback} =
"$form->{script}?action=list_account&path=$form->{path}&login=$form->{login}"
...
|;
	...
	$form->hide_form( map { "${_}_accno_id" }
		  qw(inventory income expense fxgain fxloss) );
	...
	<tr valign=top>
	  <td colspan=2>
		...
		<input name=AR_paid class=checkbox type=checkbox value=AR_paid $form->{AR_paid}>&nbsp;|
		  . $locale->text('Payment') . qq|<br>
		<input name=AR_discount class=checkbox type=checkbox value=AR_discount $form->{AR_discount}>&nbsp;|
		  . $locale->text('Discount') . qq|<br>
		...
		<input name=AP_amount class=checkbox type=checkbox value=AP_amount $form->{AP_amount}>&nbsp;|
		  . $locale->text('Expense/Asset') . qq|<br>
		<input name=AP_paid class=checkbox type=checkbox value=AP_paid $form->{AP_paid}>&nbsp;|
		  . $locale->text('Payment') . qq|<br>
		<input name=AP_discount class=checkbox type=checkbox value=AP_discount $form->{AP_discount}>&nbsp;|
		  . $locale->text('Discount') . qq|<br>
...
|;
	...
	print qq|
	  ...
	  <td><input name=gifi_accno size=9 value="|
	  . $form->quote( $form->{gifi_accno} )
	  . qq|"></td>
	</tr>
      </table>
    </td>
...
sub save_account {
	...
	if ( $form->{gifi_accno} ) {
		...
		if (
			!$dbh->selectrow_array(
				"SELECT COUNT(*) FROM gifi WHERE accno = '$form->{gifi_accno}'")
		  )
		{
	...
	foreach my $item (qw(AR AP)) {
		my $i = 0;
		for (
			"${item}_amount",   "${item}_paid",
			"${item}_discount", "${item}_tax"
		  )
		{
			$i++ if $form->{$_};
		}
		$form->error(
			$locale->text('Cannot set multiple options for') . " $item" )
		  if $i > 1;
	}
...
"$form->{script}?action=list_account&path=$form->{path}&login=$form->{login}";
	...
	$column_data{allow_gl} =
	  qq|<th class=listtop>| . $locale->text('GL') . qq|</a></th>|;
...
|;
	...
	foreach my $ref ( @{ $form->{CA} } ) {
		...
		if ( $ref->{charttype} eq "H" ) {
			...
			$column_data{accno} =
qq|<th><a class=listheading href=$form->{script}?action=edit_account&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{accno}</a></th>|;
			$column_data{gifi_accno} =
qq|<th class=listheading><a href=$form->{script}?action=edit_gifi&accno=$gifi_accno&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{gifi_accno}</a>&nbsp;</th>|;
			$column_data{description} =
			  qq|<th class=listheading>$ref->{description}&nbsp;</th>|;
...
<tr valign=top class=listrow$i>|;
			$column_data{accno} =
qq|<td><a href=$form->{script}?action=edit_account&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{accno}</a></td>|;
			$column_data{gifi_accno} =
qq|<td><a href=$form->{script}?action=edit_gifi&accno=$ref->{gifi_accno}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{gifi_accno}</a>&nbsp;</td>|;
			...
			$column_data{allow_gl} =
			  qq|<td align="center">$ref->{allow_gl}&nbsp;</td>|;
...
sub list_gifi {
	...
	# construct callback
	my $callback =
"$form->{script}?action=list_gifi&path=$form->{path}&login=$form->{login}";
	...
	$column_data{description} =
	  qq|<th class=listheading>| . $locale->text('Description') . qq|</a></th>|;
...
<tr valign=top class=listrow$i>|;
		...
		$column_data{accno} =
qq|<td><a href=$form->{script}?action=edit_gifi&coa=1&accno=$accno&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{accno}</a></td>|;
		$column_data{description} = qq|<td>$ref->{description}&nbsp;</td>|;
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  <th align=right>| . $locale->text('Description') . qq|</th>
	  <td><input name=description size=60 value="|
	  . $form->quote( $form->{description} )
	  . qq|"></td>
	</tr>
      </table>
...
sub add_department {
	...
	$form->{callback} =
"$form->{script}?action=add_department&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_department {
	...
	my $href =
"$form->{script}?action=list_department&direction=$form->{direction}&path=$form->{path}&login=$form->{login}";
...
|;
		...
		$column_data{description} =
qq|<td><a href=$form->{script}?action=edit_department&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>|;
...
<table width=100%>
  ...
  <tr>
    <td></td>
    <td><input type=radio class=radio name=role value="C" $checked{C}> |
	  . $locale->text('Cost Center') . qq|
        <input type=radio class=radio name=role value="P" $checked{P}> |
...
sub add_business {
	...
	$form->{callback} =
"$form->{script}?action=add_business&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_business {
	...
	my $href =
"$form->{script}?action=list_business&direction=$form->{direction}&path=$form->{path}&login=$form->{login}";
...
|;
		...
		$discount = $form->format_amount( \%myconfig, $ref->{discount} * 100,
			$form->{precision}, "&nbsp" );

		$column_data{description} =
qq|<td><a href=$form->{script}?action=edit_business&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>|;
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><input name=description size=30 value="|
	  . $form->quote( $form->{description} )
	  . qq|"></td>
	<tr>
	<tr>
...
sub add_dispatch {
	...
	$form->{callback} =
"$form->{script}?action=add_dispatch&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_dispatch {
	...
	my $href =
"$form->{script}?action=list_dispatch&direction=$form->{direction}&path=$form->{path}&login=$form->{login}";
...
|;
		...
		$column_data{description} =
qq|<td><a href=$form->{script}?action=edit_dispatch&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>|;
...
sub add_payment_method {
	...
	$form->{callback} =
"$form->{script}?action=add_payment_method&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_paymentmethod {
	...
	my $href =
"$form->{script}?action=list_paymentmethod&direction=$form->{direction}&oldsort=$form->{oldsort}&path=$form->{path}&login=$form->{login}";
...
"$form->{script}?action=list_paymentmethod&direction=$form->{direction}&oldsort=$form->{oldsort}&path=$form->{path}&login=$form->{login}";
	...
	$column_data{rn} =
	    qq|<th><a class=listheading href=$href&sort=rn>|
	  . $locale->text('No')
	  . qq|</a></th>|;
	$column_data{description} =
	    qq|<th width=90%><a class=listheading href=$href&sort=description>|
	  . $locale->text('Description')
	  . qq|</a></th>|;
	$column_data{fee} =
	  qq|<th class=listheading>| . $locale->text('Fee') . qq|</th>|;
...
|;
		...
		$fee =
		  $form->format_amount( \%myconfig, $ref->{fee}, $form->{precision},
			"&nbsp" );

		$column_data{rn} = qq|<td align=right>$ref->{rn}</td>|;
		$column_data{description} =
qq|<td><a href=$form->{script}?action=edit_paymentmethod&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>|;
		$column_data{fee} = qq|<td align=right>$fee</td>|;
		$column_data{plus} =
qq|<td><a href=$form->{script}?action=move&db=paymentmethod&fld=id&id=$ref->{id}&move=up&path=$form->{path}&login=$form->{login}&callback=$callback><img src=$images/up.png alt="+" border=0></a></td>|;
		$column_data{minus} =
qq|<td><a href=$form->{script}?action=move&db=paymentmethod&fld=id&id=$ref->{id}&move=down&path=$form->{path}&login=$form->{login}&callback=$callback><img src=$images/down.png alt="-" border=0></a></td>|;
...
|;
	...
	print qq|
<input class=submit type=submit name=action value="|
	  . $locale->text('Add Payment Method') . qq|">|;
...
sub paymentmethod_header {
	...
	$form->{description} = $form->quote( $form->{description} );
	$form->{fee} =
	  $form->format_amount( \%myconfig, $form->{fee}, $form->{precision} );
...
sub add_sic {
	...
	$form->{callback} =
	  "$form->{script}?action=add_sic&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_sic {
	...
	my $href =
"$form->{script}?action=list_sic&direction=$form->{direction}&oldsort=$form->{oldsort}&path=$form->{path}&login=$form->{login}";
...
|;
			$column_data{code} =
qq|<th><a href=$form->{script}?action=edit_sic&code=$ref->{code}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{code}</a></th>|;
...
|;
			...
			$column_data{code} =
qq|<td><a href=$form->{script}?action=edit_sic&code=$ref->{code}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{code}</a></td>|;
			$column_data{description} = qq|<td>$ref->{description}</td>|;
...
<table width=100%>
  ...
  <tr>
    ...
    <td><input name=code size=10 value="|
	  . $form->quote( $form->{code} )
	  . qq|"></td>
  </tr>
  <tr>
    <td></td>
    <th align=left><input name=sictype class=checkbox type=checkbox value="H" $checked{H}> |
	  . $locale->text('Heading')
	  . qq|</th>
  </tr>
  <tr>
...
sub add_language {
	...
	$form->{callback} =
"$form->{script}?action=add_language&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_language {
	...
	my $href =
"$form->{script}?action=list_language&direction=$form->{direction}&oldsort=$form->{oldsort}&path=$form->{path}&login=$form->{login}";
...
|;
		...
		$column_data{code} =
qq|<td><a href=$form->{script}?action=edit_language&code=$ref->{code}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{code}</a></td>|;
		$column_data{description} = qq|<td>$ref->{description}</td>|;
...
sub list_templates {
	...
	my $href =
"$form->{script}?action=list_templates&direction=$form->{direction}&oldsort=$form->{oldsort}&file=$form->{file}&path=$form->{path}&login=$form->{login}";
...
|;
		...
		$column_data{code} =
qq|<td><a href=$form->{script}?action=display_form&file=$myconfig{templates}/$ref->{code}/$form->{file}&path=$form->{path}&login=$form->{login}&code=$ref->{code}&callback=$callback>$ref->{code}</a></td>|;
		$column_data{description} = qq|<td>$ref->{description}</td>|;
...
sub display_form {
	...
	AM->load_template( \%$form, \@accessfolders,
		$locale->text('Access Denied!') );

	$form->{title} = $form->{file};

	$form->{body} =~
s/<%include (.*?)%>/<a href=$form->{script}\?action=display_form&file=$myconfig{templates}\/$form->{code}\/$1&path=$form->{path}&login=$form->{login}>$1<\/a>/g;
...
sub edit_template {
	...
	$form->{callback} =
"$form->{script}?action=display_form&file=$form->{file}&code=$form->{code}&path=$form->{path}&login=$form->{login}";
...
sub save_template {
	...
	AM->save_template( \%$form, \@accessfolders,
		$locale->text('Access Denied!') );
	...
	<tr>
	  <th align=left>|;
		...
		if ( $form->{"taxdescription_$i"} eq $sametax ) {
			...
			print
qq|$form->{"taxaccno_$i"}</th><th align=left>$form->{"taxdescription_$i"}|;
		}
...
sub update_taxes {
	...
	foreach $item ( keys %tax ) {
		...
		if ( $i >= 1 && $validto ) {
			push @tax,
			  { id => $id, accno => $accno, taxdescription => $taxdescription };
		}
	}
	...
	foreach my $key ( keys %{ $form->{accno} } ) {
		foreach my $accno ( sort keys %{ $form->{accno}{$key} } ) {
			...
			if ( $form->{accno}{$key}{$accno}{id} == $form->{defaults}{$key} ) {
				$form->{$key} =
				  qq|$accno--$form->{accno}{$key}{$accno}{description}|;
			}
		}
	}
...
<table width=100%>
  ...
  <tr>
    <td>
	  ...
	  <td><input name=company size=35 value="|
	  . $form->quote( $form->{company} )
	  . qq|"></td>
	</tr>
	<tr valign=top>
	...
	<tr>
	  ...
	  <td><input name=businessnumber size=25 value="|
	  . $form->quote( $form->{businessnumber} )
	  . qq|"></td>
	</tr>
	<tr>
	...
	<tr>
	  ...
	  <td><input name=cdt class=checkbox type=checkbox value="1" $checked{cdt}>&nbsp;|
	  . $locale->text('Taxable')
	  . qq|</td>
	</tr>
	<tr>
	  <th align=right>| . $locale->text('Line Tax') . qq|</th>
	  <td><input name=linetax class=checkbox type=checkbox value="1" $checked{linetax}>&nbsp;|
	  . $locale->text('Yes')
	  . qq|</td>
	</tr>
	<tr>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><select name=IC>|
	  . $form->select_option( $form->{"selectIC"}, $form->{IC} )
	  . qq|</select></td>
	</tr>
	<tr>
	...
	<tr>
	  ...
	  <td><select name=IC_expense>|
	  . $form->select_option( $form->{"selectIC_expense"}, $form->{IC_expense} )
	  . qq|</select></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('Foreign Exchange Gain') . qq|</th>
	  <td><select name=FX_gain>|
	  . $form->select_option( $form->{"selectFX_gain"}, $form->{FX_gain} )
	  . qq|</select></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('Foreign Exchange Loss') . qq|</th>
	  <td><select name=FX_loss>|
	  . $form->select_option( $form->{"selectFX_loss"}, $form->{FX_loss} )
	  . qq|</select></td>
	</tr>
      </table>
    </td>
...
|;
	...
	$form->{optional} =
"companyemail companywebsite company address address1 address2 city state zip country tel fax yearend weightunit businessnumber closedto revtrans audittrail method cdt linetax namesbynumber typeofcontact";
...
sub config {
	...
	for (qw(mm-dd-yy mm/dd/yy dd-mm-yy dd/mm/yy dd.mm.yy yyyy-mm-dd)) {
		$form->{selectdateformat} .= "$_\n";
	}
	$form->{selectdateformat} = "yyyy-mm-dd" if $myconfig{dbdriver} eq 'mysql';

	for (qw(1,000.00 1000.00 1.000,00 1000,00 1'000.00)) {
		$form->{selectnumberformat} .= "$_\n";
	}
...
|;
	...
	my $outputformat = qq|
	  ...
	  . $form->select_option( $selectoutputformat, $myconfig{outputformat},
		undef, 1 )
	  . qq|</select></td>
	      </tr>
|;
	...
	$form->{title} =
	  $locale->text('Edit Preferences for') . qq| $form->{login}|;
  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		<td><input name=name size=20 value="|
	  . $form->quote( $myconfig{name} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right>| . $locale->text('E-mail') . qq|</th>
	  ...
	  . $form->select_option( $form->{selectnumberformat},
	  ...
	  . $form->select_option( $form->{selectcountrycode},
		$myconfig{countrycode}, undef, 1 )
	  . qq|</select></td>
	      </tr>
	      <tr>
...
sub save_preferences {
	...
	if ( $form->{new_password} eq $form->{old_password} ) {
		...
		$form->error(
			$locale->text('Password may only contain alphanumeric characters!')
		) if $form->{new_password} =~ /\W/;
	}
...
sub backup {
	...
	if ( $form->{media} eq 'email' ) {
		$form->error(
			$locale->text('No email address for') . " $myconfig{name}" )
	...
	if ( $form->{media} eq 'email' ) {
		$form->redirect(
			$locale->text('Backup sent to') . qq| $myconfig{email}| );
	}
...
sub audit_control {
	...
	my %checked;
	for (
		qw(revtrans audittrail aruniq apuniq gluniq souniq pouniq trackingitemsuniq nontrackingitemsuniq extendedlog)
	  )
	{
...
sub doclose {
	...
	for ( keys %uniq ) {
		if ( $form->{$_} ) {
			$msg .= "<p>";
			$msg .=
			  $locale->text('Unique numbers enforced for') . " " . $uniq{$_};
		}
	}
...
sub add_warehouse {
	...
	$form->{callback} =
"$form->{script}?action=add_warehouse&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub list_warehouse {
	...
	my $href =
"$form->{script}?action=list_warehouse&direction=$form->{direction}&path=$form->{path}&login=$form->{login}";
...
"$form->{script}?action=list_warehouse&direction=$form->{direction}&path=$form->{path}&login=$form->{login}";
	...
	$column_data{description} =
	    qq|<th width=30%><a class=listheading href=$href>|
	  . $locale->text('Description')
	  . qq|</a></th>|;
	$column_data{address} =
...
|;
		...
		$column_data{description} =
qq|<td><a href=$form->{script}?action=edit_warehouse&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>|;
		$column_data{address} =
qq|<td>$ref->{address1} $ref->{address2} $ref->{city} $ref->{state} $ref->{zipcode} $ref->{country}</td>|;
...
<table width=100%>
  ...
  <tr>
    ...
    <td><input name=address1 size=35 maxlength=32 value="|
	  . $form->quote( $form->{address1} )
	  . qq|"></td>
  </tr>
  <tr>
    <th></th>
    <td><input name=address2 size=35 maxlength=32 value="|
	  . $form->quote( $form->{address2} )
	  . qq|"></td>
  </tr>
  <tr>
    <th align=right nowrap>| . $locale->text('City') . qq|</th>
    <td><input name=city size=35 maxlength=32 value="|
	  . $form->quote( $form->{city} )
	  . qq|"></td>
  </tr>
  <tr>
    <th align=right nowrap>| . $locale->text('State/Province') . qq|</th>
    <td><input name=state size=35 maxlength=32 value="|
	  . $form->quote( $form->{state} )
	  . qq|"></td>
  </tr>
  <tr>
    <th align=right nowrap>| . $locale->text('Zip/Postal Code') . qq|</th>
    <td><input name=zipcode size=10 maxlength=10 value="|
	  . $form->quote( $form->{zipcode} )
	  . qq|"></td>
  </tr>
  <tr>
    <th align=right nowrap>| . $locale->text('Country') . qq|</th>
    <td><input name=country size=35 maxlength=32 value="|
	  . $form->quote( $form->{country} )
	  . qq|"></td>
  </tr>
  <tr>
    <td colspan=2><hr size=3 noshade></td>
...
sub yearend {
	...
	for ( @{ $form->{chart} } ) {
		$form->{selectchart} .= "$_->{accno}--$_->{description}\n";
	}
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  . $form->select_option( $form->{selectchart} ) . qq|</select></td>
	</tr>
	<tr>
          <th align=right>| . $locale->text('Method') . qq|</th>
          <td><input name=method class=radio type=radio value=accrual $checked{accrual}>&nbsp;|
	  . $locale->text('Accrual')
	  . qq|&nbsp;<input name=method class=radio type=radio value=cash $checked{cash}>&nbsp;|
	  . $locale->text('Cash')
	  . qq|</td>
        </tr>
      </table>
...
sub recurring_transactions {
	...
	$form->{title} =
	  $locale->text('Recurring Transactions') . " / $form->{company}";
...
qq|<th class=listheading width=1%><input name="allbox" type=checkbox class=checkbox value="1" $form->{allbox} onChange="CheckAll(); javascript:document.forms[0].submit()"><input type=hidden name=action value="$action"></th>|;
	...
	$column_data{description} =
	  ...
	  . "</a></th>";
	$column_data{name} =
	    "<th nowrap><a class=listheading href=$href&sort=name>"
	  . $locale->text('Company Name')
	  . "</a></th>";
	$column_data{vcnumber} =
	    "<th nowrap><a class=listheading href=$href&sort=vcnumber>"
	  . $locale->text('Company Number')
	  . "</a></th>";
	$column_data{nextdate} =
	    "<th><a class=listheading href=$href&sort=nextdate>"
	  . $locale->text('Next')
	  . "</a></th>";
	$column_data{enddate} =
	    "<th><a class=listheading href=$href&sort=enddate>"
	  . $locale->text('Ends')
	  . "</a></th>";
	$column_data{amount} =
	...
	$column_data{repeat} =
	  "<th class=listheading>" . $locale->text('Every') . "</th>";
	$column_data{howmany} =
	  "<th class=listheading>" . $locale->text('Times') . "</th>";
	$column_data{recurringemail} =
	  "<th class=listheading nowrap>" . $locale->text('E-mail') . "</th>";
	$column_data{recurringprint} =
	  "<th class=listheading>" . $locale->text('Print') . "</th>";
...
|;
		...
		foreach $ref ( @{ $form->{transactions}{$transaction} } ) {
			...
			if ( !$ref->{expired} ) {
				...
				$column_data{ndx} =
qq|<td><input name="ndx_$k" class=checkbox type=checkbox value=$ref->{id} $checked></td>|;
				$column_data{nextdate} =
qq|<td nowrap><input name="nextdate_$k" size=11 value="$ref->{nextdate}" title="$myconfig{dateformat}" onChange="validateDate(this)"></td>|;
			}
			...
			$column_data{reference} =
qq|<td nowrap><a href=$form->{script}?action=edit_recurring&id=$ref->{id}&vc=$ref->{vc}&path=$form->{path}&login=$form->{login}&module=$ref->{module}&invoice=$ref->{invoice}&transaction=$ref->{transaction}&recurringnextdate=$ref->{nextdate}&callback=$callback>$reference</a></td>|;
			...
			if ( $ref->{module} eq 'ar' ) {
				...
				$column_data{name} =
qq|<td><a href=ct.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{name_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
			}
			if ( $ref->{module} eq 'ap' ) {
...
qq|<td><a href=ct.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{name_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
			...
			$column_data{id} =
qq|<td><a href=$module?action=edit&id=$ref->{id}&vc=$ref->{vc}&path=$form->{path}&login=$form->{login}&type=$type&callback=$callback>$ref->{id}</a></td>|;

			$column_data{repeat}  = "<td align=right nowrap>$repeat</td>";
			$column_data{howmany} = "<td align=right nowrap>"
			  . $form->format_amount( \%myconfig, $ref->{howmany} ) . "</td>";
			$column_data{amount} = "<td align=right nowrap>"
			  . $form->format_amount( \%myconfig, $ref->{amount},
				$form->{precision} )
			  . "</td>";
...
|;
	...
	%button = (
		...
		'Deselect all' =>
		  { ndx => 3, key => 'A', value => $locale->text('Deselect all') },
		'Process Transactions' => {
			ndx   => 4,
			key   => 'P',
			value => $locale->text('Process Transactions')
		}
	);
...
sub process_transactions {
	...
	for ( my $i = 1 ; $i <= $pt->{lastndx} ; $i++ ) {
		if ( $pt->{"ndx_$i"} ) {
			...
			for (qw(login path stylesheet timeout precision)) {
				$form->{$_} = $pt->{$_};
			}
			$form->{id}     = $id;
			$form->{header} = $header;
			...
			if ( $pt->{arid} || $pt->{apid} || $pt->{oeid} ) {
				if ( $pt->{arid} || $pt->{apid} ) {
						...
						for ( 1 .. $form->{rowcount} - 1 ) {
							$form->{"amount_$_"} =
							  $form->format_amount( \%myconfig,
								$form->{"amount_$_"}, $form->{precision} );
						}
						for ( split / /, $form->{taxaccounts} ) {
							$form->{"tax_$_"} =
							  $form->format_amount( \%myconfig,
								$form->{"tax_$_"}, $form->{precision} );
						}
					}
					...
					$form->{discount_paid} =
					  $form->format_amount( \%myconfig, $form->{discount_paid},
						$form->{precision} );
					...
					if ( $form->{currency} ne $form->{defaultcurrency} ) {
						$exchangerate =
						  $form->get_exchangerate( \%myconfig, undef,
							$form->{currency}, $form->{transdate}, $buysell );
						$form->{exchangerate} =
						  $form->format_amount( \%myconfig, $exchangerate )
						  if $exchangerate;
					}
					...
					if ( $pt->{payment} ) {
						...
						for ( $j = 1 ; $j <= $form->{paidaccounts} ; $j++ ) {
							...
							if ( $form->{currency} ne $form->{defaultcurrency} )
							{
								$form->{"exchangerate_$j"} =
								  $form->{exchangerate};
								$exchangerate =
								  $form->get_exchangerate( \%myconfig, undef,
									$form->{currency}, $form->{"datepaid_$j"},
									$buysell );
								$form->{"exchangerate_$j"} =
								  $form->format_amount( \%myconfig,
									$exchangerate )
						...
						for (qw(datepaid datepaid_1 olddatepaid_1 paid_1)) {
							delete $form->{$_};
						}
						$form->{"$form->{ARAP}_paid_1"} =
						  $form->{payment_accno};
						$form->{"paymentmethod_1"} = $form->{payment_method};
						$form->{paidaccounts} = 1;
					}
					for (qw(id recurring printed emailed queued)) {
						delete $form->{$_};
					}
					...
					for (qw(invnumber reference description)) {
						$form->{$_} = $form->unquote( $form->{$_} );
					}
					...
					for ( "$ordnumber", "reference", "description" ) {
						$form->{$_} = $form->unquote( $form->{$_} );
					}
					$form->{closed} = 0;
	...
	$form->{callback} =
"am.pl?action=recurring_transactions&path=$form->{path}&login=$form->{login}&header=$form->{header}";
	$form->redirect;
...
sub print_recurring {
	...
	if ( $pt->{recurringprint} ) {
		...
		for ( $j = 0 ; $j <= $#f ; $j += 3 ) {
			...
			@a = (
				"perl", "$form->{script}",
"action=reprint&module=$form->{module}&type=$form->{type}&login=$form->{login}&path=$form->{path}&id=$form->{id}&formname=$f[$j]&format=$f[$j+1]&media=$media&vc=$form->{vc}&ARAP=$form->{ARAP}"
			);
...
sub email_recurring {
	...
	if ( $pt->{recurringemail} ) {
		...
		for ( my $j = 0 ; $j <= $#f ; $j += 2 ) {
			...
			@a = (
				"perl", "$form->{script}",
"action=reprint&module=$form->{module}&type=$form->{type}&login=$form->{login}&path=$form->{path}&id=$form->{id}&formname=$f[$j]&format=$f[$j+1]&media=email&vc=$form->{vc}&ARAP=$form->{ARAP}&message=$message"
			);
...
<h4>|
	  . $locale->text('Are you sure you want to remove all the locks')
	  . qq|</h4>

<input name=action class=submit type=submit value="|
	  . $locale->text('Remove Locks') . qq|">
</form>
...
sub bank_accounts {
	...
	$column_header{name} =
	  qq|<th class=listheading>| . $locale->text('Bank') . qq|</th>|;
	$column_header{iban} =
	...
	$column_header{address} =
	  qq|<th class=listheading>| . $locale->text('Address') . qq|</th>|;
	$column_header{membernumber} =
	  qq|<th class=listheading>| . $locale->text('Member No.') . qq|</th>|;
	$column_header{rvc} =
	  qq|<th class=listheading>| . $locale->text('RVC') . qq|</th>|;
	$column_header{dcn} =
	  qq|<th class=listheading>| . $locale->text('DCN') . qq|</th>|;
...
|;
	...
	foreach $ref ( @{ $form->{ALL} } ) {
		...
		$ref->{address} =~ s/\n/<br>/g;
		for (qw(name description membernumber rvc dcn)) {
			$column_data{$_} = "<td>$ref->{$_}&nbsp;</td>";
		}
		for (qw(iban bic address)) {
			$column_data{$_} = "<td nowrap>$ref->{$_}&nbsp;</td>";
		}
		$column_data{accno} =
"<td><a href=$form->{script}?action=edit_bank&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{accno}</a></td>";
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><input name=name size=32 maxlength=64 value="|
	...
	<tr>
	  <th align=right nowrap>| . $locale->text('Address') . qq|</th>
	  <td><input name=address1 size=32 maxlength=32 value="|
	  . $form->quote( $form->{address1} )
	  . qq|"></td>
	</tr>
	<tr>
	  <th></th>
	  <td><input name=address2 size=32 maxlength=32 value="|
	  . $form->quote( $form->{address2} )
	  . qq|"></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('City') . qq|</th>
	  <td><input name=city size=32 maxlength=32 value="|
	  . $form->quote( $form->{city} )
	  . qq|"></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('State/Province') . qq|</th>
	  <td><input name=state size=32 maxlength=32 value="|
	  . $form->quote( $form->{state} )
	  . qq|"></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('Zip/Postal Code') . qq|</th>
	  <td><input name=zipcode size=11 maxlength=10 value="|
	  . $form->quote( $form->{zipcode} )
	  . qq|"></td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('Country') . qq|</th>
	  <td><input name=country size=32 maxlength=32 value="|
	  . $form->quote( $form->{country} )
	  . qq|"></td>
	</tr>
	<tr>
...
sub list_exchangerates {
	...
	if ( $form->{currency} ) {
		...
		$column_header{ $form->{currency} } =
		  "<th class=listheading colspan=2>$form->{currency}</th>";
	}
	else {
		@curr = split /:/, $form->{currencies};
		shift @curr;
		...
		for $curr (@curr) {
			...
			$column_header{$curr} =
			  "<th class=listheading colspan=2>$curr</th>";
		}
	}
	...
	$column_header{transdate} =
	    "<th><a class=listheading href=$href&sort=$form->{sort}>"
	  . $locale->text('Date')
	  . "</a></th>";
...
|;
	...
	foreach $ref ( @{ $form->{transactions} } ) {
		...
		if ( $ref->{transdate} eq $samedate ) {
			$href =
"$form->{script}?action=edit_exchangerate&transdate=$ref->{transdate}&currencies=$form->{currencies}&path=$form->{path}&login=$form->{login}&callback=$callback";
			$column_data{transdate} =
			  "<td><a href=$href>$ref->{transdate}</a></td>";
			for (qw(buy sell)) {
				$column_data{"$ref->{curr}$_"} = "<td>"
				  . $form->format_amount( \%myconfig, $ref->{$_}, undef,
					"&nbsp;" )
				  . qq|</td>|;
			}
		}
...
|;
	...
	if ( $myconfig{acs} !~ /System--System/ ) {
		$i = 1;
		$button{'System--Exchange Rates'}{code} =
		  qq|<input class=submit type=submit name=action value="|
		  . $locale->text('Add Exchange Rate') . qq|"> |;
	}
...
sub edit_exchangerate {
	...
	$form->{callback} =
"$form->{script}?action=edit_exchangerate&transdate=$form->{transdate}&currencies=$form->{currencies}&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
sub add_exchange_rate {
	...
	$form->{callback} =
"$form->{script}?action=add_exchange_rate&currencies=$form->{currencies}&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
...
|;
	...
	<tr>
	  ...
	  <th align=left>$_</th>
	  <td><input name="${_}buy" value="|
		  . $form->format_amount( \%myconfig, $form->{"${_}buy"} )
		  . qq|"></td>
	  <td><input name="${_}sell" value="|
		  . $form->format_amount( \%myconfig, $form->{"${_}sell"} )
		  . qq|"></td>
	</tr>
|;
	}
...
sub list_currencies {
	...
	$column_data{rn} =
	  ...
	  . qq|</a></th>|;
	$column_data{curr} =
	    qq|<th width=99%><a class=listheading href=$href&sort=curr>|
	  . $locale->text('Currency')
	  . qq|</a></th>|;
	$column_data{precision} =
	  qq|<th class=listheading>| . $locale->text('Precision') . qq|</th>|;
	$column_data{plus}  = qq|<th class=listheading>&nbsp;</th>|;
	$column_data{minus} = qq|<th class=listheading>&nbsp;</th>|;
...
|;
		...
		$column_data{rn} = qq|<td align=right>$ref->{rn}</td>|;
		$column_data{curr} =
qq|<td><a href=$form->{script}?action=edit_currency&curr=$ref->{curr}&rn=$ref->{rn}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{curr}</a></td>|;
		$column_data{precision} = qq|<td align=right>$ref->{precision}</td>|;
		$column_data{plus} =
qq|<td align=center><a href=$form->{script}?action=move&db=curr&fld=curr&id=$ref->{curr}&move=up&path=$form->{path}&login=$form->{login}&callback=$callback><img src=$images/up.png alt="+" border=0></a></td>|;
		$column_data{minus} =
qq|<td align=center><a href=$form->{script}?action=move&db=curr&fld=curr&id=$ref->{curr}&move=down&path=$form->{path}&login=$form->{login}&callback=$callback><img src=$images/down.png alt="-" border=0></a></td>|;
...
sub add_currency {
	...
	$form->{callback} =
"$form->{script}?action=add_currency&path=$form->{path}&login=$form->{login}"
	  unless $form->{callback};
