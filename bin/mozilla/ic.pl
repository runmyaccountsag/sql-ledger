...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&item=$form->{item}&path=$form->{path}&login=$form->{login}"
...
sub link_part {
	...
	if ( $form->{changeup} ) {
		$form->{readonly} = 1
		  if $myconfig{acs} =~ /Goods \& Services--Changeup/;
	}

	if ( $form->{item} eq 'part' ) {
		$form->{readonly} = 1
		  if $myconfig{acs} =~ /Goods \& Services--Add Part/;
		$form->error( $locale->text('Cannot create Part') . ";"
			  . $locale->text('Inventory account does not exist!') )
		  if !@{ $form->{IC_links}{IC} };
		$form->error( $locale->text('Cannot create Part') . ";"
			  . $locale->text('Income account does not exist!') )
		  if !@{ $form->{IC_links}{IC_sale} };
		$form->error( $locale->text('Cannot create Part') . ";"
			  . $locale->text('COGS account does not exist!') )
	...
	if ( $form->{item} eq 'service' ) {
		...
		$form->error( $locale->text('Cannot create Service') . ";"
			  . $locale->text('Income account does not exist!') )
		  if !@{ $form->{IC_links}{IC_income} };
		$form->error( $locale->text('Cannot create Service') . ";"
			  . $locale->text('Expense account does not exist!') )
	...
	if ( $form->{item} eq 'assembly' ) {
		...
		$form->error( $locale->text('Cannot create Assembly') . ";"
			  . $locale->text('Income account does not exist!') )
		  if !@{ $form->{IC_links}{IC_income} };
	}
	if ( $form->{item} eq 'labor' ) {
		...
		$form->error( $locale->text('Cannot create Labor') . ";"
			  . $locale->text('Inventory account does not exist!') )
		  if !@{ $form->{IC_links}{IC} };
		$form->error( $locale->text('Cannot create Labor') . ";"
			  . $locale->text('COGS account does not exist!') )
	...
	if ( @{ $form->{all_vendor} } ) {
		$form->{selectvendor} = "\n";
		for ( @{ $form->{all_vendor} } ) {
			$form->{selectvendor} .= qq|$_->{name}--$_->{id}\n|;
		}
	...
	if ( @{ $form->{all_customer} } ) {
		...
		for ( @{ $form->{all_customer} } ) {
			$form->{selectcustomer} .=
			  qq|$_->{name} ($_->{customernumber})--$_->{id}\n|;
		}
	...
	for (
		qw(currency partsgroup assemblypartsgroup vendor customer pricegroup IC_inventory IC_income IC_expense)
	  )
	{
...
qq|<textarea name="description" rows=$rows cols=40 wrap=soft>$form->{description}</textarea>|;
		...
		$description = qq|<input name=description size=40 value="|
	...
	for (qw(IC_inventory IC_income IC_expense)) {
		if ( $form->{$_} ) {
			if ( $form->{changeup} ) {
				$select{$_} =
				  $form->select_option( $form->{"select$_"}, $form->{$_} );
			}
			else {
				if ( $form->{orphaned} ) {
					$select{$_} =
	...
	if ( $form->{selectpartsgroup} ) {
		$selectpartsgroup = qq|<select name=partsgroup>|
...
|;
	...
	$form->hide_form(
		qw(id item title makemodel alternate onhand orphaned taxaccounts rowcount project_id precision changeup)
	);
...
|;
	...
	if ( !$form->{readonly} ) {
		...
		%button = (
			...
			'New Number' =>
			  { ndx => 15, key => 'M', value => $locale->text('New Number') },
		);
...
|;
	...
	$l_avgcost =
	  ...
	  . $locale->text('Average Cost');
	$l_markup = qq|<input name=l_markup class=checkbox type=checkbox value=Y> |
	  . $locale->text('Markup');
	$l_account =
	...
	$fromto = qq|
	  <td>
		...
		<input name=transdateto size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	      ...
	      <tr>
		<td nowrap><input name=method class=radio type=radio value=accrual $method{accrual}>|
...
|;
	...
	$l_name = qq|<input name=l_name class=checkbox type=checkbox value=Y> |
	  . $locale->text('Name');
	$l_vcnumber =
	  qq|<input name=l_vcnumber class=checkbox type=checkbox value=Y> |
	  . $locale->text('Customer Number');
	$l_curr = qq|<input name=l_curr class=checkbox type=checkbox value=Y> |
...
|;
		...
		$l_countryorigin =
		  qq|<input name=l_countryorigin class=checkbox type=checkbox value=Y> |
		  . $locale->text('Country of Origin');
		$l_tariff_hscode =
		  qq|<input name=l_tariff_hscode class=checkbox type=checkbox value=Y> |
		  . $locale->text('HS Code');
		...
		if ( @{ $form->{all_warehouse} } ) {
			...
			$selectwarehouse =
qq|<option value="$myconfig{warehouse}--$myconfig{warehouse_id}">$myconfig{warehouse}\n|
...
|;
		...
		$l_toolnumber =
qq|<input name=l_toolnumber class=checkbox type=checkbox value=Y>&nbsp;|
		  . $locale->text('Tool Number');

		$l_barcode =
		  qq|<input name=l_barcode class=checkbox type=checkbox value=Y>&nbsp;|
		  . $locale->text('Barcode');
...
|;
		$bom = qq|<input name=itemstatus type=radio value=bom>&nbsp;|
		  . $locale->text('BOM');
...
qq|<input name=l_description class=checkbox type=checkbox value=Y checked>&nbsp;|
	...
	push @a,
	  qq|<input name=l_business class=checkbox type=checkbox value=Y>&nbsp;|
	...
	if ( $form->{changeup} ) {
		$form->{title} =
		  $locale->text( 'Changeup' . ' ' . $title{ $form->{searchitems} } );
	}
	else {
  ...
  <tr valign=top>
	...
	$toplevel
        <tr>
          <td></td>
          <td colspan=3>
          	<input name=itemstatus class=radio type=radio value=all checked>&nbsp;|
	  . $locale->text('All') . qq|
...
|;
	...
	print qq|
	  ...
	  . qq|</td>
                <td><input name=l_csv class=checkbox type=checkbox value=Y>&nbsp;|
	  . $locale->text('CSV')
	  . qq|</td>
	      </tr>
            </table>
...
sub generate_report {
	...
	for (
		qw(path login searchitems changeup itemstatus individual bom l_linetotal method)
	  )
	{
	...
	if ( $form->{toolnumber} ) {
		...
		$option .=
		  $locale->text('Tool Number') . qq| : $form->{toolnumber}<br>|;
	}
	if ( $form->{microfiche} ) {
	...
	if ( $form->{l_linetotal} ) {
		...
		for (qw(sellprice lastcost avgcost listprice)) {
			$form->{"l_linetotal$_"} = "Y" if $form->{"l_$_"};
		}
	}
...
qq|<th nowrap colspan=$colspan><a class=listheading href=$href&sort=partnumber>|
	...
	$column_data{vcnumber} =
	    qq|<th nowrap><a class=listheading href=$href&sort=vcnumber>|
	  . $locale->text('Customer Number')
	  . qq|</a></th>|;
	$column_data{business} =
	    qq|<th nowrap><a class=listheading href=$href&sort=business>|
	  . $locale->text('Type of Business')
	  . qq|</a></th>|;
	...
	$column_data{drawing} =
	  ...
	  . $locale->text('Drawing')
	  . qq|</a></th>|;
	$column_data{toolnumber} =
	    qq|<th nowrap><a class=listheading href=$href&sort=toolnumber>|
	  . $locale->text('Tool Number')
	  . qq|</a></th>|;
	...
	$column_data{countryorigin} =
	    qq|<th nowrap><a class=listheading href=$href&sort=countryorigin>|
	  . $locale->text('CO')
	  . qq|</a></th>|;
	$column_data{tariff_hscode} =
	    qq|<th nowrap><a class=listheading href=$href&sort=tariff_hscode>|
	  . $locale->text('HS Code')
	  . qq|</a></th>|;

	$column_data{barcode} =
	    qq|<th nowrap><a class=listheading href=$href&sort=barcode>|
	  . $locale->text('Barcode')
	  . qq|</a></th>|;
...
|;
  ...
  |;
	...
	if ( $form->{summary} ) {
		@groupby = ();
		for (
			qw(partnumber description notes partsgroup make model bin curr priceupdate image drawing toolnumber barcode microfiche invnumber transdate ordnumber quonumber name employee serialnumber warehouse countryorigin tariff_hscode)
		  )
		{
			$a{$_} = 1;
		}
	...
	foreach $ref ( @{ $form->{parts} } ) {
		...
		$ref->{sellprice} =
		  $form->round_amount(
			$ref->{sellprice} - ( $ref->{sellprice} * $ref->{discount} ),
			$form->{precision} );
		if ( $form->{summary} ) {
		...
		if ( !$form->{summary} ) {
			for (qw(sellprice listprice lastcost avgcost)) {
				$ref->{$_} =
				  $form->round_amount( $ref->{$_}, $form->{precision} );
			}
		}
		...
		$column_data{partnumber} =
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&changeup=$form->{changeup}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}&nbsp;</a></td>";
		...
		$column_data{assemblypartnumber} =
"<td><a href=$form->{script}?action=edit&id=$ref->{assembly_id}&changeup=$form->{changeup}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{assemblypartnumber}&nbsp;</a></td>";
...
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}&nbsp;</a></td>";
		...
		$column_data{onhand} =
		    "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{onhand}, undef, "&nbsp;" )
		  . "</td>";
		$column_data{perassembly} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{perassembly}, undef,
			"&nbsp;" )
		  . "</td>";
		...
		if ( $form->{summary} && $form->{l_linetotal} ) {
			...
			$column_data{sellprice} = "<td align=right>"
		...
		$column_data{invnumber} =
		  ( $ref->{module} ne 'oe' )
		  ? "<td><a href=$ref->{module}.pl?action=edit&type=invoice&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>"
		  : "<td>$ref->{invnumber}&nbsp;</td>";
		$column_data{ordnumber} =
		  ( $ref->{module} eq 'oe' )
		  ? "<td><a href=$ref->{module}.pl?action=edit&type=$ref->{type}&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{ordnumber}&nbsp;</a></td>"
		  : "<td>$ref->{ordnumber}&nbsp;</td>";
		$column_data{quonumber} =
		  ( $ref->{module} eq 'oe' && !$ref->{ordnumber} )
		  ? "<td><a href=$ref->{module}.pl?action=edit&type=$ref->{type}&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{quonumber}&nbsp;</a></td>"
		  : "<td>$ref->{quonumber}&nbsp;</td>";
		...
		$column_data{transdate} =
		  "<td align=right>$ref->{transdate}&nbsp;</td>";
		if ( $ref->{vc_id} ) {
			$column_data{name} =
qq|<td><a href=ct.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{vc_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
		}
		...
		for (
			qw(make model serialnumber warehouse inventory income expense tax toolnumber countryorigin tariff_hscode barcode)
		  )
		{
			$column_data{$_} = "<td>$ref->{$_}&nbsp;</td>";
		}
...
|;
	...
	if ( $form->{"l_linetotal"} ) {
		...
		$column_data{linetotalsellprice} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totalsellprice,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
		$column_data{linetotallastcost} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totallastcost,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
		$column_data{linetotalavgcost} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totalavgcost, $form->{precision},
			"&nbsp;" )
		  . "</th>";
		$column_data{linetotallistprice} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $totallistprice,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
...
sub parts_subtotal {
	...
	$column_data{onhand} =
	  ...
	  . $form->format_amount( \%myconfig, $subtotalonhand, undef, "&nbsp;" )
	  . "</th>";

	$column_data{linetotalsellprice} = "<th class=listsubtotal align=right>"
	  . $form->format_amount( \%myconfig, $subtotalsellprice,
		$form->{precision}, "&nbsp;" )
	  . "</th>";
	$column_data{linetotallistprice} = "<th class=listsubtotal align=right>"
	  . $form->format_amount( \%myconfig, $subtotallistprice,
		$form->{precision}, "&nbsp;" )
	  . "</th>";
	$column_data{linetotallastcost} = "<th class=listsubtotal align=right>"
	  . $form->format_amount( \%myconfig, $subtotallastcost, $form->{precision},
		"&nbsp;" )
	  . "</th>";
	$column_data{linetotalavgcost} = "<th class=listsubtotal align=right>"
	  . $form->format_amount( \%myconfig, $subtotalavgcost, $form->{precision},
		"&nbsp;" )
	  . "</th>";
...
sub history {
  ...
  |;
	...
	@columns =
	  qw(transtype transdate reference partnumber description qty sellprice);
	$column_data{transtype} = qq|<th class=listheading>&nbsp;</th>|;
	$column_data{transdate} =
	  qq|<th class=listheading>| . $locale->text('Date') . qq|</th>|;
	$column_data{reference} =
	  qq|<th class=listheading>| . $locale->text('Reference') . qq|</th>|;
	$column_data{partnumber} =
...
qq|<th nowrap colspan=$colspan><a class=listheading href=$href&sort=partnumber>|
	...
	$column_data{description} =
	  ...
	  . qq|</a></th>|;
	$column_data{onhand} =
	  qq|<th class=listheading nowrap>| . $locale->text('On Hand') . qq|</th>|;
	$column_data{so} =
  ...
  |;
	...
	foreach $ref ( @{ $form->{parts} } ) {
		...
		if ( $ref->{id} != $sameid ) {
			...
			$column_data{order} =
			    "<td align=right>"
			  . $form->format_amount( \%myconfig, $order, undef, "-" )
			  . "</td>";
			$j++;
			$j %= 2;
			print "<tr class=listrow$j>";
...
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}&nbsp;</a></td>";
		...
		$column_data{so} =
		    "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{so}, undef, "&nbsp;" )
		  . "</td>";
		$column_data{po} =
		    "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{po}, undef, "&nbsp;" )
		  . "</td>";
		...
		$column_data{year} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $qty, undef, "&nbsp;" ) . "</td>";
	...
	if ( @{ $form->{parts} } ) {
		...
		$order = 0 if $order < 0;
		$column_data{order} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $order, undef, "-" ) . "</td>";
		$j++;
		$j %= 2;
		print "<tr class=listrow$j>";
...
<table width="100%">
  ...
  <tr valign=top>
	...
	<tr>
	  ...
	  <td colspan=3><input name=reqdatefrom size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"> <b>|
	  . $locale->text('To')
	  . qq|</b> <input name=reqdateto size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	</tr>
	  $selectfrom
	<tr>
	  <td></td>
...
sub so_requirements_report {
	...
	$form->isvaldate( \%myconfig, $form->{reqdatefrom},
		$locale->text('Invalid from date ...') );
	$form->isvaldate( \%myconfig, $form->{reqdateto},
		$locale->text('Invalid to date ...') );
	...
	for (qw(searchitems vc direction oldsort path login)) {
		$href .= qq|&$_=$form->{$_}|;
	}
	...
	if ( $form->{reqdatefrom} ) {
		...
		$option .=
		    $locale->text('From') 
		  . "&nbsp;"
		  . $locale->date( \%myconfig, $form->{reqdatefrom}, 1 );
	}
	if ( $form->{reqdateto} ) {
		...
		$option .=
		    $locale->text('To') 
		  . "&nbsp;"
		  . $locale->date( \%myconfig, $form->{reqdateto}, 1 );
	}

	@column_index = $form->sort_columns(
		qw(reqdate id ordnumber name customernumber partnumber description qty)
	);
	...
	$column_data{reqdate} =
	    "<th><a class=listheading href=$href&sort=reqdate>"
...
|;
	...
	foreach $ref ( @{ $form->{all_parts} } ) {
		...
		$column_data{qty} =
		    "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{qty}, undef, "&nbsp;" )
		  . "</td>";

		$column_data{ordnumber} =
"<td><a href=oe.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=sales_order&callback=$callback>$ref->{ordnumber}&nbsp;</a></td>";
		...
		$column_data{partnumber} =
qq|<td><a href=ic.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{parts_id}&callback=$callback>$ref->{partnumber}</a></td>|;
...
|;
	...
	for $i ( 1 .. $numrows ) {
		print qq|
	<tr>
	  <td><input name="make_$i" size=30 value="|
		  . $form->quote( $form->{"make_$i"} )
		  . qq|"></td>
	  <td><input name="model_$i" size=30 value="|
		  . $form->quote( $form->{"model_$i"} )
		  . qq|"></td>
	</tr>
|;
	}
	...
	for $i ( 1 .. $numrows ) {
	  ...
	  <td><select name="vendorcurr_$i">|
		...
		if ( $i == $numrows ) {
			...
			$vendor = qq|
          <td><input name="vendor_$i" size=35 value="|
			  . $form->quote( $form->{"vendor_$i"} )
			  . qq|"></td>
|;

			if ( $form->{selectvendor} ) {
				$vendor = qq|
	  <td width=99%><select name="vendor_$i">|
				  . $form->select_option( $form->{selectvendor}, undef, 1 )
				  . qq|</select></td>
|;
			}
...
|;
		...
		$form->{decimalplacescost} =
		  $form->get_precision( \%myconfig, $form->{"vendorcurr_$i"} );

		print qq|
	<tr>
	  $vendor
	  <td><input name="partnumber_$i" size=20 value="|
		  . $form->quote( $form->{"partnumber_$i"} )
		  . qq|"></td>
	  <td><input name="lastcost_$i" size=11 value=|
		  . $form->format_amount(
			\%myconfig,
			$form->{"lastcost_$i"},
			$form->{decimalplacescost}
...
|;
	...
	for $i ( 1 .. $numrows ) {
	  ...
	  <td><select name="customercurr_$i">|
		...
		if ( $i == $numrows ) {
			$customer = qq|
          <td><input name="customer_$i" size=35 value="|
			  . $form->quote( $form->{"customer_$i"} )
			  . qq|"></td>
	  |;

			if ( $form->{selectcustomer} ) {
				$customer = qq|
	  <td><select name="customer_$i">|
				  . $form->select_option( $form->{selectcustomer}, undef, 1 )
				  . qq|</select></td>
|;
			}
			...
			if ( $form->{selectpricegroup} ) {
				$pricegroup = qq|
	  <td><select name="pricegroup_$i">|
				  . $form->select_option( $form->{selectpricegroup}, undef, 1 )
				  . qq|</select></td>
|;
			}
	  ...
	  <td>$pricegroup</td>| . $form->hide_form("pricegroup_$i");
		...
		# bp 2010/02/12 use decimalplaces for the selected currency
		$form->{decimalplacessell} =
		  $form->get_precision( \%myconfig, $form->{"customercurr_$i"} );
	...
	<tr>
	  ...
	  <td><input name="pricebreak_$i" size=5 value=|
		  ...
		  . qq|></td>
	  <td><input name="customerprice_$i" size=11 value=|
		  . $form->format_amount(
			\%myconfig,
...
|;
	...
	for $i ( 1 .. $numrows ) {
		...
		$linetotallistprice =
		  $form->round_amount( $form->{"listprice_$i"} * $form->{"qty_$i"},
			$form->{decimalplacessell} );
		$form->{listprice} += $linetotallistprice;

		$linetotallastcost =
		  $form->round_amount( $form->{"lastcost_$i"} * $form->{"qty_$i"},
			$form->{decimalplacescost} );
...
qq|<td><input name="qty_$i" size=6 value="$form->{"qty_$i"}" accesskey="$i" title="[Alt-$i]"></td>|;
			$column_data{partnumber} =
			  qq|<td><input name="partnumber_$i" size=15></td>|;
			$column_data{description} =
			  qq|<td><input name="description_$i" size=30></td>|;
...
|;
		$form->hide_form( map { "${_}_$i" }
			  qw(id sellprice listprice lastcost weight assembly) );

	}

	for (@column_index) { $column_data{$_} = "<td>&nbsp;</td>" }

	$column_data{sellprice} = "<th align=right>"
	  . $form->format_amount( \%myconfig, $form->{sellprice},
		$form->{decimalplacessell} )
	  . "</th>";
	$column_data{listprice} = "<th align=right>"
	  . $form->format_amount( \%myconfig, $form->{listprice},
		$form->{decimalplacessell} )
	  . "</th>";
	$column_data{lastcost} = "<th align=right>"
	  . $form->format_amount( \%myconfig, $form->{lastcost},
		$form->{decimalplacescost} )
	  . "</th>";
...
sub select_name {
	...
	$column_data{"${table}number"} =
	  qq|<th class=listheading>| . $locale->text($labelnumber) . qq|</th>|;
	$column_data{address} =
...
|;
	...
	@column_index =
	  ( ndx, name, "${table}number", address, city, state, zipcode, country );
...
# $locale->text('Inventory quantity must be zero before you can set this assembly obsolete!')
	...
	$form->isvaldate( \%myconfig, $form->{priceupdate},
	...
	if ( $form->{previousform} && !$form->{callback} ) {
		...
		if ( $form->{item} eq 'assembly' ) {
			...
			foreach $item (qw(listprice sellprice lastcost)) {
				...
				$form->{$item} =
				  $form->round_amount( $form->{$item}, $form->{precision} );
			}
			...
			if ( $form->{vendor_id} ) {
				...
				for ( $j = 1 ; $j <= $newform{vendor_rows} ; $j++ ) {
					...
					if (   $newform{"vendor_$j"}
						&& $newform{"vendor_$j"} eq $form->{oldvendor} )
					{
						if ( $newform{"partnumber_$j"} ) {
							$form->{"partnumber_$i"} =
							  $newform{"partnumber_$j"};
			...
			if ( !$form->{taxincluded} ) {
				for ( split / /, $form->{"taxaccounts_$i"} ) {
					$amount += ( $form->{"${_}_base"} * $form->{"${_}_rate"} );
				}
			}
...
sub list_assemblies {
	...
	$callback =
"$form->{script}?action=list_assemblies&direction=$form->{direction}&oldsort=$form->{oldsort}&path=$form->{path}&login=$form->{login}&checkinventory=$form->{checkinventory}";
...
|;
	...
	foreach $ref ( @{ $form->{assembly_items} } ) {
		...
		$column_data{partnumber} =
"<td width=20%><a href=$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}&nbsp;</a></td>";
		...
		$column_data{rop} =
		    qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $ref->{rop}, undef, "&nbsp;" )
		  . qq|</td>|;
		$column_data{stock} =
		    qq|<td width=10%><input name="qty_$i" size=8 value=|
...
sub restock_assemblies {
	...
	$form->isblank( 'warehouse', $locale->text('Warehouse missing') )
	  if $form->{selectwarehouse};
