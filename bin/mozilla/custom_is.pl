...
sub repost_cogs {
	...
	if ( $form->{build_invoicetax} ) {
		...
		$query =
		  qq|INSERT INTO invoicetax (trans_id, invoice_id, chart_id, taxamount)
		   VALUES (?, ?, ?, ?)|;
	...
	$query =
qq|UPDATE invoice SET lastcost = sellprice WHERE trans_id IN (SELECT id FROM ap)|;
	$dbh->do($query) || $form->dberror($query);
   ...
   |;
	...
	$query =
qq|UPDATE inventory SET cogs = 0 WHERE trans_id NOT IN (SELECT id FROM trf)|;
	$dbh->do($query) || $form->dberror($query);
	...
	$query =
qq|UPDATE invoice SET allocated = allocated + ?, cogs = cogs + ? WHERE id = ?|;
	$invoiceupdate = $dbh->prepare($query) || $form->dberror($query);
	$query =
	  qq|UPDATE inventory SET cogs = cogs + ?, cost = ? WHERE invoice_id = ?|;
	...
	while ( $partsref = $sth->fetchrow_hashref(NAME_lc) ) {
		print
"--- Processing $partsref->{partnumber}--$partsref->{description}<br>\n";
		$apsth->execute( $partsref->{id} );
		while ( $apref = $apsth->fetchrow_hashref(NAME_lc) ) {
			$qty2allocate = 0 -
			  $apref->{qty};   # qty IN is always -ve so change sign for clarity
			                   # select unallocated sale invoice transactions
			$arsth->execute( $partsref->{id} );
			$inventoryupdate->execute( $apref->{sellprice} * $apref->{qty} * -1,
				$apref->{sellprice}, $apref->{id} );
			while ( $arref = $arsth->fetchrow_hashref(NAME_lc) ) {
				...
				if ( $qty2allocate != 0 ) {
					...
					$invoiceupdate->execute( $thisallocation, 0, $apref->{id} )
					  || $form->error('Error updating AP');
					$invoiceupdate->execute(
						0.00 - $thisallocation,
						$apref->{sellprice} * $thisallocation,
						$arref->{id}
					) || $form->error('Error updating AR');
					$inventoryupdate->execute(
						$apref->{sellprice} * $thisallocation * -1,
						$apref->{sellprice}, $arref->{id} );
					$fifoadd->execute(
						$arref->{trans_id},  "$arref->{transdate}",
						$partsref->{id},     $thisallocation,
						$apref->{sellprice}, $arref->{sellprice},
   ...
   |;
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		$cogssth->execute( $ref->{trans_id}, $ref->{inventory_accno_id},
			$ref->{amount}, $ref->{transdate}, 'cogs', $ref->{invoice_id} );
		$cogssth->execute(
			$ref->{trans_id},
			$ref->{expense_accno_id},
			0 - ( $ref->{amount} ),
			$ref->{transdate}, 'cogs', $ref->{invoice_id}
		);
	}
   ...
   |;
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		# Post income
		$cogssth->execute( $ref->{trans_id}, $ref->{income_accno_id},
			$ref->{sale}, $ref->{transdate}, "income", $ref->{id} );
		...
		if ( $ref->{inventory_accno_id} ) {
			...
			$cogssth->execute(
				...
				$ref->{cogs}, $ref->{transdate}, "cogs:$ref->{sale}:$ref->{id}",
				$ref->{id}
			);
			$cogssth->execute(
				$ref->{trans_id}, $ref->{expense_accno_id},
				0 - ( $ref->{cogs} ), $ref->{transdate},
				"cogs:$ref->{sale}:$ref->{id}", $ref->{id}
			);
		}

		$fifoadd->execute(
			$ref->{trans_id}, "$ref->{transdate}", $ref->{parts_id},
