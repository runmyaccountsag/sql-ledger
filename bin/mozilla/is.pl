...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&type=$form->{type}&login=$form->{login}&path=$form->{path}"
...
sub invoice_links {
	...
	for (
		qw(currency partsgroup projectnumber department warehouse employee language paymentmethod)
	  )
	{
...
"$form->{acc_trans}{$key}->[0]->{accno}--$form->{acc_trans}{$key}->[0]->{description}"
	...
	$form->{cd_available} =
	  $form->round_amount(
		$form->{netamount} * $form->{cashdiscount} / $form->{exchangerate},
		$form->{precision} );
	...
	if ( $form->{id} ) {
		%title = (
			invoice        => $locale->text('Edit Sales Invoice'),
		...
		%title = (
			invoice        => $locale->text('Add Sales Invoice'),
...
sub prepare_invoice {
	...
	if ( $form->{id} ) {
		...
		for (
			qw(invnumber ordnumber ponumber quonumber shippingpoint shipvia waybill notes intnotes)
		  )
		{
		...
		foreach $ref ( @{ $form->{invoice_details} } ) {
			...
			$form->{"sellprice_$i"} =
			  $form->format_amount( \%myconfig, $form->{"sellprice_$i"},
				$decimalplaces );
			$form->{"qty_$i"} =
			  $form->format_amount( \%myconfig, $form->{"qty_$i"} * $ml );
...
|;
	...
	$form->hide_form(
		qw(id type printed emailed queued title vc discount creditlimit creditremaining tradediscount business closedto locked shipped oldtransdate oldduedate recurring defaultcurrency oldterms cdt precision order_id remittancevoucher)
	);

	$form->hide_form( map { "select$_" }
		  ( "$form->{vc}", "AR", "AR_paid", "AR_discount" ) );
	$form->hide_form( map { "select$_" }
		  qw(formname currency partsgroup projectnumber department warehouse employee language paymentmethod)
	);
	$form->hide_form(
		"$form->{vc}_id", "old$form->{vc}",
		"quonumber",      "old$form->{vc}number"
	);
	...
	$terms = qq|
		...
		| . $locale->text('Net') . qq|
		<input name=terms size=3 value="$form->{terms}"> |
	  . $locale->text('days') . qq|
		</th>
	      </tr>
|;
	...
	if ( $form->{type} !~ /credit_/ ) {
		...
		<input name=cashdiscount size=3 value="|
			  . $form->format_amount( \%myconfig, $form->{cashdiscount} )
			  . qq|"> /  
...
<table width=100%>
  ...
  <tr>
	...
	<tr valign=top>
	  ...
	  . $form->select_option( $form->{selectAR}, $form->{AR} ) . qq|</select>
		</td>
	      </tr>
	      $exchangerate
	      $warehouse
...
|;
	...
	$form->hide_form(
		qw(city state country message email subject cc bcc taxaccounts dcn));
...
|;
	...
	for ( split / /, $form->{taxaccounts} ) {
		...
		if ( !$form->{taxincluded} ) {
			...
			if ( $form->{"${_}_base"} ) {
				...
				if ( $form->{discount_paid} && $form->{cdt} ) {
					$cdtp = $form->{discount_paid} / $form->{invsubtotal}
					  if $form->{invsubtotal};
					$form->{"${_}_total"} -=
					  $form->round_amount( $form->{"${_}_total"} * $cdtp,
						$form->{precision} );
				}
		...
		<td id=total_tax_${_} align=right>|
...
|;
	...
	if ( $form->{discount_paid} ) {
		...
		<td align=right>|
		  . $form->format_amount( \%myconfig, $form->{discount_paid} * -1,
			$form->{precision}, 0 )
		  . qq|</td>
	      </tr>
|;
...
qq|<input name="discount_exchangerate" size=10 value=$form->{discount_exchangerate}>|
		...
		$column_data{paid} =
		  ...
		  . qq|></td>|;
		$column_data{AR_paid} =
		  qq|<td align=center><select name="AR_discount_paid">|
		  . $form->select_option( $form->{"selectAR_discount"},
			$form->{"AR_discount_paid"} )
		  . qq|</select></td>|;
		$column_data{datepaid} =
qq|<td align=center><input name="discount_datepaid" size=11 class=date value=$form->{"discount_datepaid"}></td>|;
...
qq|<td align=center><input name="paid_$i" size=11 value=$form->{"paid_$i"}></td>|;
		$column_data{exchangerate} = qq|<td align=center>$exchangerate</td>|;
		$column_data{AR_paid} =
		  qq|<td align=center><select name="AR_paid_$i">|
		  . $form->select_option( $form->{selectAR_paid},
			$form->{"AR_paid_$i"} )
		  . qq|</select></td>|;
		$column_data{datepaid} =
...
|;
	...
	$form->hide_form(
		qw(paidaccounts oldinvtotal oldtotalpaid payment_accno payment_method));
...
|;
	...
	if ( $form->{readonly} ) {
		...
		%button = (
			...
			'New Number' =>
			  { ndx => 13, key => 'M', value => $locale->text('New Number') },
			'Delete' =>
			  { ndx => 14, key => 'D', value => $locale->text('Delete') },
		);

		if ( $form->{id} ) {

			delete $button{'Sales Order'}
			  if $myconfig{acs} =~
				  /(Order Entry--Order Entry|Order Entry--Sales Order)/;
			...
			if ( $transdate > $form->{closedto} ) {
				...
				for (
					"Update", "New Number", "Ship to", "Print",
					"E-mail", "Post",       "Schedule"
				  )
				{
					$a{$_} = 1;
				}
...
sub update {
	...
	for (qw(exchangerate cashdiscount discount_paid)) {
	...
	if ( $form->{discount_paid} ) {
			...
			|| $form->{currency} ne $form->{oldcurrency} )
		{
			$form->{discount_exchangerate} =
			  $form->check_exchangerate( \%myconfig, $form->{currency},
				$form->{discount_datepaid}, 'buy' );
		}

		$expired =
		  $form->add_date( \%myconfig, $form->{transdate},
			$form->{discountterms}, 'days' );
		if ( $form->datetonum( \%myconfig, $form->{discount_datepaid} ) >
			$form->datetonum( \%myconfig, $expired ) )
		{
	...
	for $i ( 1 .. $form->{paidaccounts} ) {
		if ( $form->{"paid_$i"} ) {
			for (
				qw(olddatepaid datepaid source memo cleared vr_id paymentmethod)
			  )
			{
			...
			if ( $j++ != $i ) {
				for (
					qw(olddatepaid datepaid source memo cleared paid exchangerate vr_id paymentmethod)
				  )
				{
					delete $form->{"${_}_$i"};
				}
			...
			for (
				qw(olddatepaid datepaid source memo cleared paid exchangerate vr_id)
			  )
			{
				delete $form->{"${_}_$i"};
			}
	...
	$form->{payment_accno} =
	  $form->escape( $form->{"AR_paid_$form->{paidaccounts}"}, 1 );
	$form->{payment_method} =
...
sub post {
	...
	if ( $form->{discount_paid} ) {
		...
		for (
			qw(paid datepaid source memo exchangerate cleared vr_id paymentmethod)
		  )
		{
		...
		if ( $form->{"paid_$i"} ) {
			...
			$form->error( $locale->text('Cash discount date past due!') )
			  if ( $datepaid > $expired );
