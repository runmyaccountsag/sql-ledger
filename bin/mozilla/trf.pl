...
sub form_header {
	...
	$form->{selectexpense_accno} =
	  $form->unescape( $form->{selectexpense_accno} );
	$form->{selectfrom_warehouse} =
	  $form->unescape( $form->{selectfrom_warehouse} );
	$form->{selectto_warehouse} =
	  $form->unescape( $form->{selectto_warehouse} );
	$form->{selectpartsgroup} = $form->unescape( $form->{selectpartsgroup} );

	if ( $form->{department_id} ) {
		$form->{"selectdepartment"} =~ s/ selected//;
		$form->{"selectdepartment"} =~
		  s/(\Q--$form->{department_id}"\E)/$1 selected/;
	}

	if ( $form->{from_warehouse_id} ) {
		$form->{"selectfrom_warehouse"} =~ s/ selected//;
		$form->{"selectfrom_warehouse"} =~
		  s/(\Q--$form->{from_warehouse_id}"\E)/$1 selected/;
	}

	if ( $form->{to_warehouse_id} ) {
		$form->{"selectto_warehouse"} =~ s/ selected//;
		$form->{"selectto_warehouse"} =~
		  s/(\Q--$form->{to_warehouse_id}"\E)/$1 selected/;
	}
	...
	&print_text( 'trfnumber', $locale->text('Transfer Number'),
		10, "$form->{trfnumber}" );
	...
	for $i ( 1 .. $form->{rowcount} ) {
			...
			and ( $i != $form->{rowcount} ) )
		{
			...
			if ($itemdetailok) {
				$itemdetail =
qq|<td><a href="ic.pl?login=$form->{login}&path=$form->{path}&action=edit&id=$form->{"parts_id_$i"}" target="_blank">?</a></td>|;
			}
			$total += $form->{"cost_$i"} * $form->{"qty_$i"};
			print qq|<tr>|;
			print qq|<td><input name="no_$j" type=text size=3 value=$j></td>\n|;
			print
qq|<input type=hidden name="parts_id_$j" value="$form->{"parts_id_$i"}"></td>\n|;
			print
qq|<input type=hidden name="weight_$j" value="$form->{"weight_$i"}"></td>\n|;
			print
qq|<td><input name="partnumber_$j" type=text size=15 value="$form->{"partnumber_$i"}"></td>\n|;
			print $itemdetail;
			print
qq|<td><input name="description_$j" type=text size=48 value="$form->{"description_$i"}"></td>\n|;
			print
qq|<td><input name="qty_$j" type=text size=5 value="$form->{"qty_$i"}"></td>\n|;
			print
qq|<td><input name="unit_$j" type=text size=5 value="$form->{"unit_$i"}"></td>\n|;
			print
qq|<td><input name="cost_$j" type=text size=5 value="$form->{"cost_$i"}"></td>\n|;
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig,
				$form->{"cost_$i"} * $form->{"qty_$i"}, 2 )
			  . qq|</td>\n|;
			print qq|</tr>\n|;

			print qq|<tr>|;
			print qq|<td colspan=3>|;
			print qq|<select name=partsgroup_$j>|
			  . $form->select_option( $form->{selectpartsgroup}, '', 1 )
			  . qq|</select>|
			  if !$form->{"partnumber_$i"};
			print qq|</td>|;

			print
qq|<td><input name="itemnotes_$j" type=text size=48 value='$form->{"itemnotes_$i"}' title='Notes'</td>\n|;
			print
qq|<td colspan=4><input name="serialnumber_$j" value='$form->{"serialnumber_$i"}' title='Serial No.'</td>\n|;
			print qq|</tr>\n|;
	...
	print qq|<tr><td colspan=6></td><th>Total</th><th align=right>|
	  . $form->format_amount( \%myconfig, $total, 2 )
	  . qq|</th></tr>|;
	&end_table;
	$j--;
	$form->{rowcount} = $j;
	...
	$form->{selectfrom_warehouse} =
	  $form->escape( $form->{selectfrom_warehouse}, 1 );
	$form->{selectto_warehouse} =
	  $form->escape( $form->{selectto_warehouse}, 1 );
	...
	for (qw(html postscript pdf)) {
		$selected = '';
		$selected = 'selected' if $form->{format} eq $_;
...
<table width="100%">
    ...
    <tbody><tr>
	  ...
	  . $form->select_option( $form->{selectformname},
		$form->{formname}, undef, 1 )
	  . qq|
</option></select></td>
      <td></td>
...
|;
	...
	$form->hide_form(
		qw(vc rowcount selectdepartment selectfrom_warehouse selectto_warehouse selectpartsgroup selectformname)
	);
...
sub add {
	...
	$form->{callback} =
qq|$form->{script}?action=add&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}|;
...
sub update {
	...
	if (   ( $form->{"partnumber_$i"} ne "" )
		or ( $form->{"description_$i"} ne "" )
		or ( $form->{"partsgroup_$i"} ) )
	{
...
sub delete {
	...
	$transdate = $form->datetonum( \%myconfig, $form->{transdate} );
	$form->error( $locale->text('Cannot delete transfer for a closed period!') )
	  if ( $transdate <= $form->{closedto} );
...
<h4>|
	  . $locale->text('Are you sure you want to delete Transfer Number')
	  . qq| $form->{trfnumber}
</h4>
...
#-------------------------------
sub save {
	$form->isblank( 'trfnumber',
		$locale->text('Transfer Number cannot be blank') );
	$form->isblank( 'department', $locale->text('Department cannot be blank') );
	$form->isblank( 'from_warehouse',
		$locale->text('From warehouse cannot be blank') );
	$form->isblank( 'to_warehouse',
		$locale->text('To warehouse cannot be blank') );
	$form->isblank( 'transdate',
		$locale->text('Transfer date cannot be blank') )
	  if $form->{closed} eq 'Y';

	$transdate = $form->datetonum( \%myconfig, $form->{transdate} );
	$form->error( $locale->text('Cannot post transfer for a closed period!') )
	  if ( $transdate <= $form->{closedto} );
	...
	my ( $null, $employee_id ) =
	  $form->get_employee($dbh);    # Get employee_id of current login
...
|;
	...
	push @a, qq|<input name="l_id" class=checkbox type=checkbox value=Y> |
	  . $locale->text('ID');
	push @a,
qq|<input name="l_trfnumber" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Number');
	push @a,
...
qq|<input name="l_description" class=checkbox type=checkbox value=Y checked> |
	...
	push @a,
	  qq|<input name="l_notes" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Notes');
	push @a,
	  qq|<input name="l_department" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Department');
	push @a,
qq|<input name="l_from_warehouse" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('From WH');
	push @a,
qq|<input name="l_to_warehouse" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('To WH');
	push @a,
qq|<input name="l_partnumber" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Number');
	push @a,
	  qq|<input name="l_partdescription" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Description');
	push @a,
	  qq|<input name="l_itemnotes" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Item Notes');
	push @a,
	...
	push @a,
	  ...
	  . $locale->text('Qty');
	push @a,
	  qq|<input name="l_cost" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Cost');
	push @a, qq|<input name="l_extended" class=checkbox type=checkbox value=Y> |
...
sub list {
	...
	@columns =
	  qw(id transdate trfnumber description notes department from_warehouse to_warehouse);
	my $where = qq| (1 = 1)|;
	$where .= qq| AND (LOWER(trfnumber) LIKE '$trfnumber')|
	  if $form->{trfnumber};
	$where .= qq| AND (trf.department_id = $form->{department_id})|
	  if $form->{department};
	$where .= qq| AND (trf.from_warehouse_id = $form->{from_warehouse_id})|
	  if $form->{from_warehouse};
	$where .= qq| AND (trf.to_warehouse_id = $form->{to_warehouse_id})|
	  if $form->{to_warehouse};
	$where .= qq| AND (trf.transdate >= '$form->{fromdate}')|
	  if $form->{fromdate};
	$where .= qq| AND (trf.transdate <= '$form->{todate}')| if $form->{todate};

	if ( !$form->{summary} ) {
		@columns =
		  qw(id transdate trfnumber description notes department from_warehouse to_warehouse partnumber partdescription itemnotes serialnumber qty cost extended);
		$where .= qq| AND (LOWER(partnumber) LIKE '$partnumber')|
		  if $form->{partnumber};
		$where .= qq| AND (LOWER(partdescription) LIKE '$partdescription')|
		  if $form->{partdescription};
		$where .= qq| AND (LOWER(itemnotes) LIKE '$itemnotes')|
		  if $form->{itemnotes};
		$where .= qq| AND (LOWER(serialnumber) LIKE '$serialnumber')|
	...
	for (
		qw(l_subtotal summary transdate trfnumber description notes department from_warehouse to_warehouse partnumber partdescription itemnotes serialnumber)
	  )
	{
	...
	$column_header{trfnumber} =
	  rpt_hdr( 'trfnumber', $locale->text('Transfer Number'), $href );
	...
	$column_header{from_warehouse} =
	  rpt_hdr( 'from_warehouse', $locale->text('From WH'), $href );
	$column_header{to_warehouse} =
	  rpt_hdr( 'to_warehouse', $locale->text('To WH'), $href );
	$column_header{partnumber} =
	  rpt_hdr( 'partnumber', $locale->text('Number'), $href );
	$column_header{partdescription} =
	  rpt_hdr( 'partdescription', $locale->text('Part Description'), $href );
	$column_header{itemnotes} =
	  rpt_hdr( 'itemnotes', $locale->text('Item Notes'), $href );
	$column_header{serialnumber} =
	  rpt_hdr( 'serialnumber', $locale->text('Serial No.'), $href );
...
sub delivered_search {
	...
	print qq|<tr><th align=right>|
	  . $locale->text('To Warehouse')
	  . qq|</th><td>$myconfig{warehouse}</td></tr>|;

	print qq|<tr><th align=right>|
	  . $locale->text('Include in Report')
	  . qq|</th><td>|;

	&print_checkbox( 'undelivered', $locale->text('Un-delivered'),
		'checked', '' );
	...
	&print_checkbox( 'l_from_warehouse', $locale->text('From WH'),
		'checked', '<br>' );
...
sub delivered_form {
	...
	$where .= qq| AND (LOWER(trfnumber) LIKE '$trfnumber')|
	  if $form->{trfnumber};
	$where .= qq| AND (trf.to_warehouse_id = $myconfig{warehouse_id})|
	...
	if ( $form->{delivered} || $form->{undelivered} ) {
		...
		if ( $form->{delivered} ) {
			$where .= " AND delivereddate IS NOT NULL"
			  unless $form->{undelivered};
		}
	}

	@columns =
	  qw(id transdate trfnumber description notes department from_warehouse delivereddate);
	...
	$column_header{from_warehouse} =
	  rpt_hdr( 'from_warehouse', $locale->text('From WH'), $href );
	$column_header{delivereddate} =
	  rpt_hdr( 'delivereddate', $locale->text('Delivered Date') );
	...
	print qq|<tr><th align=left>|
	  . $locale->text('My Warehouse')
	  . qq|</th><td>|
	  . $myconfig{warehouse}
	  . qq|</td></tr>|
	  if $myconfig{warehouse};
	print qq|</table>|;
...
qq|$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
		...
		if ( $myconfig{role} eq 'user' ) {
			$column_data{delivereddate} =
qq|<input type=hidden name="delivereddate_$no" value=$ref->{delivereddate}>|;
			$column_data{delivereddate} .= qq|<td>$ref->{delivereddate}</td>|;
		}
		else {
			$column_data{delivereddate} =
			  rpt_print_date( "delivereddate_$no", $ref->{delivereddate} );
		}
...
sub save_delivered {
	...
	for $i ( 1 .. $form->{rowcount} ) {
		if ( $form->{"delivereddate_$i"} ) {
			if (
				$form->datediff(
					\%myconfig, $form->{"delivereddate_$i"},
					$form->{"transdate_$i"}
				) <= 0
			  )
			{
		...
		WHERE id = $form->{"id_$i"}|;
				$dbh->do($query) || $form->dberror($query);
				$form->info(
					qq|$form->{"id_$i"}--$form->{"delivereddate_$i"}\n|);
			}
			else {
