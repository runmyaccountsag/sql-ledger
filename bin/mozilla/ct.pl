...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&db=$form->{db}&typeofcontact=$form->{typeofcontact}&path=$form->{path}&login=$form->{login}"
...
sub create_links {
	...
	foreach $item (qw(arap discount payment)) {
		if ( @{ $form->{"${item}_accounts"} } ) {
			$form->{"select$item"} = "\n";
			for ( @{ $form->{"${item}_accounts"} } ) {
				$form->{"select$item"} .= qq|$_->{accno}--$_->{description}\n|;
			}
	...
	if ( @{ $form->{all_business} } ) {
		...
		for ( @{ $form->{all_business} } ) {
			$form->{selectbusiness} .= qq|$_->{description}--$_->{id}\n|;
		}
	...
	if ( $form->{selectbusiness} ) {
	  ...
	  <td><select name=business>|
		  . $form->select_option( $form->{selectbusiness}, $form->{business},
			1 )
		  . qq|</select>
	  </td>
|;
	...
	if ( @{ $form->{all_dispatch} } ) {
		...
		for ( @{ $form->{all_dispatch} } ) {
			$form->{selectdispatch} .= qq|$_->{description}--$_->{id}\n|;
		}
	...
	if ( @{ $form->{all_paymentmethod} } ) {
		...
		$form->{selectpaymentmethod} =
		  $form->escape( $form->{selectpaymentmethod}, 1 );
	}

	if ( @{ $form->{all_pricegroup} } && $form->{db} eq 'customer' ) {
		$form->{selectpricegroup} = qq|\n|;
		for ( @{ $form->{all_pricegroup} } ) {
			$form->{selectpricegroup} .= qq|$_->{pricegroup}--$_->{id}\n|;
		}
		$form->{selectpricegroup} =
		  $form->escape( $form->{selectpricegroup}, 1 );
	}
	...
	if ( @{ $form->{all_department} } ) {
		...
		$form->{department} = qq|$form->{department}--$form->{department_id}|
...
|;
	...
	<tr>
	  ...
	  . $locale->text('Currency') . qq|
	      ...
	      <tr>
		<td>
		<input name="l_transdate" type=checkbox class=checkbox value=Y checked>&nbsp;|
	  . $locale->text('Date') . qq|
		</td>
		<td>
		<input name="l_qty" type=checkbox class=checkbox value=Y checked>&nbsp;|
	  . $locale->text('Qty') . qq|
		</td>
		<td>
...
sub include_in_report {
	...
	push @a, qq|<input name="l_id" type=checkbox class=checkbox value=Y> |
	  . $locale->text('ID');
	push @a,
	  qq|<input name="l_typeofcontact" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Type');
	push @a,
qq|<input name="l_name" type=checkbox class=checkbox value=Y $form->{l_name}> $vcname|;
	push @a,
qq|<input name="l_$form->{db}number" type=checkbox class=checkbox value=Y> $vcnumber|;
	push @a, qq|<input name="l_address" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Address');
	push @a,
qq|<input name="l_salutation" type=checkbox class=checkbox value=Y $form->{l_salutation}> |
	  . $locale->text('Salutation');
	push @a,
qq|<input name="l_contact" type=checkbox class=checkbox value=Y $form->{l_contact}> |
	  . $locale->text('Contact');
	push @a,
qq|<input name="l_contacttitle" type=checkbox class=checkbox value=Y $form->{l_contacttitle}> |
	  . $locale->text('Titel');
	push @a,
qq|<input name="l_gender" type=checkbox class=checkbox value=Y $form->{l_gender}> |
	  . $locale->text('Gender');
	push @a,
qq|<input name="l_occupation" type=checkbox class=checkbox value=Y $form->{l_occupation}> |
	  . $locale->text('Occupation');
	push @a,
...
qq|<input name="l_phone" type=checkbox class=checkbox value=Y $form->{l_phone}> |
	  . $locale->text('Phone');
	push @a,
qq|<input name="l_mobile" type=checkbox class=checkbox value=Y $form->{l_mobile}> |
	  . $locale->text('Mobile');
	...
	push @a,
	  qq|<input name="l_threshold" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Threshold');
	push @a, qq|<input name="l_accounts" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Accounts');
	push @a,
	  qq|<input name="l_paymentmethod" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Payment Method');
	push @a,
	...
	push @a, qq|<input name="l_sic_code" type=checkbox class=checkbox value=Y> |
	  . $locale->text('SIC');
	push @a, qq|<input name="l_bankname" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Bank');
	push @a,
	  qq|<input name="l_bankaddress" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Address');
	...
	push @a, qq|<input name="l_dispatch" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Dispatch Method');
	push @a,
	  qq|<input name="l_creditlimit" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Credit Limit');
	...
	push @a, qq|<input name="l_curr" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Currency');
	push @a,
qq|<input name="l_remittancevoucher" type=checkbox class=checkbox value=Y> |
...
|;
	...
	$include .= qq|
        <tr>
	        <td><input name="l_csv" type=checkbox class=checkbox value=Y>&nbsp;|
	  . $locale->text('CSV')
	  . qq|</td>
        </tr>
	    </table>
...
|;
	...
	$department = qq|
	  ...
	  . $form->select_option( $form->{selectdepartment}, $form->{department},
		1 )
	  . qq|</select>
		</td>
	      </tr>
...
<table width=100%>
  ...
  <tr valign=top>
	...
	<tr valign=top>
	  ...
	  . $locale->text('To')
	  . qq| <input name=startdateto size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	      </tr>
	    </table>
	  </td>
	</tr>
...
sub list_names {
	...
	$form->isvaldate( \%myconfig, $form->{startdateto},
	...
	$form->isvaldate( \%myconfig, $form->{transdateto},
		$locale->text('Invalid to date ...') );

	CT->search( \%myconfig, \%$form );

	$href = "$form->{script}?action=list_names";
	for (qw(direction oldsort db ARAP path login status l_subtotal)) {
		$href .= "&$_=$form->{$_}";
	}
	...
	push @columns,
	  ( contacttitle, gender, occupation, phone,
		mobile,   fax,      email,   cc,
		bcc,      employee, manager, notes,
		discount, terms,    creditlimit
	  );
	...
	push @columns,
	  ( paymentmethod, threshold, taxnumber, gifi_accno,
		sic_code, business, dispatch, pricegroup,
		language, curr,     bankname
	  );

	if ( $form->{l_bankaddress} ) {
		for ( bankaddress1, bankaddress2, bankcity,
			bankstate, bankzipcode, bankcountry
		  )
		{
			$form->{"l_$_"} = "Y";
			push @columns, $_;
	...
	push @columns,
	  (
		iban,     bic,       remittancevoucher, startdate,
		enddate,  invnumber, invamount,         invtax,
		invtotal, ordnumber, ordamount,         ordtax,
		ordtotal, quonumber, quoamount,         quotax,
		quototal
	  );
	...
	$column_header{typeofcontact} =
	  qq|<th class=listheading>| . $locale->text('Type') . qq|</th>|;
	$column_header{"$form->{db}number"} =
qq|<th><a class=listheading href=$href&sort=$form->{db}number>$vcnumber</a></th>|;
	$column_header{name} =
	  qq|<th><a class=listheading href=$href&sort=name>$vcname</a></th>|;
	...
	$column_header{country} =
	  ...
	  . $locale->text('Country')
	  . qq|</a></th>|;
	$column_header{salutation} =
	  qq|<th>| . $locale->text('Salutation') . qq|</th>|;
	$column_header{firstname} = qq|<th>&nbsp;</th>|;
	$column_header{lastname} =
	    qq|<th><a class=listheading href=$href&sort=contact>|
	  . $locale->text('Contact')
	  . qq|</a></th>|;
	$column_header{contacttitle} =
	  qq|<th>| . $locale->text('Title') . qq|</th>|;
	$column_header{gender} = qq|<th>| . $locale->text('Gender') . qq|</th>|;
	$column_header{occupation} =
	    qq|<th><a class=listheading href=$href&sort=occupation>|
	  . $locale->text('Occupation')
	  . qq|</a></th>|;
	...
	$column_header{cc} =
	  ...
	  . qq|</a></th>|;
	$column_header{bcc} =
	    qq|<th><a class=listheading href=$href&sort=bcc>|
	  . $locale->text('Bcc')
	  . qq|</a></th>|;
	$column_header{notes} =
	...
	$column_header{threshold} =
	  qq|<th class=listheading>| . $locale->text('Threshold') . qq|</th>|;
	$column_header{paymentmethod} =
	    qq|<th><a class=listheading href=$href&sort=paymentmethod>|
	  . $locale->text('Payment Method')
	  . qq|</a></th>|;
	$column_header{creditlimit} =
	  qq|<th class=listheading>| . $locale->text('Credit Limit') . qq|</th>|;

	# $locale->text('AR')
	# $locale->text('AP')

	$column_header{arap_accno} =
	  qq|<th class=listheading>| . $locale->text( $form->{ARAP} ) . qq|</th>|;
	$column_header{payment_accno} =
	  qq|<th class=listheading>| . $locale->text('Payment') . qq|</th>|;
	$column_header{discount_accno} =
	  qq|<th class=listheading>| . $locale->text('Discount') . qq|</th>|;
	$column_header{taxaccounts} =
	...
	$column_header{business} =
	  ...
	  . $locale->text('Type of Business')
	  . qq|</a></th>|;
	$column_header{dispatch} =
	    qq|<th><a class=listheading href=$href&sort=dispatch>|
	  . $locale->text('Dispatch Method')
	  . qq|</a></th>|;

	$column_header{bank} =
	  qq|<th class=listheading>| . $locale->text('Bank') . qq|</th>|;
	$column_header{bankname} =
	  qq|<th class=listheading>| . $locale->text('Bank') . qq|</th>|;
	$column_header{bankaddress1} =
	  qq|<th class=listheading>| . $locale->text('Address') . qq|</th>|;
	$column_header{bankaddress2} = qq|<th class=listheading>&nbsp;</th>|;
	$column_header{bankcity} =
	  qq|<th class=listheading>| . $locale->text('City') . qq|</th>|;
	$column_header{bankstate} =
	  qq|<th class=listheading>| . $locale->text('State/Province') . qq|</th>|;
	$column_header{bankzipcode} =
	  qq|<th class=listheading>| . $locale->text('Zip/Postal Code') . qq|</th>|;
	$column_header{bankcountry} =
	  qq|<th class=listheading>| . $locale->text('Country') . qq|</th>|;
	$column_header{iban} =
	  qq|<th class=listheading>| . $locale->text('IBAN') . qq|</th>|;
	$column_header{bic} =
	  qq|<th class=listheading>| . $locale->text('BIC') . qq|</th>|;
	$column_header{remittancevoucher} =
	  qq|<th class=listheading>| . $locale->text('RV') . qq|</th>|;
...
|;
	...
	foreach $ref ( @{ $form->{CT} } ) {
		...
		if ( $ref->{id} eq $sameid ) {
			...
			$column_data{name} =
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&db=$form->{db}&path=$form->{path}&login=$form->{login}&status=$form->{status}&callback=$callback>$ref->{name}&nbsp;</a></td>";
...
"<td><a href=$ref->{module}.pl?action=edit&id=$ref->{invid}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>";

			$column_data{invamount} = "<td align=right>"
			...
			$column_data{invtax} = "<td align=right>"
			  . $form->format_amount( \%myconfig,
				...
				$form->{precision}, "&nbsp;" )
			  . "</td>";
			$column_data{invtotal} = "<td align=right>"
...
"<td><a href=$ref->{module}.pl?action=edit&id=$ref->{invid}&type=$ordertype&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{ordnumber}&nbsp;</a></td>";

			$column_data{ordamount} = "<td align=right>"
			  . $form->format_amount( \%myconfig, $ref->{netamount},
				$form->{precision}, "&nbsp;" )
			  . "</td>";
			$column_data{ordtax} = "<td align=right>"
			  . $form->format_amount( \%myconfig,
				...
				$form->{precision}, "&nbsp;" )
			  . "</td>";
			$column_data{ordtotal} = "<td align=right>"
			  . $form->format_amount( \%myconfig, $ref->{amount},
				$form->{precision}, "&nbsp;" )
			  . "</td>";
...
"<td><a href=$ref->{module}.pl?action=edit&id=$ref->{invid}&type=$quotationtype&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{quonumber}&nbsp;</a></td>";

			$column_data{quoamount} = "<td align=right>"
			  . $form->format_amount( \%myconfig, $ref->{netamount},
				$form->{precision}, "&nbsp;" )
			  . "</td>";
			$column_data{quotax} = "<td align=right>"
			  . $form->format_amount( \%myconfig,
				...
				$form->{precision}, "&nbsp;" )
			  . "</td>";
			$column_data{quototal} = "<td align=right>"
			  . $form->format_amount( \%myconfig, $ref->{amount},
				$form->{precision}, "&nbsp;" )
			  . "</td>";
		...
		if ( $sameid ne "$ref->{id}" ) {
			...
			if ( $form->{l_terms} ) {
				@a = ();
				push @a,
				  $form->format_amount( \%myconfig, $ref->{cashdiscount} * 100,
					undef )
				  if $ref->{cashdiscount};
			...
			if ( $form->{l_threshold} ) {
				$column_data{threshold} = "<td align=right>"
				  . $form->format_amount( \%myconfig, $ref->{threshold}, 0,
					"&nbsp;" )
				  . "</td>";
			}
			if ( $form->{l_creditlimit} ) {
				$column_data{creditlimit} = "<td align=right>"
				  . $form->format_amount( \%myconfig, $ref->{creditlimit}, 0,
					"&nbsp;" )
				  . "</td>";
			}
			if ( $form->{l_remittancevoucher} ) {
				$column_data{remittancevoucher} =
				    "<td align=center>"
				  . ( ( $ref->{remittancevoucher} ) ? "*" : "&nbsp;" )
				  . "</td>";
			}
		}
...
|;
	...
	if ( $myconfig{acs} !~ /Customers--/ ) {
		if ( $form->{db} eq 'customer' ) {
			$button{'Customers--Add Customer'}{code} =
			  qq|<input class=submit type=submit name=action value="|
			  . $locale->text('Add Customer') . qq|"> |;
			$button{'Customers--Add Customer'}{order} = $i++;
		}
	}
	if ( $myconfig{acs} !~ /Vendors--/ ) {
		if ( $form->{db} eq 'vendor' ) {
			$button{'Vendors--Add Vendor'}{code} =
			  qq|<input class=submit type=submit name=action value="|
			  . $locale->text('Add Vendor') . qq|"> |;
...
sub list_subtotal {
	...
	$column_data{invamount} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $invamountsubtotal,
		$form->{precision}, "&nbsp;" )
	  . "</td>";
	$column_data{invtax} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $invtaxsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
	$column_data{invtotal} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $invtotalsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
	...
	$column_data{ordamount} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $ordamountsubtotal,
		$form->{precision}, "&nbsp;" )
	  . "</td>";
	$column_data{ordtax} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $ordtaxsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
	$column_data{ordtotal} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $ordtotalsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
	...
	$column_data{quoamount} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $quoamountsubtotal,
		$form->{precision}, "&nbsp;" )
	  . "</td>";
	$column_data{quotax} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $quotaxsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
	$column_data{quototal} = "<td align=right>"
	  . $form->format_amount( \%myconfig, $quototalsubtotal, $form->{precision},
		"&nbsp;" )
	  . "</td>";
...
sub list_history {
	...
	$href =
"$form->{script}?action=list_history&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&type=$form->{type}&transdatefrom=$form->{transdatefrom}&transdateto=$form->{transdateto}&history=$form->{history}";
...
"$form->{script}?action=list_history&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&type=$form->{type}&transdatefrom=$form->{transdatefrom}&transdateto=$form->{transdateto}&history=$form->{history}";
	...
	@columns = $form->sort_columns( partnumber, description, transdate, qty,
		unit, sellprice, fxsellprice,  total,
		curr, discount,  deliverydate, projectnumber,
		serialnumber
	);
	...
	$column_header{discount} =
	  qq|<th class=listheading>| . $locale->text('Disc') . qq|</th>|;
	$column_header{qty} =
	...
	$label = ucfirst $form->{db};
	$form->{title} =
	  $locale->text( $label . " History" ) . " / $form->{company}";
...
|;
			...
			if ( $form->{type} eq 'invoice' ) {
				print
qq|<th align=left colspan=$colspan><a href=${module}.pl?action=edit&id=$ref->{invid}&path=$form->{path}&login=$form->{login}&callback=$callback>$invlabel $ref->{invnumber} / $ref->{employee}</a></th>|;
			}

			if ( $form->{type} eq 'order' ) {
				print
qq|<th align=left colspan=$colspan><a href=${module}.pl?action=edit&id=$ref->{invid}&type=$ordertype&path=$form->{path}&login=$form->{login}&callback=$callback>$ordlabel $ref->{ordnumber} / $ref->{employee}</a></th>|;
			}

			if ( $form->{type} eq 'quotation' ) {
				print
qq|<th align=left colspan=$colspan><a href=${module}.pl?action=edit&id=$ref->{invid}&type=$quotationtype&path=$form->{path}&login=$form->{login}&callback=$callback>$quolabel $ref->{quonumber} / $ref->{employee}</a></th>|;
			}
...
|;
		...
		if ( $form->{l_curr} ) {
			$column_data{fxsellprice} = qq|<td align=right>|
			  . $form->format_amount( \%myconfig,
				$ref->{sellprice} / $ref->{exchangerate},
				$form->{precision} )
			  . "</td>";
		...
		$column_data{sellprice} = qq|<td align=right>|
		  ...
		  . "</td>";
		$column_data{total} = qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $ref->{sellprice} * $ref->{qty},
			$form->{precision} )
		  . "</td>";

		$column_data{qty} = qq|<td align=right>|
...
qq|<td><a href=ic.pl?action=edit&id=$ref->{pid}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}</a></td>|;
...
sub form_header {
	...
	for (qw(creditlimit threshold)) {
		$form->{$_} = $form->format_amount( \%myconfig, $form->{$_}, 0 );
	}
	for (qw(discount cashdiscount)) {
		$form->{$_} = $form->format_amount( \%myconfig, $form->{$_}, undef );
	}
...
|;
	...
	for ( split / /, $form->{taxaccounts} ) {
		...
		if ( $form->{"tax_$_"} ) {
			$taxable .=
qq| <input name="tax_$_" value=1 class=checkbox type=checkbox checked>&nbsp;<b>$form->{"tax_${_}_description"}</b>|;
		}
		else {
			$taxable .=
qq| <input name="tax_$_" value=1 class=checkbox type=checkbox>&nbsp;<b>$form->{"tax_${_}_description"}</b>|;
		}
	}
	  ...
	  <tr>
	    <td>
	      <table>
	        <tr>
		  <td>$taxable</td>
		  <td><input name=taxincluded class=checkbox type=checkbox value=1 $form->{taxincluded}> <b>|
...
|;
	...
	$discountaccount = qq|
	  ...
	  <td><b>|
	  . $locale->text('Net')
	  . qq|</b> <input name=terms size=3 value="$form->{terms}"> <b>|
	  . $locale->text('days')
	  . qq|</b></td>
	</tr>
| . $form->hide_form(qw(discount_accno cashdiscount discountterms));
	...
	if ( $form->{selectdiscount} ) {
	  ...
	  <td><select name="discount_accno">|
		  . $form->select_option( $form->{selectdiscount},
			$form->{discount_accno} )
		  . qq|</select>
	  </td>
	  <th align=right>| . $locale->text('Terms') . qq|</th>
	  <td><input name=cashdiscount size=3 value=$form->{cashdiscount}>% / <input name=discountterms size=3 value=$form->{discountterms}> <b>|
		  . $locale->text('Net')
		  . qq|</b> <input name=terms size=3 value="$form->{terms}"> <b>|
		  . $locale->text('days')
		  . qq|</b></td>
...
|;
	...
	if ( $form->{selectpaymentmethod} ) {
	  ...
	  <select name=paymentmethod>|
		  . $form->select_option( $form->{selectpaymentmethod},
...
|;
	...
	<tr>
	  ...
	  <td><select name="payment_accno">|
		  . $form->select_option( $form->{selectpayment},
			$form->{payment_accno} )
		  . qq|</select>
	  </td>
...
|;
	...
	if ( $form->{selectdispatch} ) {
	  ...
	  <td><select name=dispatch>|
		  . $form->select_option( $form->{selectdispatch}, $form->{dispatch},
			1 )
		  . qq|</select>
	  </td>
      </tr>
|;
	...
	if ( $form->{selectpricegroup} && $form->{db} eq 'customer' ) {
	  ...
	  <td><select name=pricegroup>|
		  . $form->select_option( $form->{selectpricegroup},
			$form->{pricegroup}, 1 )
		  . qq|</select>
	  </td>
|;
...
|;
	...
	if ( $form->{selectemployee} ) {
		...
		<td><select name=employee>|
		  . $form->select_option( $form->{selectemployee}, $form->{employee},
			1 )
		  . qq|</select>
		</td>
|;
	...
	$typeofcontact = qq|
		...
		<input name=typeofcontact type=radio value="company" $typeofcontact{company} onClick="javascript:document.forms[0].submit()">|
	  . $locale->text('Company') . qq|
		<input name=typeofcontact type=radio value="person" $typeofcontact{person} onClick="javascript:document.forms[0].submit()">|
	  . $locale->text('Person') . qq|
		</td>
	      </tr>
|;
	...
	if ( $form->{typeofcontact} eq 'person' ) {
		...
		<td><input name=salutation size=32 maxlength=32 value="|
		  . $form->quote( $form->{salutation} )
		  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('First Name') . qq|</th>
		<td><input name=firstname size=32 maxlength=31 value="|
		  . $form->quote( $form->{firstname} )
		  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Last Name') . qq|</th>
		<td><input name=lastname size=32 maxlength=32 value="|
		  . $form->quote( $form->{lastname} )
		  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Title') . qq|</th>
		<td><input name=contacttitle size=32 maxlength=32 value="|
		  . $form->quote( $form->{contacttitle} )
		  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Occupation') . qq|</th>
		<td><input name=occupation size=32 maxlength=32 value="|
		  . $form->quote( $form->{occupation} )
		  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right></th>
		<td><input name=gender type=radio value="M" $gender{M}>|
		  . $locale->text('Male') . qq|
		<input name=gender type=radio value="F" $gender{F}>|
		  . $locale->text('Female') . qq|
		</td>
	      </tr>
|;
...
<table width=100%>
	...
	<tr class=listheading>
	  ...
	  <td width=50%>
	    <table>
	      <tr>
		<th align=right nowrap>$vcnumber</th>
		<td><input name="$form->{db}number" size=32 maxlength=32 value="|
	  . $form->quote( $form->{"$form->{db}number"} )
	  . qq|"></td>
	      </tr>
	      ...
	      <tr>
		<th align=right nowrap>| . $locale->text('Address') . qq|</th>
		<td><input name=address1 size=32 maxlength=64 value="|
	  . $form->quote( $form->{address1} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th></th>
		<td><input name=address2 size=32 maxlength=64 value="|
	  ...
	  . qq|"></td>
	      ...
	      <tr>
		<th align=right nowrap>| . $locale->text('Zip/Postal Code') . qq|</th>
		<td><input name=zipcode size=11 maxlength=32 value="|
	  . $form->quote( $form->{zipcode} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Country') . qq|</th>
		<td><input name=country size=32 maxlength=64 value="|
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><input name=taxnumber size=20 value="|
	  . $form->quote( $form->{taxnumber} )
	  . qq|"></td>
	  $gifi
	  <th align=right>| . $locale->text('SIC') . qq|</th>
	  <td><input name=sic_code size=6 maxlength=6 value="|
	  . $form->quote( $form->{sic_code} )
	  . qq|"></td>
	</tr>
	<tr>
	  $typeofbusiness
	...
	<tr valign=top>
	  ...
	  <td colspan=4>
		...
		<td><input name=bankname size=32 maxlength=64 value="|
	  . $form->quote( $form->{bankname} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Address') . qq|</th>
		<td><input name=bankaddress1 size=32 maxlength=32 value="|
	  . $form->quote( $form->{bankaddress1} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th></th>
		<td><input name=bankaddress2 size=32 maxlength=32 value="|
	  . $form->quote( $form->{bankaddress2} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('City') . qq|</th>
		<td><input name=bankcity size=32 maxlength=32 value="|
	  . $form->quote( $form->{bankcity} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('State/Province') . qq|</th>
		<td><input name=bankstate size=32 maxlength=32 value="|
	  . $form->quote( $form->{bankstate} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Zip/Postal Code') . qq|</th>
		<td><input name=bankzipcode size=11 maxlength=10 value="|
	  . $form->quote( $form->{bankzipcode} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('Country') . qq|</th>
		<td><input name=bankcountry size=32 maxlength=32 value="|
	  . $form->quote( $form->{bankcountry} )
	  . qq|"></td>
	      </tr>
	    </table>
	  </td>
...
|;
	...
	$form->hide_form( map { "tax_${_}_description" }
		  ( split / /, $form->{taxaccounts} ) )
	  if $form->{taxaccounts};
	$form->hide_form( map { "select$_" }
		  qw(currency arap discount payment business dispatch pricegroup language employee paymentmethod)
	);
	$form->hide_form( map { "shipto$_" }
...
sub form_footer {
	...
	%button = (
		...
		'Shipping Address' =>
		  { ndx => 3, key => 'H', value => $locale->text('Shipping Address') },
		'Save as new' =>
		  { ndx => 4, key => 'N', value => $locale->text('Save as new') },
		'AR Transaction' =>
		...
		'Credit Invoice' =>
		  { ndx => 10, key => 'R', value => $locale->text('Credit Invoice') },
		'POS' => { ndx => 11, key => 'C', value => $locale->text('POS') },
		'Sales Order' =>
		  { ndx => 12, key => 'O', value => $locale->text('Sales Order') },
		'Quotation' =>
		  { ndx => 13, key => 'Q', value => $locale->text('Quotation') },
		'Vendor Invoice' =>
		  { ndx => 14, key => 'I', value => $locale->text('Vendor Invoice') },
		'Debit Invoice' =>
		  { ndx => 15, key => 'R', value => $locale->text('Debit Invoice') },
		'Purchase Order' =>
		  { ndx => 16, key => 'O', value => $locale->text('Purchase Order') },
		'RFQ' => { ndx => 17, key => 'Q', value => $locale->text('RFQ') },
		'Pricelist' =>
		  { ndx => 18, key => 'P', value => $locale->text('Pricelist') },
		'New Number' =>
		  { ndx => 19, key => 'M', value => $locale->text('New Number') },
	...
	$form->hide_form(
		qw(id ARAP update_contact addressid contactid taxaccounts path login callback db status previousform addvc)
	);
...
sub report {
	...
	$form->{callback} =
qq|$module.pl?action=search&due_checked=checked&$form->{db}number=$form->{"$form->{db}number"}&vc=$form->{db}&nextsub=transactions|;
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  <td></td>
	  <th align=right nowrap>$vcname</th>
	  <td><input name=shiptoname size=32 maxlength=64 value="|
	  . $form->quote( $form->{shiptoname} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{address1}{label}</th>
	  <td><input name=shiptoaddress1 size=32 maxlength=32 value="|
	  . $form->quote( $form->{shiptoaddress1} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <td></td>
	  <td><input name=shiptoaddress2 size=32 maxlength=32 value="|
	  . $form->quote( $form->{shiptoaddress2} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{city}{label}</th>
	  <td><input name=shiptocity size=32 maxlength=32 value="|
	  . $form->quote( $form->{shiptocity} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{state}{label}</th>
	  <td><input name=shiptostate size=32 maxlength=32 value="|
	  . $form->quote( $form->{shiptostate} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{zipcode}{label}</th>
	  <td><input name=shiptozipcode size=11 maxlength=10 value="|
	  . $form->quote( $form->{shiptozipcode} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{country}{label}</th>
	  <td><input name=shiptocountry size=32 maxlength=32 value="|
	  . $form->quote( $form->{shiptocountry} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
	  <th align=right nowrap>$shipto{contact}{label}</th>
	  <td><input name=shiptocontact size=35 maxlength=64 value="|
	  . $form->quote( $form->{shiptocontact} )
	  . qq|"></td>
	</tr>
	<tr>
	  <td></td>
...
sub shipto_selected {
	...
	for $i ( 1 .. $form->{shipto_rows} ) {
		if ( $form->{"ndx_$i"} ) {
			for ( split / /, $form->{flds} ) {
				$form->{"shipto$_"} = $form->{"shipto${_}_$i"};
			}
			last;
		}
	}
...
sub pricelist {
	...
	if ( @{ $form->{all_partsgroup} } ) {
		...
		$form->{selectpartsgroup} =
		  $form->escape( $form->{selectpartsgroup}, 1 );
	}
...
|;
	...
	foreach $i ( 1 .. $form->{rowcount} ) {
		...
		if ( $i < $form->{rowcount} ) {
			$column_data{partsgroup} = qq|<td>$form->{"partsgroup_$i"}</td>|
			...
			if ( $form->{"id_$i"} eq $sameid ) {
				...
				$form->hide_form( map { "${_}_$i" }
					  qw(id partnumber description partsgroup partsgroup_id) );

			}
			else {

				$column_data{sku} =
				    qq|<td><input name="sku_$i" value="|
				  . $form->quote( $form->{"sku_$i"} )
				  . qq|"></td>|;
				$column_data{partnumber} =
				    qq|<td><input name="partnumber_$i" value="|
				  . $form->quote( $form->{"partnumber_$i"} )
				  . qq|"></td>|;

				$column_data{description} =
				  qq|<td>$form->{"description_$i"}&nbsp;</td>|;
				$form->hide_form( map { "${_}_$i" }
					  qw(id description partsgroup partsgroup_id) );
			}
			...
			if ( $form->{db} eq 'customer' ) {
				$column_data{partnumber} =
				  qq|<td><input name="partnumber_$i"></td>|;
			}
			else {
			...
			$column_data{description} =
			  qq|<td><input name="description_$i"></td>|;

			if ( $form->{selectpartsgroup} ) {
				$column_data{partsgroup} =
				    qq|<td><select name="partsgroup_$i">|
...
|;
		...
		if ( $form->{db} eq 'customer' ) {
			...
			$column_data{sellprice} =
			    qq|<td align=right><input name="sellprice_$i" size=11 value=|
			  . $form->format_amount( \%myconfig, $form->{"sellprice_$i"} )
			  . qq|></td>|;

			$column_data{validfrom} =
qq|<td nowrap><input name="validfrom_$i" size=11 value=$form->{"validfrom_$i"}></td>|;
			$column_data{validto} =
qq|<td nowrap><input name="validto_$i" size=11 value=$form->{"validto_$i"}></td>|;
		}
		...
		if ( $form->{db} eq 'vendor' ) {
			...
			$column_data{lastcost} =
			    qq|<td align=right><input name="lastcost_$i" size=11 value=|
			  . $form->format_amount( \%myconfig, $form->{"lastcost_$i"} )
			  . qq|></td>|;
		}

		$column_data{curr} =
		    qq|<td><select name="curr_$i">|
		  . $form->select_option( $form->{selectcurrency}, $form->{"curr_$i"} )
		  . qq|</select></td>|;
...
|;
	...
	# delete variables
	foreach $i ( 1 .. $form->{rowcount} ) {
		for ( @column_index, "id", "partsgroup_id" ) {
			delete $form->{"${_}_$i"};
		}
	}
...
sub update {
	...
	for (qw(creditlimit threshold discount cashdiscount)) {
...
qq|<td><input name="ndx_$i" class=checkbox type=checkbox value=$i></td>|;
		...
		$column_data{sellprice} =
		  qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $ref->{sellprice}, undef,
			"&nbsp;" )
		  . qq|</td>|;
		$column_data{lastcost} =
		  qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $ref->{lastcost}, undef,
			"&nbsp;" )
		  . qq|</td>|;
...
|;
		...
		{
			print qq|<input type=hidden name="new_${_}_$i" value="|
			  . $form->quote( $ref->{$_} )
			  . qq|">\n|;
		}
	}
...
sub add_transaction {
	...
	$form->{callback} =
"$form->{script}?login=$form->{login}&path=$form->{path}&action=add&vc=$form->{db}&$form->{db}_id=$form->{id}&$form->{db}=$name&type=$form->{type}&callback=$form->{callback}";
...
sub save {
	...
	$form->isvaldate( \%myconfig, $form->{startdate},
		$locale->text('Invalid start date ...') );
	$form->isvaldate( \%myconfig, $form->{enddate},
		$locale->text('Invalid end date ...') );
	...
	if ( $form->{previousform} && !$form->{callback} ) {
		...
		$form->{callback} .=
qq|&$form->{db}_id=$form->{id}&old$form->{db}="$form->{name}--$form->{id}"|;
	}
...
sub new_number {
	...
	$form->{"$form->{db}number"} =
	  $form->update_defaults( \%myconfig, "$form->{db}number" );
