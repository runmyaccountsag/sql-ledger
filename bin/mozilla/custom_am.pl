...
sub ask_dbcheck {
	...
	$form->error( $locale->text('Only for admin ...') )
	  unless $myconfig{role} eq 'admin';

	$form->{title} = $locale->text('Ledger Doctor');
	$form->header;
	my $dbh = $form->dbconnect( \%myconfig );
	my ($firstdate) =
	  $dbh->selectrow_array("SELECT MIN(transdate) FROM acc_trans");
	my ($lastdate) =
	  $dbh->selectrow_array("SELECT MAX(transdate) FROM acc_trans");
	print qq|
<body>
  <table width=100%>
...
<form method=post action=$form->{script}>
  <table>
	...
	<td><input name=lastdate size=11 value='$lastdate' title='$myconfig{dateformat}'></td>
	  ...
	  . $locale->text(
'All transactions outside this date range will be reported as having invalid dates.'
	  )
	  . qq|
<br><br><hr/>
...
qq|<body><table width=100%><tr><th class=listtop>$form->{title}</th></tr></table><br />|;
	...
	my $query, $sth, $i;
	my $callback =
"$form->{script}?action=do_dbcheck&firstdate=$form->{firstdate}&lastdate=$form->{lastdate}&path=$form->{path}&login=$form->{login}";
...
|;
	...
	if ( $count or $count2 or $count3 ) {
		...
		$form->info(
			$locale->text("There are $count3 blank non-TAX rows ...") );
		print qq|
<form method=post action=$form->{script}>
<input type=submit class=submit name=action value="|
		  . $locale->text('Click here to delete blank rows') . qq|">
<input type=submit class=submit name=action value="|
		  . $locale->text('Click here to delete blank TAX rows') . qq|">
<input type=submit class=submit name=action value="|
		  . $locale->text('Click here to delete blank non-TAX rows') . qq|">
|;
...
|;
  ...
  |;
	...
	print qq|<th class=listheading>|
	  . $locale->text('Invoice Number / Reference')
	  . qq|</td>|;
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		if ( $form->round_amount( $ref->{amount}, 2 ) != 0 ) {
			...
			print
qq|<td><a href=$module.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}</a></td>|;
			print qq|<td>$ref->{transdate}</td>|;
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $ref->{amount}, 6 )
			  . qq|</td>|;
			print qq|</tr>|;
	...
	print
	  qq|<tr class=listtotal><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>|
	...
	print
qq|<h3>Paid transactions from AR where amount booked on AR account in debit is different from amount booked on AR account in credit.
</h3>|;
...
|;
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		print qq|<tr class=listrow$i>|;
		print
qq|<td><a href=$module?action=edit&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}</a></td>|;
	...
	print
qq|<h3>Paid transactions from AP where amount booked on AP account in debit is different from amount booked on AP account in credit.
</h3>|;
...
qq|<td><a href=$module?action=edit&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}</a></td>|;
	...
	$form->info(
'To delete these orphaned rows, run following query in psql or phpPgAdmin or pgAdmin3. 
...
|;
	...
	if ($blankrows) {
		print qq|<p>There were '|
		  . $blankrows
		  . qq| rows with blank transdate in invoice table. Being corrected now ...'|;
		$dbh->do(
'UPDATE invoice SET transdate = (SELECT transdate FROM ar WHERE ar.id = invoice.trans_id) WHERE trans_id IN (SELECT id FROM ar) AND transdate IS NULL'
		);
		$dbh->do(
'UPDATE invoice SET transdate = (SELECT transdate FROM ap WHERE ap.id = invoice.trans_id) WHERE trans_id IN (SELECT id FROM ap) AND transdate IS NULL'
		);
	...
	my ($blankrows) = $dbh->selectrow_array($query);
	if ($blankrows) {
		print qq|<p>There are |
		  . $blankrows
		  . qq| rows with blank shippingdate in invoice table. ...'|;
...
#$dbh->do('UPDATE invoice SET transdate = (SELECT transdate FROM ap WHERE ap.id = invoice.trans_id) WHERE trans_id IN (SELECT id FROM ap) AND transdate IS NULL');
  ...
  |;
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		if ( $form->round_amount( $ref->{tax1}, 2 ) !=
			$form->round_amount( $ref->{tax2}, 2 ) )
		{
			print qq|<tr class=listrow$i>|;
			print qq|<td>$ref->{module}</td>|;
			print
qq|<td><a href=$module.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}</a></td>|;
			...
			print qq|<td align=right>|
			  ...
			  . qq|</td>|;
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $ref->{tax1}, 2 )
			  . qq|</td>|;
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $ref->{tax2}, 2 )
			  . qq|</td>|;
			print qq|</tr>|;
		}
	}
...
qq|INSERT INTO invoicetax (trans_id, invoice_id, chart_id, amount, taxamount)
	...
	while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		while ( $itref = $itsth->fetchrow_hashref(NAME_lc) ) {
			$itins->execute(
				$itref->{trans_id}, $itref->{id}, $itref->{chart_id},
				$itref->{amount},   $itref->{taxamount}
			);
		}
	}
