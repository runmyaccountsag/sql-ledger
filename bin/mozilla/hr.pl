...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&db=$form->{db}&path=$form->{path}&login=$form->{login}"
...
qq|<input name="l_workphone" type=checkbox class=checkbox value=Y checked> |
	...
	push @a,
	  qq|<input name="l_workfax" type=checkbox class=checkbox value=Y checked> |
	  . $locale->text('Work Fax');
	push @a,
	  qq|<input name="l_workmobile" type=checkbox class=checkbox value=Y> |
	  . $locale->text('Work Mobile');
	push @a,
...
sub list_employees {
	...
	$href =
"$form->{script}?action=list_employees&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&status=$form->{status}";
...
"$form->{script}?action=list_employees&direction=$form->{direction}&oldsort=$form->{oldsort}&db=$form->{db}&path=$form->{path}&login=$form->{login}&status=$form->{status}";
	...
	@columns = $form->sort_columns(
		qw(id employeenumber name address city state zipcode country workphone workfax workmobile homephone email startdate enddate ssn dob iban bic sales role manager login notes)
	);
	unshift @columns, "ndx";
	...
	$column_header{workfax} =
	    qq|<th><a class=listheading href=$href&sort=workfax>|
	  . $locale->text('Work Fax')
	  . qq|</a></th>|;
	$column_header{workmobile} =
	    qq|<th><a class=listheading href=$href&sort=workmobile>|
	  . $locale->text('Work Mobile')
	  . qq|</a></th>|;
...
|;
	...
	foreach $ref ( @{ $form->{all_employee} } ) {
		...
		$column_data{name} =
"<td><a href=$form->{script}?action=edit&db=employee&id=$ref->{id}&path=$form->{path}&login=$form->{login}&status=$form->{status}&callback=$callback>$ref->{name}&nbsp;</a></td>";
...
sub employee_links {
	...
	if ( @{ $form->{all_deduction} } ) {
		...
		for ( @{ $form->{all_deduction} } ) {
			$form->{selectdeduction} .= qq|$_->{description}--$_->{id}\n|;
		}
	}
	...
	if ( @{ $form->{all_manager} } ) {
		...
		for ( @{ $form->{all_manager} } ) {
			$form->{selectmanager} .= qq|$_->{name}--$_->{id}\n|;
		}
	}
...
sub new_number {
	...
	$form->{employeenumber} =
	  $form->update_defaults( \%myconfig, employeenumber );
...
|;
	  ...
	  <td><select name=manager>|
			  . $form->select_option( $form->{selectmanager},
				$form->{manager}, 1 )
			  . qq|</select></td>
	</tr>
|;
    ...
    <td>
	  ...
	  <td>
	    <table>
	      <tr>
		<th align=right nowrap>| . $locale->text('Employee Number') . qq|</th>
		<td><input name=employeenumber size=32 maxlength=32 value="|
	  . $form->quote( $form->{employeenumber} )
	  . qq|"></td>
	      </tr>
	      <tr>
	  ...
	  . qq| <font color=red>*</font></th>
		<td><input name=name size=35 maxlength=64 value="|
	  ...
	  <td>
		...
		<td><input name=ssn size=20 maxlength=20 value="|
	  . $form->quote( $form->{ssn} )
	  . qq|"></td>
	      </tr>
	      <tr>
		...
		<td><input name=iban size=34 maxlength=34 value="|
	  . $form->quote( $form->{iban} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right nowrap>| . $locale->text('BIC') . qq|</th>
		<td><input name=bic size=11 maxlength=11 value="|
	  . $form->quote( $form->{bic} )
	  . qq|"></td>
	      </tr>
	    </table>
...
|;
	  ...
	  <td><select name="deduction_$i">|
			  . $form->select_option( $form->{selectdeduction},
				$form->{"deduction_$i"}, 1 )
			  . qq|</select></td>
	  <td><input name="before_$i" value=|
			  . $form->format_amount( \%myconfig, $form->{"before_$i"},
				$form->{precision} )
			  . qq|></td>
	  <td><input name="after_$i" value=|
			  . $form->format_amount( \%myconfig, $form->{"after_$i"},
				$form->{precision} )
			  . qq|></td>
	  <td><input name="rate_$i" size=8 value=|
...
sub employee_footer {
	...
	if ( !$form->{readonly} ) {
		%button = (
			...
			'Save as new' =>
			  { ndx => 5, key => 'N', value => $locale->text('Save as new') },
			'New Number' =>
...
sub deduction_links {
	...
	for ( @{ $form->{ap_accounts} } ) {
		$form->{selectap} .= "$_->{accno}--$_->{description}\n";
	}
	...
	for ( @{ $form->{expense_accounts} } ) {
		$form->{selectexpense} .= "$_->{accno}--$_->{description}\n";
	}
	...
	$form->{selectbase} = "\n";
	for ( @{ $form->{all_deduction} } ) {
		$form->{selectbase} .= qq|$_->{description}--$_->{id}\n|;
	}
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  . $locale->text('Description')
	  . qq| <font color=red>*</font></th>
	  <td><input name=description size=35 value="|
	...
	<tr>
	  ...
	  . $form->select_option( $form->{selectap}, $form->{ap_accno} )
	  . qq|</select></td>
	...
	<tr>
	  ...
	  . $form->select_option( $form->{selectexpense}, $form->{expense_accno} )
	  . qq|</select></td>
...
|;
	...
	for $i ( 1 .. $form->{rate_rows} ) {
		print qq|
	      <tr>
		<td><input name="rate_$i" size=11 value=|
		  . $form->format_amount( \%myconfig, $form->{"rate_$i"} )
		  . qq|></td>
		<td><input name="amount_$i" size=11 value=|
		  . $form->format_amount( \%myconfig, $form->{"amount_$i"},
			$form->{precision} )
		  . qq|></td>
		<td><input name="above_$i" size=11 value=|
		  . $form->format_amount( \%myconfig, $form->{"above_$i"},
			$form->{precision} )
		  . qq|></td>
		<td><input name="below_$i" size=11 value=|
		  . $form->format_amount( \%myconfig, $form->{"below_$i"},
			$form->{precision} )
		  . qq|></td>
	      </tr>
|;
...
|;
	...
	<tr>
	  ...
	  <td><select name="base_$i">|
		  . $form->select_option( $form->{selectbase}, $form->{"base_$i"}, 1 )
		  . qq|</select></td>
	  <th>$maximum</th>
	  <td><input name="maximum_$i" value=|
		  . $form->format_amount( \%myconfig, $form->{"maximum_$i"},
			$form->{precision} )
		  . qq|></td>
	</tr>
|;
	...
	<tr>
	  ...
	  <td><select name="after_$i">|
		  . $form->select_option( $form->{selectbase}, $form->{"after_$_"}, 1 )
		  . qq|</select></td>
	</tr>
|;
		$deductafter = "";
