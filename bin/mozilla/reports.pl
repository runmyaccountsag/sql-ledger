...
sub alltaxes {
	...
	if ( @{ $form->{all_years} } ) {
      ...
      <td colspan=3>
      <select name=month>|
		  . $form->select_option( $selectaccountingmonth, $form->{month}, 1, 1 )
		  . qq|</select>
      <select name=year>|
		  . $form->select_option( $selectaccountingyear, $form->{year}, 1 )
		  . qq|</select>
...
|;
	...
	my @columns =
	  qw(module account transdate invnumber description name number f amount tax);
	...
	$sortorder =
	  ( $sort eq $oldsort ) ? ( $sortorder eq 'asc' ? 'desc' : 'asc' ) : 'asc';
	...
	if ( $form->{year} && $form->{month} ) {
		...
		for (qw(fromdate todate)) {
			$form->{$_} =
			  $form->format_date( $myconfig{dateformat}, $form->{$_} );
		}
	}
	if ( !$form->{runit} ) {
...
<tr>
	...
	for (@columns) {
		$checked =
		  $form->{runit} ? ( $form->{"l_$_"} ? 'checked' : '' ) : 'checked';
		print qq|<input type=checkbox name=l_$_ value="1" $checked> |
		  . ucfirst($_);
	}
...
|;
	...
	$where .= qq| AND aa.department_id = $form->{department_id}|
	  if $form->{department};
		...
		for my $m (qw(AR AP GL)) {
			for my $a ( sort keys %{ $summary{$m} } ) {
				print qq|<tr class="listrow0">
                    ...
                    <td>$a</td>
                    <td align="right">|
				  . $form->format_amount( \%myconfig, $summary{$m}{$a}{amount},
					2 )
				  . qq|</td>
                    <td align="right">|
				  . $form->format_amount( \%myconfig, $summary{$m}{$a}{tax}, 2 )
				  . qq|</td>
                </tr>|;
			}
		}
	...
	for (qw(action nextsub sortorder l_subtotal login path)) {
		$url .= "&$_=$form->{$_}";
	}
	for (@report_columns) {
		$url .= qq|&l_$_=$form->{"l_$_"}| if $form->{"l_$_"};
	}
	for (@search_columns) { $url .= qq|&$_=$form->{$_}| if $form->{$_} }
	for (@report_columns) {
		$tabledata{$_} =
		    qq|<th><a class="listheading" href="$url&sort=$_">|
		  . ucfirst $_
		  . qq|</a></th>\n|;
	}
...
|;
	...
	for $row (@allrows) {
		...
		if (
			$form->{l_subtotal}
			and
			( $row->{account} ne $groupvalue or $row->{module} ne $groupvalue2 )
		  )
		{
			...
			for (@total_columns) {
				$tabledata{$_} =
				    qq|<th align="right">|
				  . $form->format_amount( \%myconfig, $subtotals{$_}, 2 )
				  . qq|</th>|;
			}
			...
			if ( $groupvalue2 ne $row->{module} ) {
				...
				for (@total_columns) {
					$tabledata{$_} =
					    qq|<th align="right">|
					  . $form->format_amount( \%myconfig, $totals{$_}, 2 )
					  . qq|</th>|;
				}
				print qq|<tr class="listsubtotal">|;
				for (@report_columns) { print $tabledata{$_} }
		...
		$invnumber =
qq|<a href="$row->{script}?id=$row->{id}&action=edit&path=$form->{path}&login=$form->{login}" target="_blank">$row->{invnumber}</a>|;
		...
		if ($db) {
			$vc =
qq|<a href="ct.pl?id=$row->{vc_id}&db=$db&action=edit&path=$form->{path}&login=$form->{login}" target="_blank">$row->{name}</a>|;
		...
		for (@total_columns) {
			$tabledata{$_} =
			    qq|<td align="right">|
			  . $form->format_amount( \%myconfig, $row->{$_}, 2 )
			  . qq|</td>|;
		}
	...
	for (@total_columns) {
		$tabledata{$_} =
		    qq|<th align="right">|
		  . $form->format_amount( \%myconfig, $grandtotals{$_}, 2 )
		  . qq|</th>|;
	}
	print qq|<tr class="listtotal">|;
	for (@report_columns) { print $tabledata{$_} }
...
sub onhandvalue_search {
	...
	&print_checkbox( 'l_onhand_qty', $locale->text('Onhand Qty'),
		'checked', '' );
	&print_checkbox( 'l_components', $locale->text('Components'), '', '' );
	&print_checkbox( 'l_onhand_amt', $locale->text('Onhand Amount'),
		'checked', '<br>' );
...
sub onhandvalue_list {
	...
	$where .= qq| AND (LOWER(p.partnumber) LIKE '$partnumber')|
	  if $form->{partnumber};
	$where .= qq| AND (LOWER(p.description) LIKE '$name')|
	  if $form->{description};
...
#$where .= qq| AND (i.warehouse_id = $form->{warehouse_id})| if $form->{warehouse};
	...
	my $componentswhere;
	$componentswhere = qq| AND i.assemblyitem IS FALSE|
	  if !$form->{l_components};

	@columns =
	  qw(id warehouse partnumber description partsgroup unit onhand_qty onhand_amt);
	...
	for (
		qw(l_subtotal l_components department warehouse partsgroup partnumber description dateto)
	  )
	{
	...
	$column_header{partnumber} =
	  rpt_hdr( 'partnumber', $locale->text('Number'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	$column_header{partsgroup} =
	  rpt_hdr( 'partsgroup', $locale->text('Group'), $href );
	$column_header{unit} = rpt_hdr( 'unit', $locale->text('Unit'), $href );
	$column_header{onhand_qty} =
	  rpt_hdr( 'onhand_qty', $locale->text('Onhand Qty') );
	$column_header{onhand_amt} =
	  rpt_hdr( 'onhand_amt', $locale->text('Onhand Amount') );
...
qq|$form->{script}?action=onhandvalue_detail&id=$ref->{id}&l_components=$form->{l_components}&l_sql=$form->{l_sql}&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
		...
		$column_data{description} =
		  rpt_txt( $ref->{description}, $form->{link} );
...
sub onhandvalue_detail {
	...
	# No. columns should always come first
	splice @columns, 0, 0, 'no';
	for (qw(no invnumber transdate qty sellprice extended)) {
		$form->{"l_$_"} = 'Y';
	}
	...
	$column_header{invnumber} =
	  rpt_hdr( 'invnumber', $locale->text('Invoice'), $href );
	$column_header{transdate} =
	  rpt_hdr( 'transdate', $locale->text('Date'), $href );
	$column_header{qty} = rpt_hdr( 'qty', $locale->text('Qty'), $href );
	$column_header{sellprice} =
	  rpt_hdr( 'sellprice', $locale->text('Price'), $href );
	...
	&print_title;
	print $locale->text('Number')
	  . qq|: $form->{partnumber} / $form->{description}|;
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{link} =
qq|$ref->{module}.pl?readonly=1&action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
...
<tr>
  <th align=right>|
	  . $locale->text('From')
	  . qq|</th><td><input name=datefrom size=11 title='$myconfig{dateformat}'>
  |
	  . $locale->text('To')
	  . qq| <input name=dateto size=11 title='$myconfig{dateformat}'></td>
</tr>
$selectfrom
|;
	...
	&print_checkbox( 'l_notransactions', $locale->text('No Transactions'),
		'', '<br>' );
	&print_checkbox( 'l_group', $locale->text('Group'), '', '' );
	&print_checkbox( 'l_csv', $locale->text('CSV'), '' );
	&print_checkbox( 'fx_transaction',
		$locale->text('Include Exchange Rate Difference'), 'checked' );
	print qq|</td></tr>|;
	&end_table;
	print('<hr size=3 noshade>');
...
sub gl_list {
	...
	$form->isvaldate( \%myconfig, $form->{datefrom},
		$locale->text('Invalid from date ...') );
	$form->isvaldate( \%myconfig, $form->{dateto},
	...
	my $glwhere = qq| (1 = 1)|;
	$glwhere .= qq| AND c.accno >= '| . $form->dbclean($fromaccount) . qq|'|
	  if $form->{fromaccount};
	$glwhere .= qq| AND c.accno <= '| . $form->dbclean($toaccount) . qq|'|
	  if $form->{toaccount};
	$glwhere .= qq| AND ac.transdate >= '$form->{datefrom}'|
	  if $form->{datefrom};
	...
	for (qw(fromaccount toaccount datefrom dateto)) {
		$callback .= "&$_=" . $form->escape( $form->{$_}, 1 );
	}

	@columns =
	  qw(id transdate reference description name source debit credit balance);
	...
	$callback .=
	  "&l_subtotal=$form->{l_subtotal}&fx_transaction=$form->{fx_transaction}";
	my $href = $callback;
	$form->{callback} = $form->escape( $callback, 1 );
			...
			$query .= qq|
                ...
                FROM chart c
                     WHERE NOT EXISTS (
                        ...
                        |;
			$query .= qq| AND ac.transdate >= '$form->{datefrom}'|
			  if $form->{datefrom};
			$query .= qq| AND ac.transdate <= '$form->{dateto}'|
			  if $form->{dateto};
			$query .= qq|)|;
			$query .= qq| AND c.accno >= '$fromaccount'|
			  if $form->{fromaccount};
	...
	$column_header{reference} =
	  rpt_hdr( 'reference', $locale->text('Reference'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	$column_header{name} =
	  rpt_hdr( 'name', $locale->text('Company Name'), $href );
	$column_header{source} =
	  rpt_hdr( 'source', $locale->text('Source'), $href );
	$column_header{debit} =
	  rpt_hdr( 'debit', $locale->text('Debit'), undef, 'right' );
	$column_header{credit} =
	  rpt_hdr( 'credit', $locale->text('Credit'), undef, 'right' );
	$column_header{balance} =
	  rpt_hdr( 'balance', $locale->text('Balance'), undef, 'right' );
		...
		while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
			if ( $groupbreak ne "$ref->{accno}--$ref->{accdescription}" ) {
				if ( $groupbreak ne 'none' ) {
					...
					$column_data{debit} =
					  $form->format_amount( \%myconfig, $debit_subtotal * -1,
						$form->{precision}, "0" );
					$column_data{credit} =
					  $form->format_amount( \%myconfig, $credit_subtotal,
						$form->{precision}, "0" );
					$column_data{balance} =
					  $form->format_amount( \%myconfig, $balance * -1,
						$form->{precision}, "0" );
			...
			if ( $ref->{type} ne "empty" ) {
				...
				$link =
qq|$script?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
				$column_data{no}        = '"' . $no . '"';
				$column_data{transdate} = '"' . $ref->{transdate} . '"';
				$column_data{reference} =
				  '"' . &escape_csv( $ref->{reference} ) . '"';
				$column_data{description} =
				  '"' . &escape_csv( $ref->{description} ) . '"';
				$column_data{name} = '"' . &escape_csv( $ref->{name} ) . '"';
				$column_data{source} =
				  '"' . &escape_csv( $ref->{source} ) . '"';
		...
		$column_data{credit} =
		  $form->format_amount( \%myconfig, $credit_subtotal,
			$form->{precision}, "0" );
		$column_data{balance} = $form->format_amount( \%myconfig, $balance * -1,
			$form->{precision}, "0" );
		...
		$column_data{debit} =
		  $form->format_amount( \%myconfig, $debit_total * -1,
			$form->{precision}, "0" );
		$column_data{credit} =
		  $form->format_amount( \%myconfig, $credit_total, $form->{precision},
			"0" );
		$column_data{balance} = ' ';
		...
		$form->{title} =
		  $locale->text('General Ledger') . " / $form->{company}";
		$form->header;
		print qq|<body><table class="noprint report-header" width=100%>|;
		print qq|<tr><th class="listtop">$form->{title}</th></tr>|;
		print qq|<tr><th class="listtopheader" align=left colspan=7>|
		  . $locale->text('From')
		  . qq| $form->{datefrom}</th></tr>|
		  if $form->{datefrom};
		print qq|<tr><th class="listtopheader" align=left colspan=7>|
		  . $locale->text('To')
		  . qq| $form->{dateto}</th></tr>|
		  if $form->{dateto};
		print qq|<tr><th class="listtopheader" align=left colspan=7>|
		  . $locale->text('From Account')
		  . qq| $fromaccount</th></tr>|
		  if $form->{fromaccount};
		print qq|<tr><th class="listtopheader" align=left colspan=7>|
		  . $locale->text('To Account')
		  . qq| $toaccount</th></tr>|
		  if $form->{$toaccount};
		print qq|</table>\n|;
		...
		print
qq|<div class="printonly"><span class="creation-date">$today</span></div>|;

		# Subtotal and total variables
		my $debit_total, $credit_total, $debit_subtotal, $credit_subtotal,
		  $balance;
		...
		my $groupbreak = 'none';
		my $period     = '';
		$period .= $locale->text('From') . ' ' . $form->{datefrom}
		  if $form->{datefrom};
		$period .= $locale->text('To') . ' ' . $form->{dateto}
		  if $form->{dateto};
  ...
  <img style="max-width: 100%" src="https://my.runmyaccounts.com/assets/img/file-icons/icons8-pdf-96.png">
		...
		while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
			if ( $groupbreak ne "$ref->{accno}--$ref->{accdescription}" ) {
				if ( $groupbreak ne 'none' ) {
					for (@column_index) { $column_data{$_} = rpt_txt('&nbsp;') }
					$column_data{debit} = qq|<th align=right>|
					  . $form->format_amount( \%myconfig, $debit_subtotal * -1,
						$form->{precision}, "0" )
					  . qq|</th>|;
					$column_data{credit} = qq|<th align=right>|
					  . $form->format_amount( \%myconfig, $credit_subtotal,
						$form->{precision}, "0" )
					  . qq|</th>|;
					$column_data{balance} = qq|<th align=right>|
					  . $form->format_amount( \%myconfig, $balance * -1,
						$form->{precision}, "0" )
					  . qq|</th>|;
				...
				print qq|<span class="page-topleft">|
				  . $form->{company}
				  . qq|</span>|;
				...
				print qq|<th class=pb10 align=left colspan=7>|
				  . $locale->text('Account')
				  . qq| $groupbreak</th>|;
				print qq|</tr>|;
                ...
                FROM acc_trans
                WHERE chart_id = (SELECT id FROM chart WHERE accno = '$ref->{accno}')|;
					$openingquery .= qq| AND transdate < '$form->{datefrom}'|
					  if $form->{datefrom};
					($balance) = $dbh->selectrow_array($openingquery);
					if ( $balance != 0 ) {
						for (@column_index) {
							$column_data{$_} = rpt_txt('&nbsp;');
						}
						$column_data{debit} =
						  rpt_dec( 0, $form->{precision}, '0' );
						$column_data{credit} =
						  rpt_dec( 0, $form->{precision}, '0' );
						$column_data{balance} =
						  rpt_dec( 0 - $balance, $form->{precision}, '0' );
...
qq|$script?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
				...
				if ( $ref->{amount} > 0 ) {
					...
					$column_data{credit} =
					  rpt_dec( $ref->{amount}, $form->{precision}, '0' );
				}
				else {
					$column_data{debit} =
					  rpt_dec( 0 - $ref->{amount}, $form->{precision}, '0' );
					$column_data{credit} =
					  rpt_dec( 0, $form->{precision}, '0' );
				}
				$balance += $ref->{amount};
				$column_data{balance} =
				  rpt_dec( $balance * -1, $form->{precision}, '0' );
...
sub audit_list {
	...
	$form->isvaldate( \%myconfig, $form->{totransdate},
		$locale->text('Invalid to date ...') );
	...
	$where .= qq| AND (a.tablename = | . $dbh->quote($tablename) . qq|)|
	  if $form->{tablename};
	$where .=
	  qq| AND (a.LOWER(reference) LIKE | . $dbh->quote($reference) . qq|)|
	  if $form->{reference};
	$where .= qq| AND (a.formname = | . $dbh->quote($formname) . qq|)|
	  if $form->{formname};
	$where .= qq| AND (a.action = | . $dbh->quote($formaction) . qq|)|
	  if $form->{formaction};
	$where .= qq| AND (a.transdate >= '$form->{fromtransdate}')|
	  if $form->{fromtransdate};
	$where .= qq| AND (a.transdate <= '$form->{totransdate}')|
	  if $form->{totransdate};
	$where .= qq| AND (a.employee_id = $form->{employee_id})|
	  if $form->{employee};

	@columns =
	  qw(trans_id tablename reference formname action transdate employee_id);
	...
	$column_header{no} = rpt_hdr( 'no', $locale->text('No.') );
	$column_header{trans_id} =
	  rpt_hdr( 'trans_id', $locale->text('Trans ID'), $href );
	$column_header{tablename} =
	  rpt_hdr( 'tablename', $locale->text('Table'), $href );
	$column_header{reference} =
	  rpt_hdr( 'reference', $locale->text('Reference'), $href );
	$column_header{formname} =
	  rpt_hdr( 'formname', $locale->text('Form'), $href );
	$column_header{action} =
	  rpt_hdr( 'action', $locale->text('Action'), $href );
	$column_header{transdate} =
	  rpt_hdr( 'transdate', $locale->text('Date'), $href );
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{link} =
qq|$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
...
<tr><td>&nbsp;</td><td>
  <input type="button" class="check" value="|
	  . $locale->text('check all') . qq|" />
</td></tr>
...
sub income_statement_by_project {
	...
	$where .=
	  qq| AND ac.trans_id NOT IN (SELECT trans_id FROM yearend WHERE $ywhere)|;
...
qq|<body><table width=100%><tr><th class=listtop>$form->{title}</th></tr></table><br/>|;
	...
	my $dbh = $form->dbconnect( \%myconfig );
	my $query =
qq|SELECT id, projectnumber FROM project UNION SELECT 0, '(blank)' ORDER BY projectnumber|;
	my $sth = $dbh->prepare($query) || $form->dberror($query);
	$sth->execute || $form->dberror($query);
	...
	foreach my $key ( sort keys %projects ) {
		$is_query .=
qq|SUM(CASE WHEN ac.project_id = $projects{$key}->{id} THEN ac.amount ELSE 0 END) AS p_$projects{$key}->{id},\n|;
	}
  ...
  |;
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		$form->{ $ref->{category} }{ $ref->{accno} }{charttype} =
		  "$ref->{charttype}";
		$form->{ $ref->{category} }{ $ref->{accno} }{category} =
		  "$ref->{category}";
		$form->{ $ref->{category} }{ $ref->{accno} }{description} =
		  "$ref->{description}";
		for ( sort keys %projects ) {
			$form->{ $ref->{category} }{ $ref->{accno} }{$_} =
			  $ref->{"p_$projects{$_}->{id}"};
		}
	}
	$sth->finish;
	...
	foreach $accno ( sort keys %{ $form->{I} } ) {
		...
		for ( sort keys %projects ) {
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $form->{I}{$accno}{$_}, 0 )
			  . qq|</td>|;
			$form->{I}{$_}{totalincome} += $form->{I}{$accno}{$_};
			$line_total += $form->{I}{$accno}{$_};
		}
		print qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $line_total, 0 )
		  . qq|</td>|;
		print qq|</tr>|;
	}
...
qq|<tr><td colspan=2><b>|.$locale->text('EXPENSES').qq|<br><hr width=300 size=5 align=left noshade></b></td></tr>|;
	foreach $accno ( sort keys %{ $form->{E} } ) {
		...
		for ( sort keys %projects ) {
			print qq|<td align=right>|
			  . $form->format_amount( \%myconfig, $form->{E}{$accno}{$_} * -1,
				0 )
			  . qq|</td>|;
	...
	for ( sort keys %projects ) {
		print qq|<td align=right>|
		  . $form->format_amount( \%myconfig,
			$form->{I}{$_}{totalincome} - $form->{E}{$_}{totalexpenses}, 0 )
		  . qq|</td>|;
		$line_total +=
		  ( $form->{I}{$_}{totalincome} - $form->{E}{$_}{totalexpenses} );
	}
	print qq|<td align=right>|
	  . $form->format_amount( \%myconfig, $line_total, 0 )
	  . qq|</td>|;
...
sub income_statement_by_department {
	...
	$dbh->do(
"INSERT INTO dpt_trans (department_id, trans_id) SELECT 0, id FROM ar WHERE department_id = 0"
	);
	$dbh->do(
"INSERT INTO dpt_trans (department_id, trans_id) SELECT 0, id FROM ap WHERE department_id = 0"
	);
	$dbh->do(
"INSERT INTO dpt_trans (department_id, trans_id) SELECT 0, id FROM gl WHERE department_id = 0"
	);
	...
	$form->header;
	print
qq|<body class="bill main2"><table width=100%><tr><td class=listtop>$form->{title}</td></tr></table><br/>|;
	print qq|<h2 align=center>|
	  . $locale->text('Income Statement Departments')
	  . qq|</h2>|;
	print qq|<h2 align=center>| . $locale->text('For Period') . qq|</h2>|
  ...
  |;
	...
	for my $ref (@sorted) {
		if ( $form->{"p_$ref->{id}"} ) {
			...
			$is_query .=
qq|SUM(CASE WHEN d.department_id = $ref->{id} THEN ac.amount ELSE 0 END) AS p_$ref->{no},\n|;
		}
	}
...
# print qq|<tr><th colspan=| .(3 + (scalar keys %departments)). qq|><b>|.$locale->text('Income').qq|<br><hr width=300 size=5 align=left noshade></b></th></tr>|;
	...
	print qq|<tr><th colspan=2 align=left>|
	...
	# Print EXPENSES
	print qq|<tr><th colspan=|
	  . ( 3 + ( scalar keys %departments ) ) . qq|>|
	  . $locale->text('COGS')
	  . qq|</th></tr>|;
	foreach $accno ( sort keys %{ $form->{E} } ) {
		print qq|<tr>|;
...
|;
	...
	&print_checkbox( 'l_no', $locale->text('No.'), '', '<br>' );
	&print_checkbox( 'l_partnumber', $locale->text('Number'),
		'checked', '<br>' );
	&print_checkbox( 'l_category', $locale->text('Category'),
		'checked', '<br>' );
	&print_checkbox( 'l_description', $locale->text('Description'),
		'checked', '<br>' );
	&print_checkbox( 'l_onhand', $locale->text('Onhand'), 'checked', '<br>' );
	&print_checkbox( 'l_lastcost', $locale->text('Last Cost'),
		'checked', '<br>' );
	&print_checkbox( 'l_extended', $locale->text('Extended'),
		'checked', '<br>' );
...
sub aa_qty_list {
	...
	my $where = qq| (1 = 1)|;
	$where .= qq| AND (aa.transdate >= '$form->{fromdate}')|
	  if $form->{fromdate};
	...
	$where .= qq| AND (LOWER(p.partnumber) LIKE '$partnumber')|
	  if $form->{partnumber};
	$where .= qq| AND (LOWER(p.description) LIKE '$description')|
	  if $form->{description};
	...
	if ( $form->{sort} =~ /(onhand|lastcost|extended)/ ) {
		...
		for (
			sort {
				$parts{$a}->{ $form->{sort} } cmp $parts{$b}->{ $form->{sort} }
			} keys %parts
		  )
		{
			push @{ $form->{parts} }, $parts{$_};
	...
	$column_header{category} =
	  rpt_hdr( 'category', $locale->text('Category'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	$column_header{onhand} =
	  rpt_hdr( 'onhand', $locale->text('Onhand'), $href );
	$column_header{lastcost} =
	  rpt_hdr( 'lastcost', $locale->text('Last Cost'), $href );
	$column_header{extended} =
	  rpt_hdr( 'extended', $locale->text('Extended'), $href );
...
sub vc_search {
	...
	&print_checkbox( 'l_no', $locale->text('No.'), '', '<br>' );
	&print_checkbox( "l_$form->{vc}number", $locale->text('Number'),
		'checked', '<br>' );
...
sub vc_list {
	...
	$where .= qq| AND (LOWER("$form->{vc}number") LIKE '$vcnumber')|
	...
	$column_header{"$form->{vc}number"} =
	  rpt_hdr( "$form->{vc}number", $locale->text('Number'), $href );
	$column_header{name} = rpt_hdr( 'name', $locale->text('Name'), $href );
	$column_header{balance} =
	  rpt_hdr( 'balance', $locale->text('Balance'), $href );
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{link} =
qq|ct.pl?action=edit&db=$form->{vc}&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
		$groupbreak = $ref->{ $form->{sort} } if $groupbreak eq 'none';
		if ( $form->{l_subtotal} ) {
			if ( $groupbreak ne $ref->{ $form->{sort} } ) {
				$groupbreak = $ref->{ $form->{sort} };
...
sub vcactivity_search {
	...
	&print_checkbox( 'l_invnumber', $locale->text('Invoice Number'),
		'checked', '<br>' );
	&print_checkbox( 'l_description', $locale->text('Description'),
		'checked', '<br>' );
...
sub vcactivity_list {
	...
	@columns = (
		"id",        "$form->{vc}number", "name",  "transdate",
		"invnumber", "description",       "debit", "credit",
		"balance"
	);
	...
	$column_header{transdate} =
	  rpt_hdr( 'transdate', $locale->text('Date'), $href );
	$column_header{invnumber} =
	  rpt_hdr( 'invnumber', $locale->text('Invoice Number'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	$column_header{debit} = rpt_hdr( 'debit', $locale->text('Debit'), $href );
	$column_header{credit} =
	  rpt_hdr( 'credit', $locale->text('Credit'), $href );
	$column_header{balance} =
	  rpt_hdr( 'balance', $locale->text('Balance'), $href );
...
sub onhand_list {
	...
	# callback to report list
	my $callback =
qq|$form->{script}?action=onhand_list&path=$form->{path}&login=$form->{login}|;

	# link to drill down to inventory activity and columns to be displayed
	my $link =
qq|$form->{script}?action=iactivity_list&path=$form->{path}&login=$form->{login}|;
	for (
		qw(l_no l_shippingdate l_reference l_department l_warehouse l_warehouse2 l_unit l_in l_out l_onhand l_subtotal)
	  )
	{
		$link .= "&$_=Y";
	}
	...
	for (
		qw(summary l_subtotal department warehouse partsgroup partnumber description dateto)
	  )
	{
		$callback .= "&$_=" . $form->escape( $form->{$_} );
	}
	...
	$column_header{warehouse} =
	  rpt_hdr( 'warehouse', $locale->text('Warehouse'), $href );
	$column_header{partnumber} =
	  rpt_hdr( 'partnumber', $locale->text('Number'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{link} =
qq|ic.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
		$groupbreak = $ref->{ $form->{sort} } if $groupbreak eq 'none';
		if ( $form->{l_subtotal} ) {
			if ( $groupbreak ne $ref->{ $form->{sort} } ) {
				$groupbreak = $ref->{ $form->{sort} };
		...
		if ( $form->{summary} ) {
			$column_data{partnumber} = rpt_txt( $ref->{partnumber},
"$link&partnumber=$ref->{partnumber}&warehouse=$form->{warehouse}"
			);
		}
		else {
			$column_data{partnumber} = rpt_txt( $ref->{partnumber},
"$link&partnumber=$ref->{partnumber}&warehouse=$ref->{warehouse}--$ref->{warehouse_id}"
			);
		}
		$column_data{description} = rpt_txt( $ref->{description} );
		$column_data{partsgroup}  = rpt_txt( $ref->{partsgroup} );
...
sub iactivity_search {
	...
	&print_checkbox( 'l_warehouse', $locale->text('Warehouse'), 'checked', '' );
	&print_checkbox( 'l_warehouse2', $locale->text('Warehouse2'),
		'checked', '<br>' );
	&print_checkbox( 'l_partnumber', $locale->text('Number'), 'checked', '' );
	&print_checkbox( 'l_description', $locale->text('Description'), '', '' );
	&print_checkbox( 'l_unit',   $locale->text('Unit'),       'checked', '' );
	...
	&print_checkbox( 'l_cogs_balance', $locale->text('Cost Balance'),
		'checked', '<br>' );
...
sub iactivity_list {
	...
	@columns =
	  qw(partnumber description id shippingdate reference department warehouse warehouse2 in out onhand cost cogs cogs_balance);
	...
	for (
		qw(l_subtotal partnumber datefrom dateto partsgroup department warehouse)
	  )
	{
		$callback .= "&$_=" . $form->escape( $form->{$_} );
	}
	...
	$column_header{no} = rpt_hdr( 'no', $locale->text('No.') );
	$column_header{shippingdate} =
	  rpt_hdr( 'shippingdate', $locale->text('Date'), $href );
	$column_header{reference} =
	  rpt_hdr( 'reference', $locale->text('Reference'), $href );
	$column_header{department} =
	  rpt_hdr( 'department', $locale->text('Department'), $href );
	$column_header{warehouse} =
	  rpt_hdr( 'warehouse', $locale->text('Warehouse'), $href );
	$column_header{warehouse2} =
	  rpt_hdr( 'warehouse2', $locale->text('Warehouse2'), $href );
	$column_header{partnumber} =
	  rpt_hdr( 'partnumber', $locale->text('Number'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	...
	$column_header{cogs} =
	  rpt_hdr( 'cogs', $locale->text('Total Cost'), $href );
	$column_header{cogs_balance} =
	  rpt_hdr( 'cogs_balance', $locale->text('Cost Balance'), $href );
...
qq|$form->{script}?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&sessionid=$form->{sessionid}&callback=$form->{callback}|;
		...
		$column_data{reference}    = rpt_txt(
			$ref->{reference} . $ref->{ap_reference} . $ref->{ar_reference} );
...
sub trans_search {
	...
	&print_text( "${db}number", $locale->text( ( ucfirst $db ) . ' Number' ),
		15 );
	&print_text( 'name',        $locale->text('Name'),           30 );
...
|;
	...
	@a = ();
	push @a, qq|<input name="invoices" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Invoices');
	push @a, qq|<input name="trans" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Transactions');

	push @a, qq|<input name="l_no" class=checkbox type=checkbox value=Y> |
	  . $locale->text('No.');
	push @a,
qq|<input name="l_${db}number" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Number');
	push @a,
...
qq|<input name="l_sellprice" class=checkbox type=checkbox value=Y checked> |
	...
	push @a, qq|<input name="l_tax" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Tax');
	push @a, qq|<input name="l_total" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Total');

	if ( $form->{aa} eq 'AR' ) {
		push @a,
qq|<input name="l_cogs" class=checkbox type=checkbox value=Y checked> |
		  . $locale->text('COGS');
		push @a,
qq|<input name="l_markup" class=checkbox type=checkbox value=Y checked> |
		  . $locale->text('Markup %');
	}
	push @a,
qq|<input name="l_employee" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text($employee_caption);
	push @a, qq|<input name="l_subtotal" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Subtotal');
	push @a,
	  qq|<input name="l_subtotalonly" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Subtotal Only');
	push @a, qq|<input name="l_csv" class=checkbox type=checkbox value=Y> |
	  . $locale->text('CSV');
	push @a, qq|<input name="l_sql" class=checkbox type=checkbox value=Y> |
	  . $locale->text('SQL');
...
sub trans_list {
	...
	$where .= qq| AND (aa.department_id = $form->{department_id})|
	  if $form->{department};
	$where .= qq| AND (aa.warehouse_id = $form->{warehouse_id})|
	  if $form->{warehouse};
	$where .= qq| AND (aa.employee_id = $form->{employee_id})|
	  if $form->{employee};
	$where .= qq| AND (LOWER(ct.${db}number) LIKE '$vcnumber')|
	  if $form->{"${db}number"};
	$where .= qq| AND (LOWER(ct.name) LIKE '$name')| if $form->{name};
	$where .= qq| AND (LOWER(aa.invnumber) LIKE '$invnumber')|
	  if $form->{invnumber};
	$where .= qq| AND (LOWER(aa.description) LIKE '$description')|
	  if $form->{description};
	...
	@columns = (
		qw(id invnumber transdate customernumber vendornumber name partnumber description qty sellprice amount tax total cogs markup employee)
	);
	...
	for (qw(aa l_subtotal l_subtotalonly summary)) {
		$callback .= "&$_=$form->{$_}";
	}
	for (
		qw(customernumber vendornumber name invnumber description notes fromdate todate partnumber department warehouse partsgroup employee)
	  )
	{
		$callback .= "&$_=" . $form->escape( $form->{$_}, 1 );
	}
	...
	$column_header{invnumber} =
	  rpt_hdr( 'invnumber', $locale->text('Invoice Number'), $href );
	$column_header{transdate} =
	  rpt_hdr( 'transdate', $locale->text('Invoice Date'), $href );
	$column_header{"${db}number"} =
	  rpt_hdr( "${db}number", $locale->text('Number'), $href );
	$column_header{name} = rpt_hdr( 'name', $locale->text('Name'), $href );
	$column_header{partnumber} =
	  rpt_hdr( 'partnumber', $locale->text('Number'), $href );
	$column_header{description} =
	...
	$column_header{amount} =
	  rpt_hdr( 'amount', $locale->text('Amount'), $href );
	...
	$column_header{employee} =
	  rpt_hdr( 'employee', $locale->text($employee_caption), $href );
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		$form->{link} =
qq|$module?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
		$groupbreak = $ref->{ $form->{sort} } if $groupbreak eq 'none';
		if ( $form->{l_subtotal} ) {
			if ( $groupbreak ne $ref->{ $form->{sort} } ) {
				...
				$column_data{ $form->{sort} } = rpt_txt($oldgroupbreak)
				  if $form->{l_subtotalonly};
				my $markup = 0;
				if ( $amount_subtotal > 0 ) {
					$markup =
					  ( ( $amount_subtotal - $cogs_subtotal ) * 100 ) /
					  $amount_subtotal;
				}
		...
		$column_data{cogs}          = rpt_dec( $ref->{cogs} );
		if ( $ref->{amount} > 0 ) {
			$column_data{markup} = rpt_dec(
				( ( $ref->{amount} - $ref->{cogs} ) * 100 ) / $ref->{amount} );
		}
		else {
	...
	# Print subtotal value of sorted column as heading
	$column_data{ $form->{sort} } = rpt_txt($groupbreak)
	  if $form->{l_subtotalonly};
...
sub build_search {
	...
	&print_checkbox( 'l_department', $locale->text('Warehouse'), 'checked',
		'' );
	&print_checkbox( 'l_warehouse', $locale->text('Warehouse'),
		'checked', '<br />' );
	&print_checkbox( 'l_partnumber', $locale->text('Number'), 'checked', '' );
	&print_checkbox( 'l_description', $locale->text('Description'),
		'checked', '' );
...
sub build_list {
	...
	my $where = qq| (1 = 1)|;
	$where .= qq| AND (LOWER(b.reference) LIKE '$reference')|
	  if $form->{reference};

#$where .= qq| AND (p.partsgroup_id = $form->{partsgroup_id})| if $form->{partsgroup};
	$where .= qq| AND (b.department_id = $form->{department_id})|
	  if $form->{department};
	$where .= qq| AND (b.warehouse_id = $form->{warehouse_id})|
	  if $form->{warehouse};
	$where .= qq| AND (b.transdate >= '$form->{datefrom}')|
	  if $form->{datefrom};
	$where .= qq| AND (b.transdate <= '$form->{dateto}')| if $form->{dateto};
	$where .= qq| AND (LOWER(p.partnumber) LIKE '$partnumber')|
	  if $form->{partnumber} and !$form->{summary};

	@columns =
	  qw(id reference transdate department warehouse partnumber description qty unit);
...
sub projects_search {
	...
	&print_checkbox( 'l_no', $locale->text('No.'), '', '' );
	&print_checkbox( 'l_projectnumber', $locale->text('Project Number'),
		'checked', '' );
	&print_checkbox( 'l_description', $locale->text('Description'),
		'checked', '' );
...
sub projects_list {
	...
	my $where = qq| (1 = 1)|;
	my $subwhere;
	$where .= qq| AND (LOWER(p.projectnumber) LIKE '$projectnumber')|
	  if $form->{projectnumber};
	$where .= qq| AND (LOWER(p.description) LIKE '$description')|
	  if $form->{description};

	$subwhere .= qq| AND (ac.transdate >= '$form->{datefrom}')|
	  if $form->{datefrom};
	$subwhere .= qq| AND (ac.transdate <= '$form->{dateto}')|
	  if $form->{dateto};
	$subwhere .=
qq| AND (ac.trans_id IN (SELECT trans_id FROM dpt_trans WHERE department_id = $form->{department_id}))|
	  if $form->{department};

	@columns =
	  qw(id projectnumber description startdate enddate income expenses net);
	...
	$column_header{projectnumber} =
	  rpt_hdr( 'projectnumber', $locale->text('Number'), $href );
	$column_header{description} =
	  rpt_hdr( 'description', $locale->text('Description'), $href );
	$column_header{startdate} =
	  rpt_hdr( 'startdate', $locale->text('Startdate'), $href );
	$column_header{enddate} =
	  rpt_hdr( 'enddate', $locale->text('Enddate'), $href );
	$column_header{income} =
	  rpt_hdr( 'income', $locale->text('Income'), $href );
	$column_header{expenses} =
	  rpt_hdr( 'expenses', $locale->text('Expenses'), $href );
...
qq|rp.pl?action=continue&nextsub=generate_projects&projectnumber=$ref->{projectnumber}--$ref->{id}|;
		...
		if ( $form->{l_subtotal} ) {
			if ( $groupbreak ne $ref->{ $form->{sort} } ) {
				...
				$column_data{net} =
				  rpt_dec( $income_subtotal - $expenses_subtotal );
		...
		$column_data{projectnumber} =
		  rpt_txt( $ref->{projectnumber}, $form->{link} );
		$column_data{description} = rpt_txt( $ref->{description} );
...
qq|<input name="l_invnumber" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Invoice Number');
	push @a,
qq|<input name="l_ordnumber" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Order Number');
	push @a,
	  qq|<input name="l_duedate" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Due Date');
	push @a,
	  qq|<input name="l_due" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Due');
...
sub list_reminders {
	...
	if ( $form->{"$form->{vc}number"} ne "" ) {
		...
		$options .= $locale->text('Customer Number')
		  . qq| : $form->{"$form->{vc}number"}<br/>|;
	}
	if ( $form->{ $form->{vc} } ne "" ) {
  ...
  |;
	@columns = (
		qw(transdate reference name customernumber invnumber ordnumber duedate due)
	);
	$form->sort_order();
	...
	$column_data{transdate} =
	    qq|<th width=90%><a class=listheading>|
	  . $locale->text('Date')
	  . qq|</a></th>|;
	$column_data{reference} =
	  qq|<th class=listheading nowrap>| . $locale->text('Level') . qq|</th>|;
	$column_data{name} =
	  qq|<th class=listheading nowrap>| . $locale->text('Customer') . qq|</th>|;
	$column_data{customernumber} =
	    qq|<th class=listheading nowrap>|
	  . $locale->text('Customer Number')
	  . qq|</th>|;
...
|;
		...
		$column_data{due} =
		    qq|<td align="right">|
		  . $form->format_amount( \%myconfig, $ref->{due}, $form->{precision} )
		  . qq|</td>|;
