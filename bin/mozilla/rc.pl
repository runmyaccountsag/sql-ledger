...
sub reconciliation {
	...
	if ( @{ $form->{all_years} } ) {
	  ...
	  <input name=interval class=radio type=radio value=0>&nbsp;|
		  . $locale->text('Current') . qq|
	  <input name=interval class=radio type=radio value=1 checked>&nbsp;|
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td colspan=3><input name=fromdate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"> <b>|
	  . $locale->text('To')
...
<li>|
	  . $locale->text(
		'Leave from/to dates blank to get all un-reconciled transactions.')
	  . qq|</li>
<li>|
	  . $locale->text(
'Specify from/to dates to display both reconciled and un-reconciled transactions for that period.'
	  )
	  . qq|</li>
<li>|
	  . $locale->text(
'You can also un-check already reconciled transactions by specifying from/to dates.'
	  )
	  . qq|</li>
<li>|
	  . $locale->text(
'Summary report groups and totals transactions with same date+source. Detail report shows all individual transactions.'
	  )
	  . qq|</li>
<li>|
	  . $locale->text(
qq|Check 'Ignore Difference' box to ignore statement balance difference and save reconciled transactions.|
	  )
	  . qq|</li>
</ol>
|;
...
sub get_payments {
	...
	$form->{statementbalance} =
	  $form->format_amount( \%myconfig, $form->{statementbalance},
		$form->{precision}, 0 );
...
|;
	...
	if ( !$form->{report} ) {
		...
		$column_data{balance} =
		    "<td align=right>"
		  . $form->format_amount( \%myconfig, $balance, $form->{precision}, 0 )
		  . "</td>";
		print qq|
	<tr class=listrow$j>
|;
    ...
    <input type=hidden name="payment_id_$i" value="$ref->{payment_id}">|;
		...
		if ( $ref->{fx_transaction} ) {
			...
			if ( $form->{report} ) {
				...
				$clearfx = ($checked) ? 1 : 0;
				$column_data{cleared} =
qq|<td align=center><input name="cleared_$i" type=checkbox class=checkbox value=1 $checked></td>
	<input type=hidden name="source_$i" value="|
				  . $form->quote( $ref->{source} ) . qq|">
...
|;
	...
	$column_data{debit} = "<th class=listtotal align=right>"
	  . $form->format_amount( \%myconfig, $totaldebits, $form->{precision},
		"&nbsp;" )
	  . "</th>";
	$column_data{credit} = "<th class=listtotal align=right>"
	  . $form->format_amount( \%myconfig, $totalcredits, $form->{precision},
		"&nbsp;" )
	  . "</th>";
...
|;
		...
		$form->hide_form(
			qw(recdate fx_transaction summary rowcount accno account fromdate todate path login)
		);
...
sub done {
	...
	$form->{callback} =
"$form->{script}?path=$form->{path}&action=reconciliation&login=$form->{login}";
