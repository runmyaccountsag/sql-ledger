...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&type=$form->{type}&login=$form->{login}&path=$form->{path}&project=$form->{project}"
...
|;
	...
	<tr>
	  ...
	  <td>|
	  . $locale->text('From')
	  . qq| <input name=startdatefrom size=11 title="$myconfig{dateformat}" onChange="validateDate(this)">
	  |
	  . $locale->text('To')
	  . qq| <input name=startdateto size=11 title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	</tr>
	$selectfrom
|;
	...
	if ( $form->{project} eq 'job' ) {
		...
		if ( $form->{type} ) {
			...
			$partnumberlabel =
			  $locale->text('Part Number') . "/" . $locale->text('Labor Code');
		}
		...
		$projectnumberlabel =
		  $locale->text('Project Number') . "/" . $locale->text('Job Number');
		$partnumberlabel =
		  $locale->text('Service Code') . "/" . $locale->text('Labor Code');
		$projectdescriptionlabel =
		  $locale->text('Project Name') . "/" . $locale->text('Job Name');
...
|;
		...
		$l_time =
		  qq|<input name=l_time class=checkbox type=checkbox value=Y>&nbsp;|
		  . $locale->text('Time');
...
qq|<input name="l_transdate" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Date');
	push @a,
qq|<input name="l_projectnumber" class=checkbox type=checkbox value=Y checked> $projectnumberlabel|;
	push @a,
qq|<input name="l_projectdescription" class=checkbox type=checkbox value=Y checked> $projectdescriptionlabel|;
	push @a,
	  qq|<input name="l_id" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('ID');
	push @a,
qq|<input name="l_partnumber" class=checkbox type=checkbox value=Y checked> $partnumberlabel|;
	push @a,
qq|<input name="l_description" class=checkbox type=checkbox value=Y checked> |
	...
	push @a, qq|<input name=l_allocated class=checkbox type=checkbox value=Y> |
...
sub prepare_timecard {
	...
	$form->{allocated} =
	  $form->format_amount( \%myconfig, $form->{allocated}, 4 );
...
qq|<textarea name=description rows=$rows cols=46 wrap=soft>$form->{description}</textarea>|;
		...
		$description = qq|<input name=description size=48 value="|
...
|;
	...
	$form->hide_form( map { "select$_" }
		  qw(projectnumber employee formname language) );
	$form->hide_form(
		qw(id type printed queued title closedto locked project pricematrix parts_id precision)
	);
	$form->hide_form( map { "old$_" }
		  qw(transdate checkedin checkedout partnumber qty noncharge project_id)
	);
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  . $form->hide_form(qw(projectdescription)) . qq|
	</tr>
	<tr>
	  <th align=right nowrap>|
	  . $locale->text('Date worked')
	  . qq| <font color=red>*</font></th>
...
|;
	...
	$form->hide_form(
		qw(id type printed queued title closedto locked project parts_id employee precision)
	);
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  . qq| <font color=red>*</font></th>
	  <td colspan=3><input name=partnumber value="|
	  . $form->quote( $form->{partnumber} ) . qq|">
	  </td>
	</tr>
	<tr valign=top>
	...
	if ( !$form->{readonly} ) {
		...
		if ( $form->{id} ) {
			...
			if ( !$form->{locked} ) {
				for ( 'Update', 'Print', 'Save', 'Save as new' ) { $a{$_} = 1 }
				if ($latex) {
					for ( 'Print and Save', 'Print and Save as new' ) {
						$a{$_} = 1;
					}
				}
				if ( $form->{orphaned} ) {
...
sub update {
	...
	if ( $form->{oldpartnumber} ne $form->{partnumber} ) {
		$form->error( $locale->text('Project/Job Number missing!') )
		  if !$form->{project};
		if ( $form->{project} eq 'project' ) {
			$form->error( $locale->text('Project Number missing!') )
			  if !$form->{projectnumber};
		}
		if ( $form->{project} eq 'job' ) {
			$form->error( $locale->text('Job Number missing!') )
			  if !$form->{projectnumber};
		}
...
sub list_cards {
	...
	@columns = $form->sort_columns(
		qw(transdate id projectnumber projectname partnumber description notes)
	);
	...
	if ( $form->{type} eq 'timecard' ) {
		push @column_index, (qw(2 3 4 5 6 7 1))
		  if ( $form->{l_qty} || $form->{l_time} );
	}
	else {
	...
	if ( $form->{project} eq 'job' ) {
		...
		$laborlabel =
		  $locale->text('Service Code') . "/" . $locale->text('Labor Code');
		$desclabel =
		    $locale->text('Project Description') . "/"
		  . $locale->text('Job Name');
	}
	...
	if ( $form->{projectnumber} ) {
		...
		$option .= "\n<br>" if ($option);
		$option .= "$joblabel : $var";
		@column_index = grep !/(projectnumber|projectdescription)/,
		  @column_index;
		$option .= "\n<br>$desclabel : "
		  . $form->{transactions}->[0]->{projectdescription};
	}
	if ( $form->{partnumber} ) {
	...
	if ( $form->{type} eq 'timecard' ) {
		...
		for ( keys %weekday ) {
			$column_header{$_} = "<th class=listheading width=25>"
...
"<th><a class=listheading href=$href&sort=projectdescription>$desclabel</a></th>";
	...
	$column_header{allocated} =
	  "<th class=listheading>" . $locale->text('Allocated') . "</th>";
...
|;
					...
					$column_data{ $form->{sort} } =
					  ...
					  . $form->format_amount( \%myconfig, $weektotal, undef,
						"&nbsp;" )
					  . "</th>";
...
|;
				...
				$total = 0;
				for ( keys %weekday ) {
					$column_data{$_} = "<th class=listtotal align=right>"
					  . $form->format_amount( \%myconfig, $total{$_}, undef,
						"&nbsp;" )
					  . "</th>";
					$total += $total{$_};
					$total{$_} = 0;
				...
				$column_data{ $form->{sort} } =
				  ...
				  . $form->format_amount( \%myconfig, $total, undef, "&nbsp;" )
				  . "</th>";
...
|;
				...
				if ( $sameitem ne $ref->{ $form->{sort} } ) {
					$column_data{qty} = "<th class=listsubtotal align=right>"
					  . $form->format_amount( \%myconfig, $subtotal{qty}, undef,
						"&nbsp;" )
					  . "</th>";
					$column_data{sellprice} =
					  "<th class=listsubtotal align=right>"
					  . $form->format_amount( \%myconfig, $subtotal{sellprice},
						$form->{precision} )
					  . "</th>";
...
|;
		...
		$column_data{allocated} =
		  "<td align=right>"
		  . $form->format_amount( \%myconfig, $ref->{allocated}, undef,
			"&nbsp;" )
		  . "</td>";
		...
		if ( $form->{l_time} ) {
			...
			$column_data{ $ref->{weekday} } .=
			  "$ref->{checkedin}<br>$ref->{checkedout}";
		}
		$column_data{ $ref->{weekday} } .= "</td>";

		$column_data{id} =
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&type=$ref->{type}&path=$form->{path}&login=$form->{login}&project=$ref->{project}&callback=$callback>$ref->{id}</a></td>";
...
|;
		...
		if ( $form->{type} eq 'timecard' ) {
			...
			for ( keys %weekday ) {
				$column_data{$_} = "<th class=listsubtotal align=right>"
				  . $form->format_amount( \%myconfig, $subtotal{$_}, undef,
					"&nbsp;" )
				  . "</th>";
				$weektotal += $subtotal{$_};
			}
			...
			$column_data{qty} = "<th class=listsubtotal align=right>"
			  . $form->format_amount( \%myconfig, $subtotal{qty}, undef,
				"&nbsp;" )
			  . "</th>";
			$column_data{sellprice} = "<th class=listsubtotal align=right>"
			  . $form->format_amount( \%myconfig, $subtotal{sellprice},
				$form->{precision} )
			  . "</th>";
		}
...
|;
	...
	if ( $form->{type} eq 'timecard' ) {
		...
		$column_data{qty} =
		    "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $total{qty}, undef, "&nbsp;" )
		  . "</th>";
		$column_data{sellprice} = "<th class=listtotal align=right>"
		  . $form->format_amount( \%myconfig, $total{sellprice},
			$form->{precision} )
		  . "</th>";
		...
		if ( $form->{type} eq 'timecard' ) {
			...
			if ( $myconfig{acs} !~ /Production--Add Stores Card/ ) {
				...
				$button{'Production--Add Stores Card'}{code} =
				  qq|<input class=submit type=submit name=action value="|
				  . $locale->text('Add Stores Card') . qq|"> |;
...
sub print_form {
	...
	if ( $form->{type} eq 'timecard' ) {
		...
		foreach $item (qw(in out)) {
			for (@a) {
				$form->{"$item$_"} = substr( qq|00$form->{"$item$_"}|, -2 );
			}
			$form->{"checked$item"} =
qq|$form->{"${item}hour"}:$form->{"${item}min"}:$form->{"${item}sec"}|;
	...
	push @a,
	  qw(companyemail companywebsite company address tel fax businessnumber username useremail);
...
|;
	...
	foreach $ref ( @{ $form->{item_list} } ) {
		...
		$column_data{ndx} =
		  qq|<td><input name="ndx" class=radio type=radio value=$i></td>|;
...
sub item_selected {
	...
	# format amounts
	$form->{sellprice} =
	  $form->format_amount( \%myconfig, $form->{sellprice}, $decimalplaces );
...
sub new_item {
	...
	for (qw(partnumber description previousform)) {
	...
	if ( $form->{type} eq 'timecard' ) {
		if ( $form->{project} eq 'project' ) {
			$form->error(
				$locale->text('You are not authorized to add a new item!') )
