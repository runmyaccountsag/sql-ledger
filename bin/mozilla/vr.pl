...
<table width=100%>
  ...
  <tr valign=top>
	...
	<tr>
	  ...
	  <td><input name=batchdescription size=40 value="|
	  . $form->quote( $form->{batchdescription} )
	  . qq|"></td>
	</tr>
	$transdate
      </table>
...
<input class=submit type=submit name=action value="|
	...
	$form->hide_form(
		qw(path login nextsub type batch batchid batchnumber callback));
...
sub payments_batch {
	...
	$form->{callback} =
"$form->{script}?action=add_payments_batch&batch=payment&path=$form->{path}&login=$form->{login}";
...
sub edit_payment_reversal {
	...
	if ( @{ $form->{all_accounts} } ) {
		for ( @{ $form->{all_accounts} } ) {
			$form->{selectaccount} .= qq|$_->{accno}--$_->{description}\n|;
		}
	}
	else {
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  . $form->select_option( $form->{selectaccount},
		$form->{ $form->{account} } )
	  . qq|</select>
	  </td>
	</tr>
...
|;
	...
	if ( !$form->{id} ) {
		...
		for (qw(transdate memo path login batch batchid batchnumber)) {
			...
			$form->{callback} .= "&batchdescription="
			  . $form->escape( $form->{batchdescription}, 1 );
		}
	}

	$form->hide_form(
		qw(id vouchernumber transdate memo path login batch batchid batchnumber batchdescription callback)
	);

	%button = (
		'Delete' => { ndx => 1, key => 'D', value => $locale->text('Delete') },
...
sub post {
	...
	if ( VR->post_payment_reversal( \%myconfig, \%$form ) ) {
		...
		$form->{callback} .=
		  "&batch=$form->{batch}&batchid=$form->{batchid}&batchdescription="
		  . $form->escape( $form->{batchdescription}, 1 );
...
<h4>|
	  . $locale->text('Are you sure you want to delete Batch')
	  . qq| $form->{batchnumber}</h4>
...
sub search {
  ...
  |;
	...
	$l_employee =
qq|<input name="l_employee" class=checkbox type=checkbox value=Y checked> $employeelabel|;
...
|;
	...
	push @a,
	  ...
	  . $locale->text('No.');
	push @a, qq|<input name="l_batchid" class=checkbox type=checkbox value=Y> |
	  . $locale->text('ID');
	push @a,
qq|<input name="l_batchnumber" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Batch Number');
	push @a,
...
qq|<input name="l_transdate" class=checkbox type=checkbox value=Y checked> |
	  . $locale->text('Posting Date');
	push @a, qq|<input name="l_apprdate" class=checkbox type=checkbox value=Y> |
	  . $locale->text('Approved');
	push @a, $l_employee;
	push @a, $l_manager;
...
sub list_batches {
	...
	for (qw(direction oldsort path login batch)) {
		$href .= qq|&$_=$form->{$_}|;
	}
	...
	@columns = $form->sort_columns(
		qw(batchid batchnumber description transdate employee apprdate manager amount)
	);
	...
	$column_header{transdate} =
	  ...
	  . $locale->text('Posting Date')
	  . "</a></th>";
	$column_header{apprdate} =
	    "<th><a class=listheading href=$href&sort=apprdate>"
	  . $locale->text('Approved')
	  . "</a></th>";
	$column_header{batchnumber} =
	    "<th><a class=listheading href=$href&sort=batchnumber>"
	  . $locale->text('Batch Number')
	  . "</a></th>";
...
|;
	...
	foreach $ref ( @{ $form->{transactions} } ) {
		...
		for (qw(batch batchnumber description)) {
			$form->{"${_}_$i"} = $ref->{$_};
		}
		...
		if ( $ref->{apprdate} ) {
			$column_data{ndx} =
qq|<td><input type=hidden name="batchid_$i" value=$ref->{id}>&nbsp;</td>|;
		}
		else {
			$column_data{ndx} =
qq|<td><input name="checked_$i" class=checkbox type=checkbox $checked></td><input type=hidden name="batchid_$i" value=$ref->{id}>|;
		}

		$column_data{amount} = "<td align=right>"
		...
		$column_data{batchnumber} =
"<td><a href=$form->{script}?action=list_vouchers&batchid=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{batchnumber}</a></td>";

		for (qw(description)) { $ref->{$_} =~ s/\r?\n/<br>/g }
		for (qw(transdate apprdate)) {
			$column_data{$_} = "<td nowrap>$ref->{$_}&nbsp;</td>";
		}
		for (qw(description employee manager)) {
	...
	if ( $myconfig{acs} !~ /Vouchers--Vouchers/ ) {
		%button = (
			'ap' => {
				ndx   => 2,
				key   => 'A',
				value => $locale->text('Add Payable Batch')
			},
			'gl' => {
				ndx   => 3,
				key   => 'G',
				value => $locale->text('Add General Ledger Batch')
			},
			'payment' => {
				ndx   => 4,
				key   => 'P',
				value => $locale->text('Add Payment Batch')
			},
			'payment_reversal' => {
				ndx   => 5,
				key   => 'P',
				value => $locale->text('Add Payment Reversal Batch')
			}
		);
		...
		if ( $form->{unselect} ) {
			$button{'Deselect all'} =
			  { ndx => 1, key => 'S', value => $locale->text('Deselect all') };
		}
		else {
			$button{'Select all'} =
			  { ndx => 1, key => 'S', value => $locale->text('Select all') };
		}

		$button{'Post Batches'} =
		  { ndx => 5, key => 'O', value => $locale->text('Post Batches') };
...
sub list_vouchers {
	...
	for (qw(batchid direction oldsort path login)) {
		$href .= qq|&$_=$form->{$_}|;
	}
	...
	for (qw(direction batchid oldsort path login)) {
		$callback .= qq|&$_=$form->{$_}|;
	}
	...
	if ( $form->{apprdate} ) {
		...
		$option =
qq|<a href="$form->{script}?action=edit_batch&login=$form->{login}&path=$form->{path}&batchid=$form->{batchid}&batch=$form->{batch}&title=$title&callback=$callback">|
		  . $locale->text('Batch Number')
		  . " : $form->{batchnumber}</a>";
	}
	...
	$column_header{id} =
	  ...
	  . $locale->text('ID')
	  . "</a></th>";
	$column_header{vouchernumber} =
	    "<th><a class=listheading href=$href&sort=vouchernumber>"
	  . $locale->text('Voucher Number')
	  . "</a></th>";
...
"<th><a class=listheading href=$href&sort=$form->{vc}number>$vcnumber</a></th>";
...
|;
	...
	foreach $ref ( @{ $form->{transactions} } ) {
		...
		if ( $form->{readonly} ) {
			...
			$column_data{vouchernumber} =
qq|<td><a href=$module{$form->{batch}}.pl?action=edit&transdate=$form->{transdate}&type=$form->{type}&batch=$form->{batch}&batchid=$form->{batchid}&id=$ref->{id}&$form->{vc}_id=$ref->{"$form->{vc}_id"}&path=$form->{path}&login=$form->{login}&callback=$callback&batchdescription=$batchdescription>$ref->{vouchernumber} </a></td>|;
		}
		...
		if ( $myconfig{acs} !~ /Vouchers--Vouchers/ ) {
			...
			if ( !$form->{apprdate} ) {
				$button{'Add Voucher'} = {
					ndx   => 1,
					key   => 'A',
					value => $locale->text('Add Voucher')
				};
				if ( $myconfig{role} ne 'user' ) {
					$button{'Post Batch'} = {
						ndx   => 2,
						key   => 'O',
						value => $locale->text('Post Batch')
					};
				}
			}

			$button{'Delete Batch'} =
			  { ndx => 3, key => 'D', value => $locale->text('Delete Batch') };
...
|;
	...
	$form->hide_form(
		qw(transdate batchid batchnumber batchdescription batch callback path login)
	);
...
sub add_voucher {
	...
	$form->{callback} =
	  "$module{$form->{batch}}.pl?action=$sub{$form->{batch}}";
...
sub post_batches {
	...
	for $i ( 1 .. $form->{rowcount} ) {
		if ( $form->{"checked_$i"} ) {
			...
			$form->info( $locale->text('Posting Batch')
				  . qq| $form->{"batchnumber_$i"}| );
