...
sub invoice_links {
	...
	for (
		qw(partsgroup projectnumber department warehouse employee language paymentmethod)
	  )
	{
...
"$form->{acc_trans}{$key}->[0]->{accno}--$form->{acc_trans}{$key}->[0]->{description}"
	...
	if ( $form->{id} ) {
		%title = (
			invoice       => $locale->text('Edit Vendor Invoice'),
			debit_invoice => $locale->text('Edit Debit Invoice')
		);
	}
	else {
		%title = (
			invoice       => $locale->text('Add Vendor Invoice'),
...
|;
	...
	$form->hide_form(
		qw(id type printed emailed queued title vc creditlimit creditremaining business closedto locked shipped oldtransdate oldduedate recurring defaultcurrency duedate oldterms cdt precision order_id)
	);

	$form->hide_form( map { "select$_" }
		  ( "$form->{vc}", "AP", "AP_paid", "AP_discount" ) );
	$form->hide_form( map { "select$_" }
...
qq|<input name="discount_exchangerate" size=10 value=$form->{"discount_exchangerate"}>|
		...
		$column_data{paid} =
		  ...
		  . qq|></td>|;
		$column_data{AP_paid} =
		  qq|<td align=center><select name="AP_discount_paid">|
		  . $form->select_option( $form->{"selectAP_discount"},
			$form->{"AP_discount_paid"} )
		  . qq|</select></td>|;
...
|;
	...
	$form->{"AP_paid_$form->{paidaccounts}"} =
...
qq|<td align=center><input name="paid_$i" size=11 value=$form->{"paid_$i"}></td>|;
		$column_data{exchangerate} = qq|<td align=center>$exchangerate</td>|;
		$column_data{AP_paid} = qq|<td align=center><select name="AP_paid_$i">|
		  . $form->select_option( $form->{"selectAP_paid"},
			$form->{"AP_paid_$i"} )
		  . qq|</select></td>|;
		$column_data{datepaid} =
...
|;
	...
	if ( $form->{readonly} ) {
		...
		if ( $form->{id} ) {
			...
			delete $button{'Purchase Order'}
			  if $myconfig{acs} =~
				  /(Order Entry--Order Entry|Order Entry--Purchase Order)/;
			...
			if ( $transdate > $form->{closedto} ) {
				for (
					'Update', "Ship to",  "Print", "E-mail",
					'Post',   'Schedule', 'New Number'
				  )
				{
					$a{$_} = 1;
				}
...
sub update {
	...
	$form->{payment_accno} =
	  $form->escape( $form->{"AP_paid_$form->{paidaccounts}"}, 1 );
	$form->{payment_method} =
...
sub post {
	...
	if ( $form->{discount_paid} ) {
		...
		if ( $form->{"paid_$i"} ) {
			...
			$form->isblank( "datepaid_$i",
				$locale->text('Date for cash discount missing!') );
