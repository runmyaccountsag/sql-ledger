...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&fxadj=$form->{fxadj}&path=$form->{path}&login=$form->{login}"
...
sub edit {
	...
	foreach $ref ( @{ $form->{GL} } ) {
		...
		$form->{"projectnumber_$i"} =
		  ...
		  if $ref->{project_id};
		for (qw(fx_transaction source memo cleared tax taxamount)) {
			$form->{"${_}_$i"} = $ref->{$_};
		}
	...
	if ( !$form->{readonly} ) {
		if ( $form->{batch} ) {
			$form->{readonly} = 1
			  if $myconfig{acs} =~ /VR--General Ledger/ || $form->{approved};
		}
		else {
			$form->{readonly} = 1
...
sub create_links {
	...
	for ( @{ $form->{all_accno} } ) {
		$form->{selectaccno} .= "$_->{accno}--$_->{description}\n";
	}
	...
	for (qw(department projectnumber accno currency tax)) {
...
sub search {
	...
	$form->{title} =
	  $locale->text('General Ledger') . " " . $locale->text('Reports');
	...
	if ( @{ $form->{all_department} } ) {
	  ...
	  <td><select name=department>|
		  . $form->select_option( $form->{selectdepartment},
			$form->{department}, 1 )
		  . qq|
	  </select></td>
	</tr>
|;
	...
	<select name=month>|
		  ...
		  . qq|</select>
	<select name=year>|
		  . $form->select_option( $selectaccountingyear, $form->{year} )
		  . qq|</select>
...
|;
	...
	if ( @{ $form->{all_report} } ) {
		...
		for ( @{ $form->{all_report} } ) {
			$form->{selectreportform} .=
			  qq|$_->{reportdescription}--$_->{reportid}\n|;
		}
...
|;
	...
	for (qw(transdate reference description debit credit accno)) {
		$form->{"l_$_"} = "checked";
	}

	@checked = qw(l_subtotal);
	@input =
	  qw(reference description name vcnumber lineitem notes source memo datefrom dateto month year accnofrom accnoto amountfrom amountto sort direction reportlogin projectnumber intnotes);
	for (qw(department)) {
	...
	$includeinreport{id} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_id" class=checkbox type=checkbox value=Y $form->{l_id}>|,
...
qq|<input name="l_description" class=checkbox type=checkbox value=Y $form->{l_description}>|,
	...
	$includeinreport{name} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_name" class=checkbox type=checkbox value=Y $form->{l_name}>|,
	...
	$includeinreport{vcnumber} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_vcnumber" class=checkbox type=checkbox value=Y $form->{l_vcnumber}>|,
	...
	$includeinreport{address} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_address" class=checkbox type=checkbox value=Y $form->{l_address}>|,
...
qq|<input name="l_department" class=checkbox type=checkbox value=Y $form->{l_department}>|,
	...
	$includeinreport{projectnumber} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_projectnumber" class=checkbox type=checkbox value=Y $form->{l_projectnumber}>|,
	...
	$includeinreport{notes} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_notes" class=checkbox type=checkbox value=Y $form->{l_notes}>|,
	...
	$includeinreport{debit} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_debit" class=checkbox type=checkbox value=Y $form->{l_debit}>|,
	...
	$includeinreport{credit} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_credit" class=checkbox type=checkbox value=Y $form->{l_credit}>|,
	...
	$includeinreport{source} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_source" class=checkbox type=checkbox value=Y $form->{l_source}>|,
	...
	$includeinreport{memo} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_memo" class=checkbox type=checkbox value=Y $form->{l_memo}>|,
	...
	$includeinreport{lineitem} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_lineitem" class=checkbox type=checkbox value=Y $form->{l_lineitem}>|,
	...
	$includeinreport{accno} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_accno" class=checkbox type=checkbox value=Y $form->{l_accno}>|,
	...
	$includeinreport{accdescription} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_accdescription" class=checkbox type=checkbox value=Y $form->{l_accdescription}>|,
...
qq|<input name="l_gifi_accno" class=checkbox type=checkbox value=Y $form->{l_gifi_accno}>|,
	...
	$includeinreport{contra} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_contra" class=checkbox type=checkbox value=Y $form->{l_contra}>|,
	...
	$includeinreport{intnotes} = {
		...
		checkbox => 1,
		html =>
qq|<input name="l_intnotes" class=checkbox type=checkbox value=Y $form->{l_intnotes}>|,
		label => $locale->text('Internal Notes')
	};
	$includeinreport{include_log} = {
		ndx      => $i++,
		checkbox => 1,
		html =>
qq|<input name="include_log" class=checkbox type=checkbox value=Y $form->{include_log}>|,
		label => $locale->text('Include Log')
	};
	$includeinreport{ts} = {
		ndx      => $i++,
		checkbox => 1,
		html =>
qq|<input name="l_ts" class=checkbox type=checkbox value=Y $form->{l_ts}>|,
		label => $locale->text('TS')
	};
	$includeinreport{curr} = {
		ndx      => $i++,
		checkbox => 1,
		html =>
qq|<input name="l_curr" class=checkbox type=checkbox value=Y $form->{l_curr}>|,
		label => $locale->text('Currency')
	};
	$includeinreport{exchangerate} = {
		ndx      => $i++,
		checkbox => 1,
		html =>
qq|<input name="l_exchangerate" class=checkbox type=checkbox value=Y $form->{l_exchangerate}>|,
		label => $locale->text('Exchange rate')
	};

	@f = ();
	$form->{flds} = "";

	for ( sort { $includeinreport{$a}->{ndx} <=> $includeinreport{$b}->{ndx} }
		keys %includeinreport )
	{
		$form->{flds} .=
		  "$_=$includeinreport{$_}->{label}=$includeinreport{$_}->{sort},";
		push @checked, "l_$_";
		if ( $includeinreport{$_}->{checkbox} ) {
			push @f,
			  "$includeinreport{$_}->{html} $includeinreport{$_}->{label}";
		}
	}
	chop $form->{flds};
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><input name=datefrom size=11 class=date title="$myconfig{dateformat}"> <b>|
	  . $locale->text('To')
	  . qq|</b> <input name=dateto size=11 class=date title="$myconfig{dateformat}"></td>
	</tr>
	...
	<tr>
	  <th align=right>| . $locale->text('Account') . qq| >=</th>
	  <td><input name=accnofrom> <b>|
	  . $locale->text('Account')
	  . qq| <=</b> <input name=accnoto></td>
	</tr>
	
	<tr>
	  <th align=right>| . $locale->text('Amount') . qq| >=</th>
	  <td><input name=amountfrom size=11> <b>|
	  . $locale->text('Amount')
	  . qq| <=</b> <input name=amountto size=11></td>
	</tr>
...
|;
	...
	print qq|
	  ...
	  . qq|</td>
              <td><input type=checkbox class=checkbox name=fx_transaction value=1 checked> |
	  . $locale->text('Exchange Rate Difference')
	  . qq|</td>
              <td><input type=checkbox class=checkbox name=filter_amounts value=1> |
	  . $locale->text('Filter Amounts')
	  . qq|</td>
...
|;
	...
	%button =
	  ( 'Continue' =>
		  { ndx => 1, key => 'C', value => $locale->text('Continue') } );
	...
	$form->hide_form(
		qw(sort direction reportlogin nextsub initreport path login flds));
...
sub transactions {
	...
	( $form->{reportdescription}, $form->{reportid} ) = split /--/,
	  $form->{report};
	$form->{sort} ||= "transdate";
	$form->{reportcode} = 'gl';
	...
	for (
		qw(direction oldsort path login month year interval reportlogin fx_transaction include_log l_ts)
	  )
	{
		$href .= "&$_=$form->{$_}";
	}
	...
	for (
		qw(direction oldsort path login month year interval reportlogin fx_transaction include_log l_ts filter_amounts)
	  )
	{
		$callback .= "&$_=$form->{$_}";
	}
	...
	if ( $form->{accnofrom} ) {
		...
		$option   .= "\n<br>" if $option;
		$option   .= $locale->text('Account')
		  . " >= $form->{accnofrom} $form->{accnofrom_description}";
	}
	if ( $form->{accnoto} ) {
	...
	if ( $form->{amountfrom} ) {
		...
		$option .=
		  $locale->text('Amount') . " >= "
		  . $form->format_amount( \%myconfig, $form->{amountfrom},
			$form->{precision} );
	}
	if ( $form->{amountto} ) {
		...
		$option .=
		  $form->format_amount( \%myconfig, $form->{amountto},
			$form->{precision} );
	}
	...
	if ( $form->{accno} || $form->{gifi_accno} ) {
		@column_index = grep !/(accno|gifi_accno|contra|gifi_contra)/,
		  @column_index;
	...
	if ( $form->{initreport} ) {
		if ( $form->{movecolumn} ) {
			...
			for ( sort { $column_index{$a} <=> $column_index{$b} }
				keys %column_index )
			{
				push @column_index, $_;
			}
		...
		if ( $column_sort{ $column_index[$_] } ) {
			...
			if ( $form->{sort} eq $column_sort{ $column_index[$_] } ) {
				if ( $form->{direction} eq 'ASC' ) {
					$sort =
qq|<span class="noprint"><img src=$images/up.png>&nbsp;&nbsp;&nbsp;</span>|;
				}
				else {
					$sort =
qq|<span class="noprint"><img src=$images/down.png class="noprint" >&nbsp;&nbsp;&nbsp;</span>|;
				}
			}
			print
	...
	foreach $ref ( @{ $form->{GL} } ) {
		...
		$ref->{credit} =
		  $form->format_amount( \%myconfig, $ref->{credit}, $form->{precision},
			"&nbsp;" );
		$ref->{exchangerate} =
		  $form->format_amount( \%myconfig, $ref->{exchangerate}, 8, "&nbsp;" );
...
"<td align=left><a href=$ref->{module}.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{reference}</a></td>";
		if ( $ref->{log} eq '*' ) {
			$column_data{reference} =
"<td align=left><a href=$ref->{module}.pl?action=view&id=$ref->{id}&ts="
		...
		for (qw(department projectnumber name vcnumber address)) {
		...
		if ( $ref->{vc_id} ) {
			$column_data{name} =
"<td align=left><a href=ct.pl?action=edit&id=$ref->{vc_id}&db=$ref->{db}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</a></td>";
		}

		$column_data{debit}  = "<td align=right>$ref->{debit}</td>";
		$column_data{credit} = "<td align=right>$ref->{credit}</td>";
		$column_data{exchangerate} =
		  "<td align=right>$ref->{exchangerate}</td>";

		$column_data{accno} =
"<td align=left><a href=$href&accno=$ref->{accno}&callback=$callback>$ref->{accno}</a></td>";
		$column_data{accdescription} =
		  "<td align=left>$ref->{accdescription}</td>";
		$column_data{curr}   = "<td>$ref->{curr}</td>";
		$column_data{contra} = "<td align=left>";
		for ( split / /, $ref->{contra} ) {
			$column_data{contra} .=
			  qq|<a href=$href&accno=$_&callback=$callback>$_</a>&nbsp;|;
		}
		$column_data{contra} .= "</td>";
		$column_data{gifi_accno} =
"<td align=left><a href=$href&gifi_accno=$ref->{gifi_accno}&callback=$callback>$ref->{gifi_accno}</a>&nbsp;</td>";
		$column_data{gifi_contra} = "<td align=left>";
		for ( split / /, $ref->{gifi_contra} ) {
			$column_data{gifi_contra} .=
			  qq|<a href=$href&gifi_accno=$_&callback=$callback>$_</a>&nbsp;|;
		}
	...
	$column_data{debit} = "<th align=right class=listtotal>"
	  . $form->format_amount( \%myconfig, $totaldebit, $form->{precision},
		"&nbsp;" )
	  . "</th>";
	$column_data{credit} = "<th align=right class=listtotal>"
	  . $form->format_amount( \%myconfig, $totalcredit, $form->{precision},
		"&nbsp;" )
	  . "</th>";
	$column_data{balance} = "<th align=right class=listtotal>"
	  . $form->format_amount( \%myconfig, $form->{balance} * $ml * $cml,
		$form->{precision}, 0 )
	  . "</th>";
...
|;
	...
	%button = (
		...
		'AR--Add Transaction' =>
		  { ndx => 2, key => 'R', value => $locale->text('AR Transaction') },
		'AR--Sales Invoice' =>
		  { ndx => 3, key => 'I', value => $locale->text('Sales Invoice ') },
		'AR--Credit Invoice' =>
		  { ndx => 4, key => 'C', value => $locale->text('Credit Invoice ') },
		'AP--Add Transaction' =>
		  { ndx => 5, key => 'P', value => $locale->text('AP Transaction') },
		'AP--Vendor Invoice' =>
		  { ndx => 6, key => 'V', value => $locale->text('Vendor Invoice ') },
		'AP--Vendor Invoice' =>
		  { ndx => 7, key => 'D', value => $locale->text('Debit Invoice ') },
		'Save Report' =>
...
|;
	...
	$form->hide_form(
		qw(callback path login report reportcode reportlogin column_index flds sort direction)
	);
...
sub transactions_to_csv {
	...
	$href = "$form->{script}?action=transactions";
	for (qw(direction oldsort path login month year interval reportlogin)) {
		$href .= "&$_=$form->{$_}";
	}
	for (qw(report flds)) { $href .= "&$_=" . $form->escape( $form->{$_} ) }
	...
	foreach $ref ( @{ $form->{GL} } ) {
		...
		for (
			qw(department projectnumber name vcnumber address exchangerate curr ts)
		  )
		{
			$column_data{$_} = "$ref->{$_}";
		}
...
sub update {
	...
	$count = 0;
	@flds =
	  qw(accno debit credit taxamount projectnumber tax source memo cleared);
...
sub display_rows {
	...
	for $i ( 1 .. $form->{rowcount} ) {
		...
		$source = qq|<input name="source_$i" size=10 value="|
		  . $form->quote( $form->{"source_$i"} ) . qq|">|;
		$memo = qq|<input name="memo_$i" value="|
		  . $form->quote( $form->{"memo_$i"} ) . qq|">|;

		if ($init) {
			$accno =
			    qq|<select name="accno_$i">|
			  . $form->select_option( $form->{selectaccno} )
			  . qq|</select>|;

			if ( $form->{selectprojectnumber} ) {
				$project =
				  qq|<select name="projectnumber_$i">|
				  . $form->select_option( $form->{selectprojectnumber}, undef,
					1 )
				  . qq|</select>|;
			}
			...
			if ( $form->{selecttax} ) {
				$tax =
				    qq|<select name="tax_$i">|
				  . $form->select_option( $form->{selecttax} )
				  . qq|</select>|;
				$taxamount =
qq|<input name="taxamount_$i" class="inputright" type=text size=12 value="|
				  . $form->format_amount( \%myconfig, $form->{"taxamount_$i"},
					$form->{precision} )
				  . qq|">|;
			}
			...
			if ( $form->{fxadj} ) {
				$fx_transaction =
qq|<td><input name="fx_transaction_$i" class=checkbox type=checkbox value=1></td>|;
...
qq|<input name="taxamount_$i" class="inputright" type=text size=12 value="|
				...
				if ( $form->{fxadj} ) {
					...
					$x = ($checked) ? "x" : "";
					$fx_transaction =
qq|<td><input type=hidden name="fx_transaction_$i" value="$checked">$x</td>|;
					$fx_transaction2 = qq|<td>&nbsp;</td>|;
				}

				$form->hide_form( map { "${_}_$i" }
					  qw(accno projectnumber tax) );
				...
				if ( $form->{selectprojectnumber} ) {
					$project = qq|<select name="projectnumber_$i">|
...
qq|<textarea name=description rows=$rows cols=50 wrap=soft>$form->{description}</textarea>|;
		...
		$description = qq|<input name=description size=50 value="|
...
qq|<textarea name=notes rows=$rows cols=50 wrap=soft>$form->{notes}</textarea>|;
		...
		$notes = qq|<input name=notes size=50 value="|
		  . $form->quote( $form->{notes} ) . qq|">|;
	}
	...
	if ( !$form->{fxadj} ) {
		...
		if ( $form->{currency} ne $form->{defaultcurrency} ) {
			...
			$exchangerate .= qq|
			  ...
			  . $locale->text('Buy')
			  . qq|</th><td>|
			  . $form->format_amount( \%myconfig, $form->{fxbuy} )
			  . qq|</td>
              <th align=right nowrap>|
			  . $locale->text('Sell')
			  . qq|</th><td>|
			  . $form->format_amount( \%myconfig, $form->{fxsell} )
			  . qq|</td>|;
		}
	...
	$department = qq|
	  ...
	  <td><select name=department>|
	  . $form->select_option( $form->unescape( $form->{selectdepartment} ),
		$form->{department}, 1 )
	  . qq|</select></td>
...
| if $form->{selectprojectnumber} and !$form->{selecttax};
	...
	if ( $form->{selecttax} ) {
		...
		$tax .= qq| / | . $locale->text('Project')
		  if $form->{selectprojectnumber};
		$tax .= qq|</th>|;
		$taxamount =
		  qq|<th class=listheading>| . $locale->text('Tax Amount') . qq|</th>|;
	}
...
|;
	...
	$form->hide_form(
		qw(id fxadj closedto locked oldtransdate oldcurrency recurring batch batchid batchnumber batchdescription defaultcurrency fxbuy fxsell precision)
	);
...
| if $form->{selectprojectnumber};
	...
	$taxamount = qq|
	  ...
	  . $form->format_amount( \%myconfig, $form->{totaltaxamount}, 2 )
	  . qq|</th>
| if $form->{selecttax};
...
|;
	...
	if ( $form->{readonly} ) {
		...
		if ( $form->{id} ) {
			for ( 'Update', 'Post as new', 'Schedule', 'New Number' ) {
				$a{$_} = 1;
			}
			...
			if ( $transdate > $form->{closedto} ) {
				for ( "Update", "Post", "Schedule", "New Number" ) {
					$a{$_} = 1;
				}
			}
		}
...
'Cannot post transaction with a debit and credit entry for the same account!'
	...
	if ( $form->round_amount( $debit, $form->{precision} ) !=
		$form->round_amount( $credit, $form->{precision} ) )
	{
