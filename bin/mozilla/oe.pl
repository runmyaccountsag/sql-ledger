...
sub add {
	...
	$form->{callback} =
"$form->{script}?action=add&type=$form->{type}&vc=$form->{vc}&login=$form->{login}&path=$form->{path}"
...
sub order_links {
	...
	if ( @{ $form->{all_partsgroup} } ) {
		...
		foreach $ref ( @{ $form->{all_partsgroup} } ) {
			if ( $ref->{translation} ) {
				$form->{selectpartsgroup} .=
				  qq|$ref->{translation}--$ref->{id}\n|;
			}
			else {
				$form->{selectpartsgroup} .=
				  qq|$ref->{partsgroup}--$ref->{id}\n|;
			}
		}
	}
	...
	for (
		qw(currency partsgroup projectnumber department warehouse employee language)
	  )
	{
...
sub prepare_order {
	...
	if ( $form->{id} ) {
		...
		for (
			qw(ordnumber quonumber shippingpoint shipvia waybill notes intnotes shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptocontact)
		  )
		{
	...
	if ( $form->{type} eq 'sales_quotation' ) {
		...
		$form->{selectformname} =
		  qq|sales_quotation--| . $locale->text('Quotation');
	}
	...
	if ( $form->{type} eq 'request_quotation' ) {
		...
		$form->{selectformname} =
		  qq|request_quotation--| . $locale->text('RFQ');
	}
...
sub lookup_order {
	...
	if ( $id = OE->lookup_order( \%myconfig, \%$form ) ) {
		$type =
		  ( $form->{vc} eq 'customer' ) ? 'sales_order' : 'purchase_order';
		$form->{callback} =
"oe.pl?action=edit&id=$id&type=$type&vc=$form->{vc}&path=$form->{path}&login=$form->{login}";
		$form->redirect;
	}
	else {
...
|;
	...
	$terms = qq|
                    <tr>
		      <th align=right nowrap>|
	  . $locale->text('Terms') . qq| |
	  . $locale->text('Net')
	  . qq|</th>
		      <td nowrap><input name=terms size="3" maxlength="3" value=$form->{terms}> <b>|
	  . $locale->text('days')
	  . qq|</b></td>
                    </tr>
|;
...
|;
		...
		<th width=70% align=right nowrap>|
			  ...
			  . qq|</th>
		<td><input name=quonumber size=20 value="|
			  . $form->quote( $form->{quonumber} )
			  . qq|"></td>|
...
| if $form->{selectwarehouse};
	...
	if ( $form->{type} eq 'sales_order' ) {
		...
		<td><select name=employee>|
			  . $form->select_option( $form->{selectemployee},
				$form->{employee}, 1 )
			  . qq|</select>
		</td>
	      </tr>
...
|;
	...
	$title = " / $title{$form->{formname}}"
	  if $form->{formname} !~ /(sales_order|purchase_order|quotation)/;
...
|;
	...
	$form->hide_form(
		qw(id type defaultcurrency formname printed emailed queued vc title discount creditlimit creditremaining tradediscount business oldtransdate recurring city state country closedto precision)
	);

	$form->hide_form("select$form->{vc}");
	$form->hide_form( map { "select$_" }
		  qw(formname currency partsgroup projectnumber department warehouse employee language)
	);
	$form->hide_form( "$form->{vc}_id", "old$form->{vc}",
		"old$form->{vc}number" );
  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		<td colspan=3><input name=shippingpoint size=35 value="|
	  . $form->quote( $form->{shippingpoint} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right>| . $locale->text('Ship via') . qq|</th>
		<td colspan=3><input name=shipvia size=35 value="|
	  . $form->quote( $form->{shipvia} )
	  . qq|"></td>
	      </tr>
	      <tr>
		<th align=right>| . $locale->text('Waybill') . qq|</th>
		<td colspan=3><input name=waybill size=35 value="|
	  . $form->quote( $form->{waybill} )
	  . qq|"></td>
	      </tr>
	    </table>
	  </td>
...
|;
	...
	$form->hide_form(
		qw(shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptocontact shiptophone shiptofax shiptoemail message email subject cc bcc taxaccounts aa_id inventory_items)
	);

	foreach $accno ( split / /, $form->{taxaccounts} ) {
		$form->hide_form( map { "${accno}_$_" }
		...
		for ( split / /, $form->{taxaccounts} ) {
			if ( $form->{"${_}_base"} ) {
				$form->{invtotal} += $form->{"${_}_total"} =
				  $form->round_amount(
					$form->{"${_}_base"} * $form->{"${_}_rate"},
					$form->{precision} );
				$form->{"${_}_total"} =
				  $form->format_amount( \%myconfig, $form->{"${_}_total"},
					$form->{precision} );
...
|;
		...
		$form->{invsubtotal} =
		  $form->format_amount( \%myconfig, $form->{invsubtotal},
			$form->{precision}, 0 );
...
|;
	...
	if ( $form->{readonly} ) {
		...
		%button = (
			...
			'Ship all' =>
			  { ndx => 5, key => 'A', value => $locale->text('Ship all') },
			'E-mail' =>
			  { ndx => 6, key => 'E', value => $locale->text('E-mail') },
			'Save as new' =>
			  { ndx => 8, key => 'N', value => $locale->text('Save as new') },
			'Sales Invoice' => {
				ndx   => 10,
				key   => 'I',
				value => $locale->text('Sales Invoice')
			},
			'Sales Order' =>
			...
			'Schedule' =>
			  { ndx => 16, key => 'H', value => $locale->text('Schedule') },
			'Delete' =>
			  { ndx => 17, key => 'D', value => $locale->text('Delete') },
		);
...
sub update {
	...
	$form->isvaldate( \%myconfig, $form->{reqdate},
		$locale->text('Invalid required date ...') );
		...
		&& ( $form->{"partsgroup_$i"}  eq "" ) )
	{
		...
		if ($rows) {
			...
			if ( $rows > 1 ) {
				...
				for (qw(qty discount netweight grossweight)) {
					$form->{"${_}_$i"} =
					  $form->format_amount( \%myconfig, $form->{"${_}_$i"} );
				}
...
sub search {
	...
	if ( $form->{type} eq 'generate_purchase_order' ) {
		...
		$detail     = qq|
                <tr>
		  <td><input name=detail type=radio class=radio value=0 checked> |
		  . $locale->text('Summary')
		  . qq|</td>
		  <td><input name=detail type=radio class=radio value=1> |
...
qq|<input name="l_employee" class=checkbox type=checkbox value=Y> $employee|;
	...
	$vcnumber = $locale->text('Customer Number');
	$l_name =
qq|<input name="l_name" class=checkbox type=checkbox value=Y checked> $vcname|;
	$l_customernumber =
	   ...
	   <td colspan=3><select name=$form->{vc}><option>\n|;
		...
		for ( @{ $form->{"all_$form->{vc}"} } ) {
			$vc .=
			    qq|<option value="|
			  . $form->quote( $_->{name} )
			  . qq|--$_->{id}">$_->{name} ($_->{"$form->{vc}number"})\n|;
		}
...
qq|<input name="l_name" class=checkbox type=checkbox value=Y checked> $vcname|;
	push @a,
qq|<input name="l_$form->{vc}number" class=checkbox type=checkbox value=Y checked> $vcnumber|;
...
sub transactions {
	...
	for (qw(oldsort direction path type vc login)) {
		$href .= qq|&$_=$form->{$_}|;
	}
	...
	if ( $form->{vc} eq 'vendor' ) {
		...
		if ( $form->{type} eq 'generate_sales_order' ) {
			...
			if ( $myconfig{acs} !~ /Order Entry--Order Entry/ ) {
				$button{'Order Entry--Sales Order'}{code} =
				  qq|<input class=submit type=submit name=action value="|
				  . $locale->text('Generate Sales Order') . qq|"> |;
		...
		if ( $form->{type} eq 'consolidate_purchase_order' ) {
			...
			if ( $myconfig{acs} !~ /Order Entry--Order Entry/ ) {
				$button{'Order Entry--Purchase Order'}{code} =
				  qq|<input class=submit type=submit name=action value="|
				  . $locale->text('Consolidate Orders') . qq|"> |;
				$button{'Order Entry--Purchase Order'}{order} = $i++;
			}
		}
	...
	if ( $form->{vc} eq 'customer' ) {
		...
		if ( $form->{type} eq 'generate_purchase_order' ) {
			...
			if ( $myconfig{acs} !~ /Order Entry--Order Entry/ ) {
				$button{'Order Entry--Purchase Order'}{code} =
				  qq|<input class=submit type=submit name=action value="|
				  . $locale->text('Generate Purchase Orders') . qq|"> |;
				$button{'Order Entry--Purchase Order'}{order} = $i++;
			}
...
qq|<th><a class=listheading href=$href&sort=$form->{vc}number>$vcnumber</a></th>|;
	...
	$column_header{shipvia} =
	  ...
	  . $locale->text('Ship via')
	  . qq|</a></th>|;
	$column_header{waybill} =
	    qq|<th><a class=listheading href=$href&sort=waybill>|
	  . $locale->text('Waybill')
	  . qq|</a></th>|;
	...
	$column_header{intnotes} =
	  qq|<th class=listheading>| . $locale->text('Internal Notes') . qq|</th>|;
	$column_header{open} =
...
|;
	...
	foreach $ref ( @{ $form->{OE} } ) {
		...
		$id =
		  ( $ref->{orderitemsid} )
		  ? "$ref->{id}--$ref->{orderitemsid}"
		  : $ref->{id};

		$column_data{ndx} =
qq|<td><input name="ndx_$i" class=checkbox type=checkbox value=$id checked></td>|;
...
qq|<td><a href=ct.pl?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{"$form->{vc}_id"}&db=$form->{vc}&callback=$callback>$ref->{name}</a></td>|;
		...
		for (
			qw(employee manager warehouse shipvia shippingpoint waybill curr ponumber notes intnotes)
		  )
		{
			$column_data{$_} = "<td>$ref->{$_}&nbsp;</td>";
		}
...
|;
	...
	$form->hide_form(
		qw(callback type vc path login department ordnumber ponumber detail));
...
sub subtotal {
	...
	$column_data{netamount} = "<th class=listsubtotal align=right>"
	  . $form->format_amount( \%myconfig, $subtotalnetamount,
		$form->{precision}, "&nbsp;" )
	  . "</th>";
	...
	if ( $form->{l_curr} && $form->{sort} eq 'curr' && $form->{l_subtotal} ) {
		$column_data{fx_netamount} = "<th class=listsubtotal align=right>"
		  . $form->format_amount( \%myconfig, $subtotalfxnetamount,
			$form->{precision}, "&nbsp;" )
		  . "</th>";
...
sub invoice {
	...
	if ( $form->round_amount( $totalship, 1 ) == 0 ) {
		...
		$form->{add_shipping} = 1
		  if ( $form->{type} =~ /_order$/ )
		  ;    # Flag to add rows to inventory table
	}
	...
	if ( $form->{currency} ne $form->{defaultcurrency} ) {
		$form->{exchangerate} =
		  $form->check_exchangerate( \%myconfig, $form->{currency},
			$form->current_date( \%myconfig ), $buysell );
	}
	...
	for (
		qw(currency oldcurrency exchangerate employee department warehouse intnotes notes taxincluded)
	  )
	{
		$temp{$_} = $form->{$_};
	}
...
|;
	...
	$form->hide_form(
		qw(id type printed emailed queued vc weightunit description));
  ...
  <tr>
    <td>
	  ...
	  . $form->hide_form( "$form->{vc}", "$form->{vc}_id" ) . qq|
		...
		<th align=right>| . $locale->text('Shipping Point') . qq|</th>
		<td colspan=3>
		<input name=shippingpoint size=35 value="|
	  . $form->quote( $form->{shippingpoint} ) . qq|">
	      </tr>
	      <tr>
		<th align=right>| . $locale->text('Ship via') . qq|</th>
		<td colspan=3>
		<input name=shipvia size=35 value="|
	  . $form->quote( $form->{shipvia} ) . qq|">
	      </tr>
	      <tr>
		<th align=right>| . $locale->text('Waybill') . qq|</th>
		<td colspan=3>
		<input name=waybill size=35 value="|
	  . $form->quote( $form->{waybill} ) . qq|">
	      </tr>
	      $warehouse
	    </table>
	  </td>
...
|;
	...
	for $i ( 1 .. $form->{rowcount} - 1 ) {
		...
		$column_data{partnumber} =
		  qq|<td>| . $form->quote( $form->{"partnumber_$i"} ) . qq|</td>|;
		$column_data{sku} =
		  qq|<td>| . $form->quote( $form->{"sku_$i"} ) . qq|</td>|;
		$column_data{description} =
		  qq|<td>| . $form->quote($description) . qq|</td>|;
		$column_data{qty} =
		    qq|<td align=right>|
		  . $form->format_amount( \%myconfig, $form->{"qty_$i"} )
		  . qq|</td>|;
		$column_data{ship} =
		    qq|<td align=right><input name="ship_$i" size=8 value=|
		  . $form->format_amount( \%myconfig, $form->{"ship_$i"} )
		  . qq|></td>|;
		$column_data{unit} =
		  qq|<td>| . $form->quote( $form->{"unit_$i"} ) . qq|</td>|;
		$column_data{bin} =
		  qq|<td>| . $form->quote( $form->{"bin_$i"} ) . qq|</td>|;

		$column_data{serialnumber} =
		    qq|<td><input name="serialnumber_$i" size=15 value="|
		  . $form->quote( $form->{"serialnumber_$i"} )
		  . qq|"></td>|;
...
|;
		...
		$form->hide_form( map { "${_}_$i" }
			  qw(partnumber description itemnotes lineitemdetail sku orderitems_id id partsgroup unit bin qty weight)
		);
		...
		<tr>
		  <td colspan=$colspan>
		  <b>$packagenumber</b>
		  <input name="package_$i" size=20 value="|
			  . $form->quote( $form->{"package_$i"} ) . qq|">
		  <b>$netweight</b>
		  <input name="netweight_$i" size=8 value=|
			  . $form->format_amount( \%myconfig, $form->{"netweight_$i"} )
			  . qq|>
		  <b>$grossweight</b>
		  <input name="grossweight_$i" size=8 value=|
			  . $form->format_amount( \%myconfig, $form->{"grossweight_$i"} )
			  . qq|> ($form->{weightunit})
		  <b>$volume</b>
		  <input name="volume_$i" size=8 value=|
			  . $form->format_amount( \%myconfig, $form->{"volume_$i"} ) . qq|>
		  </td>
		</tr>
		<tr>
		  <td colspan=$colspan>
...
|;
	...
	%button = (
		...
		'Ship all' =>
		  { ndx => 5, key => 'A', value => $locale->text('Ship all') },
		'Receive all' =>
		  { ndx => 5, key => 'A', value => $locale->text('Receive all') },
		'Ship to' =>
		  { ndx => 4, key => 'T', value => $locale->text('Ship to') },
...
|;
	...
	$form->hide_form(
		qw(callback department ponumber path login employee_id vc nextsub rowcount type detail)
	);

	print qq|
<input class=submit type=submit name=action value="|
	  . $locale->text('Generate Orders') . qq|">|;

	print qq|
<input class=submit type=submit name=action value="|
	  . $locale->text('Select Vendor') . qq|">|;
