...
sub login {
	...
	if ( ( $errno = $user->login( \%$form, $userspath ) ) <= -1 ) {
		...
		if ( $errno == 4 ) {
			...
			print
"<p><a href=menu.pl?login=$form->{login}&path=$form->{path}&action=display&main=company_logo&js=$form->{js}&password=$form->{password}>"
	...
	for (qw(login path password js sessioncookie)) {
...
sub logout {
	...
	$form->{callback} =
	  "$form->{script}?path=$form->{path}&login=$form->{login}&endsession=1";
	$form->remove_locks( \%myconfig );
