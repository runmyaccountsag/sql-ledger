...
sub getpassword {
	...
	if ( $form->{stylesheet} && ( -f "css/$form->{stylesheet}" ) ) {
		$stylesheet =
qq|<LINK REL="stylesheet" HREF="css/$form->{stylesheet}" TYPE="text/css" TITLE="Run my Accounts stylesheet">
|;
	}

	if ( $form->{charset} ) {
		$charset =
qq|<META HTTP-EQUIV="Content-Type" CONTENT="text/plain; charset=$form->{charset}">
|;
	}
...
|;
	...
	$sessionexpired =
	    qq|<b><font color=red><blink id=errorMessage>|
