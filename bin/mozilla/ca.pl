...
|;
	...
	foreach $ca ( @{ $form->{CA} } ) {
		...
		$href =
qq|$form->{script}?path=$form->{path}&action=list&accno=$ca->{accno}&login=$form->{login}&description=$description&gifi_accno=$ca->{gifi_accno}&gifi_description=$gifi_description|;
...
"<td><a href=$href&accounttype=gifi>$ca->{gifi_accno}</a>&nbsp;</td>";
		...
		$column_data{debit} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $ca->{debit}, $form->{precision},
			"&nbsp;" )
		  . "</td>\n";
		$column_data{credit} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $ca->{credit}, $form->{precision},
			"&nbsp;" )
		  . "</td>\n";
...
|;
	...
	$column_data{debit} =
	  ...
	  . $form->format_amount( \%myconfig, $totaldebit, $form->{precision}, 0 )
	  . "</th>";
	$column_data{credit} =
	    "<th align=right class=listtotal>"
	  . $form->format_amount( \%myconfig, $totalcredit, $form->{precision}, 0 )
	  . "</th>";
...
sub list {
	...
	if ( !$form->{accno} ) {
		...
		( $form->{accno} ) = $dbh->selectrow_array(
"SELECT fldvalue FROM defaults WHERE fldname='selectedaccount' LIMIT 1"
		);
		( $form->{description} ) = $dbh->selectrow_array(
			"SELECT description FROM chart WHERE accno = '$form->{accno}'");
		$clearing_account = 1;
	}
...
|;
	...
	$form->hide_form(
		qw(accno description accounttype gifi_accno gifi_description login path)
	);
...
<table border=0 width=100%>
  ...
  <tr valign=top>
	...
	<tr>
      ...
      <input type=checkbox class=checkbox name=filter_marked value=1> |
	  . $locale->text('Filter marked transactions')
	  . qq|</td>
	  </td>
	</tr>
      </table>
...
sub list_transactions {
	...
	my $dbh = $form->dbconnect( \%myconfig );
	my ($clearing_account) = $dbh->selectrow_array(
"SELECT 1 FROM defaults WHERE fldname = 'selectedaccount' AND fldvalue = '$form->{accno}'"
	);
...
"$form->{script}?action=list_transactions&department=$department&projectnumber=$projectnumber&title=$title";
	for (
		qw(path oldsort accno login fromdate todate accounttype gifi_accno l_heading l_subtotal l_accno l_name fx_transaction)
	  )
	{
		$href .= "&$_=$form->{$_}";
	}
	...
	if ( $form->{l_name} ) {
		...
		if ( $form->{link} =~ /_paid/ ) {
			@columns =
			  qw(transdate reference description name source cleared debit credit);
		}
	}
	...
	$form->{title} .=
	  " $form->{accno} - $form->{description} / $form->{company}";
...
"$form->{script}?action=list_transactions&department=$department&projectnumber=$projectnumber&title=$title";
	for (
		qw(path direction oldsort accno login fromdate todate accounttype gifi_accno l_heading l_subtotal l_accno prevreport fx_transaction filter_marked)
	  )
	{
...
|;
	...
	if ( $form->{accno} && $form->{balance} ) {
		...
		$column_data{balance} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $form->{balance} * $ml,
			$form->{precision}, 0 )
		  . "</td>";
...
|;
	...
	foreach $ca ( @{ $form->{CA} } ) {
		...
		$href =
"<a href=$ca->{module}.pl?path=$form->{path}&action=edit&id=$ca->{id}&login=$form->{login}&callback=$form->{callback}>$ca->{reference}</a>";
		$column_data{debit} = "<td align=right>"
		  . $form->format_amount( \%myconfig, $ca->{debit}, $form->{precision},
			"&nbsp;" )
		  . "</td>";
		...
		if ($clearing_account) {
			$cl_link =
"cl.pl?action=continue&nextsub=list_trans&accno=$form->{accno}&trans_id=$ca->{id}&path=$form->{path}&login=$form->{login}&filter_marked=$form->{filter_marked}&callback=$form->{callback}";
			$column_data{transdate} =
qq|<td nowrap><a href="$cl_link">$ca->{transdate}</a> $found</td>|;
		}
		else {
		...
		$href =
"<a href=ct.pl?path=$form->{path}&action=edit&id=$ca->{vc_id}&db=$ca->{db}&login=$form->{login}&callback=$form->{callback}>$ca->{name}</a>";
...
sub ca_subtotal {
	...
	$column_data{debit} = "<th align=right class=listsubtotal>"
	  . $form->format_amount( \%myconfig, $subtotaldebit, $form->{precision},
		"&nbsp;" )
	  . "</th>";
	$column_data{credit} = "<th align=right class=listsubtotal>"
