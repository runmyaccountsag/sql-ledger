...
<table><tr>
<th align="right">|
	  . $locale->text('GL Account')
	  . qq|</th><td><select name=gl_account_id>$selectaccno</select></td>
</tr></table>
</td></tr></table>
|;
    ...
    ";
	my ( $debit, $credit ) =
	     $dbs->query( $query, $form->{trans_id}, $form->{accno} )->list
	...
	if ( $form->{arap} eq 'ar' ) {
		$selectarap =
qq|<option value="$form->{arap}" selected>$form->{arap}\n<option value="ap">ap\n|;
	}
	else {
		$selectarap =
qq|<option value="$form->{arap}" selected>$form->{arap}\n<option value="ar">ar\n|;
...
<tr>
<th align="right">|
	  . $locale->text("From date")
	  . qq|</th><td><input type=text size=12 class="date" title="$myconfig{dateformat}" name=fromdate value='$form->{fromdate}'></td>
</tr>
<tr>
<th align="right">|
	  . $locale->text("To date")
	  . qq|</th><td><input type=text size=12 class="date" title="$myconfig{dateformat}" name=todate value='$form->{todate}'></td>
</tr>
<tr>
<th align="right">|
	  . $locale->text("AR or AP?")
	  . qq|</th><td><select name=arap>$selectarap</select></td>
</tr>
</table>
<hr/>
...
|;
	...
	my @allrows = $dbs->query( $query, @bind )->hashes
	  or die('No transactions found ...');

	my @report_columns =
	  qw(x invnumber transdate description ordnumber name amount paid due);
	my @total_columns = qw(amount paid due);
	my ( %tabledata, %totals, %subtotals );

	$href = qq|$form->{script}?action=list_trans|;
	for (qw(fromdate todate arap path login accno trans_id)) {
		$href .= "&$_=$form->{$_}";
	}
	for (@report_columns) {
		$tabledata{$_} =
qq|<th><a href="$href&sort=$_&oldsort=$sort&sortorder=$sortorder" class="listheading">|
		  . ucfirst $_
		  . qq|</th>\n|;
	}
...
|;
	...
	for $row (@allrows) {
		...
		$url =
qq|$arap.pl?id=$row->{id}&action=edit&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
		$tabledata{invnumber} =
		  qq|<td><a href="$url" target=_blank>$row->{invnumber}</a></td>|;

		$row->{amount} *= 1;
		$checked = '';
		if (   $row->{amount} == $search_amount
			or $row->{amount} * -1 == $search_amount )
		{
			$checked = 'checked';
		}
		...
		$tabledata{x} =
qq|<td><input type=checkbox class=checkbox name=x_$j $checked><input type=hidden name=id_$j value=$row->{id}></td>|;
...
"SELECT id FROM chart WHERE accno = (SELECT fldvalue FROM defaults WHERE fldname='selectedaccount')"
	)->list;
...
"SELECT id FROM chart WHERE accno = (SELECT fldvalue FROM defaults WHERE fldname='transitionaccount')"
		)->list;
		( $gl_accno, $gl_description ) =
		  $dbs->query( "SELECT accno, description FROM chart WHERE id = ?",
			$transition_accno_id )->list;
		$rows[$row_id][4] = $gl_accno;
		$rows[$row_id][5] = $gl_description;
	}
	elsif ( $form->{gl_account_id} ) {
		my ( $gl_accno, $gl_description ) =
		  $dbs->query( "SELECT accno, description FROM chart WHERE id = ?",
			$form->{gl_account_id} )->list;
		$rows[$row_id][4] = $gl_accno;
		$rows[$row_id][5] = $gl_description;
	}
	use DBIx::XHTML_Table;
	my $headers = [
		qw(ID Reference Date Account_ID Account Account_Description Description Source Memo Debit Credit)
	];
	...
	$table->map_cell(
		sub {
			my $datum = shift;
			return
qq|<a href="gl.pl?action=edit&id=$datum&path=$form->{path}&login=$form->{login}">$datum</a>|;
		},
		'id'
	);
...
#print $table->output;
		...
		for (@rows) {
			$link =
"$_->{module}?id=$_->{id}&action=edit&path=$form->{path}&login=$form->{login}";
          ...
          <td>$_->{transdate}</td>
          <td align="right">|
			  . $form->format_amount( \%myconfig, $_->{amount}, 2 )
			  . qq|</td>
          </tr>|;
		...
		print qq|<tr class="listtotal">
          ...
          <td>&nbsp;</td>
          <td align="right">|
		  . $form->format_amount( \%myconfig, $total_amount, 2 )
		  . qq|</td>
          </tr>|;
		print qq|</table>|;
...
"SELECT id FROM chart WHERE accno = (SELECT fldvalue FROM defaults WHERE fldname='transitionaccount')"
	...
	my $gl_amount = $dbs->query(
		"SELECT amount FROM acc_trans WHERE chart_id = ? AND trans_id = ? ",
		$clearing_accno_id, $form->{trans_id} )->list;

	# Get payment date from GL transaction
	my $gl_date =
	  $dbs->query( "SELECT transdate FROM gl WHERE id = ?", $form->{trans_id} )
	  ->list;
	...
	my $adjustment_available = $dbs->query(
		"SELECT 0-amount FROM acc_trans WHERE chart_id = ?  AND trans_id = ?",
		$clearing_accno_id, $form->{trans_id} )->list;
	...
	for (@rows) {
		...
		my $ARAP = uc $arap;
		$arap_date =
		  $dbs->query( "SELECT transdate FROM $arap WHERE id = ?", $_->{id} )
		  ->list;
		...
		my $arap_accno_id = $dbs->query( "
         SELECT chart_id FROM acc_trans WHERE trans_id = ? AND chart_id IN (SELECT id FROM chart WHERE link LIKE '$ARAP') LIMIT 1",
			$_->{id} )->list;
		if ( $arap eq 'ap' ) {
			$dbs->query( "
          INSERT INTO acc_trans(trans_id, chart_id, transdate, amount)
          VALUES (?, ?, ?, ?)", $_->{id}, $transition_accno_id, $payment_date,
				$amount_to_be_adjusted )
			  or $form->error( $dbs->error );
			$dbs->query( "
          INSERT INTO acc_trans(trans_id, chart_id, transdate, amount)
          VALUES (?, ?, ?, ?)", $_->{id}, $arap_accno_id, $payment_date,
				$_->{due} * -1 )
			  or $form->error( $dbs->error );
		}
		else {
		...
		$dbs->query(
			...
			$amount_to_be_adjusted, $payment_date, $_->{id} )
		  or $form->error( $dbs->error );
	...
	if ( !$amount ) {
		$dbs->query(
			"DELETE FROM acc_trans WHERE chart_id = ? AND trans_id = ?",
